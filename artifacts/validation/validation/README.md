...
- 病名 patientId 欠落（curl 実行ログ）: `logs/curl_disease_missing_patientid_400.txt`
- 監査イベント一覧: `logs/audit_event_recent.txt`
- 404（/orca/order/bundles）: `logs/order_bundle_missing_patientid_400_headers.txt`, `logs/order_bundle_missing_patientid_400_body.json`
- 404（/orca/order/bundles, curl 実行ログ）: `logs/curl_order_bundle_missing_patientid_404.txt`
...
- `/orca/order/bundles` の 404 応答は `Content-Length: 0` のため body なし（`logs/order_bundle_missing_patientid_400_body.json` は空ファイル）。
