...
export function setup() {
  const etags = {};
  const warmTargets = [
    cfg.scenarios?.orca05CacheHit?.path,
    cfg.scenarios?.orca06Address?.path,
  ].filter(Boolean);

  for (const target of warmTargets) {
    const res = http.get(`${base}${target}`, { headers: commonHeaders });
...
export default function (data) {
  ...
  const expectCache = target === cfg.scenarios?.orca05CacheHit?.path
    || target === cfg.scenarios?.orca06Address?.path;
  const etag = data?.etags?.[target];
  if (expectCache && etag) {
    params.headers = Object.assign({}, params.headers, { 'If-None-Match': etag });
  }
  ...
  check(res, {
    ...
    'cache-hit returns 304': (r) => !expectCache || r.status === 304 || !etag,
