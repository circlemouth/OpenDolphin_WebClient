...
(async () => {
  ...
  async function fetchEtag(path) {
    try {
      const res = await fetch(`${base}${path}`, { headers: commonHeaders });
      if (!res.ok) {
        return null;
      }
  ...
  for (const c of cases) {
    ...
    if (c.expectCache) {
      if (!etagCache[c.path]) {
        etagCache[c.path] = await fetchEtag(c.path);
      }
      if (etagCache[c.path]) {
        headers['if-none-match'] = etagCache[c.path];
      }
    }
