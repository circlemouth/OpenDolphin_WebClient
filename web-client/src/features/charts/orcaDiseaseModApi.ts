...
export const ORCA_DISEASEV3_PATH = '/orca22/diseasev3?class=01';

export async function postOrcaDiseaseV3Xml(requestXml: string): Promise<OrcaXmlResponse> {
  const runId = getObservabilityMeta().runId;
  const response = await httpFetch(ORCA_DISEASEV3_PATH, {
    method: 'POST',
    headers: {
