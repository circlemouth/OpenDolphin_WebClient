...
- 日数/回数の追加入力制約・自動補正・初期値は旧来版との差分なし。
...
- 日数/回数は用法入力後に編集可能、頓用/臨時時は院内/院外変更不可のヘルプを表示。
- 頓用/臨時は「回数」、通常は「日数」としてラベル/説明文を切り替える。
...
- `/orca/order/bundles` が classCode/className/classCodeSystem の保存・取得に対応していること。
  ...
  - OrcaOrderBundleResource で classCode/classCodeSystem/className の保存・復元が実装済みであることを確認。
