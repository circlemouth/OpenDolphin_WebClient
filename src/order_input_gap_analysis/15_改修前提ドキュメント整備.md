...
- 25_P0改修_用法検索UI.md
  - 用法マスタ検索 API の検索条件/戻り形式/表示項目を確認する。
  - 用法コード/表示名の反映先と、既存の用法入力欄との整合を定義する。
- 26_P0改修_薬剤材料行追加.md
  - 薬剤/材料のマスタ検索 API（slot/カテゴリ条件）を確認する。
  - items への追加ルール（行種別/初期値）と既存の手入力 UI との整合を整理する。
...
- 27_P1改修_処方詳細入力補助.md
  - 院内/院外・頓用/臨時・日数/回数の保存項目を確認する。
  - RP名補正のルール（空欄時のデフォルト）を Web 側で定義する。
- 28_P1改修_リハビリ部位コメント入力.md
  - 部位マスタ/コメントコードの API 有無と入力保存先を確認する。
  - memo 入力との使い分け方針（コメントコード優先/併用可否）を整理する。

### P2
- 24_P2改修_リハビリ予約連携.md
  - 予約/スケジュール API の有無と利用可否を確認する。
  - 実装不可時の代替運用（手入力/別システム）の文言を整理する。
- 25_P2改修_履歴コピー_スタンプコピー.md
  - オーダ履歴 API の取得範囲/参照権限を確認する。
  - スタンプデータ構造とコピー/ペーストの保存範囲を確認する。
  - スタンプ保存/取り込み UI との共存ルールを整理する。

## 進捗反映メモ（2025-12-30）
- 21_P1改修_DnD行操作.md: OrderBundleEditPanel の DnD 並べ替え/全クリア/行削除補助を実装済み。
  - 参照: web-client/src/features/charts/OrderBundleEditPanel.tsx
- 25_P2改修_履歴コピー_スタンプコピー.md: 履歴コピー/スタンプコピー・ペースト導線を実装済み。
  - 参照: web-client/src/features/charts/OrderBundleEditPanel.tsx
  - 参照: web-client/src/features/charts/__tests__/orderBundleHistoryCopy.test.tsx
  - 参照: web-client/src/features/charts/__tests__/orderBundleStampFlow.test.tsx
- 28_P1改修_リハビリ部位コメント入力.md: リハビリ部位検索/コメントコード入力 UI を実装済み。
  - 参照: web-client/src/features/charts/OrderBundleEditPanel.tsx
