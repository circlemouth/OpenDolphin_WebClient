...
| オーダ履歴コピー | 旧来版はオーダ履歴からコピー可能。Web はUIなし。 | オーダ履歴 / client/src/main/java/open/dolphin/impl/care/OrderHistoryPanel.java | Charts 右パネル / web-client/src/features/charts/OrderBundleEditPanel.tsx | 旧来版: オーダ履歴パネルの行をコピー。Web: 履歴コピー導線がない。 | P2 |
| スタンプコピー/ペースト | 旧来版はスタンプのコピー/ペーストが可能。Web はUIなし。 | スタンプツリー / client/src/main/java/open/dolphin/stampbox/StampTreePopupAdapter.java | Charts 右パネル / web-client/src/features/charts/OrderBundleEditPanel.tsx | 旧来版: スタンプ右クリックでコピー/ペースト。Web: スタンプ複製導線がない。 | P2 |
...
| RP名（空欄補正） | 旧来版は空欄時にデフォルト補正。Web は必須入力。 | RpEditor / client/src/main/java/open/dolphin/order/RpEditor.java | OrderBundleEditPanel / web-client/src/features/charts/OrderBundleEditPanel.tsx | 旧来版: RP名を空欄で保存すると補正。Web: RP名が必須。 | P1 |
| 薬剤/材料行 | 旧来版は検索選択で追加。Web は手入力のみ。 | RpEditor / client/src/main/java/open/dolphin/order/RpEditor.java | OrderBundleEditPanel / web-client/src/features/charts/OrderBundleEditPanel.tsx | 旧来版: 薬剤/材料を検索して追加。Web: items を手入力。 | P0 |
...
| 文書種別（紹介状/診断書/返信書） | 旧来版は多種文書を選択可能。Web は外来サマリのみ。 | ChartImpl / client/src/main/java/open/dolphin/client/ChartImpl.java | Charts 右パネル / web-client/src/features/charts/pages/ChartsPage.tsx | 旧来版: 文書種別を選択して作成。Web: 外来サマリ以外は選択不可。 | P0 |
...
## 漏れ検証チェック（11_漏れ検証と最終確認）
- チェック日時: 2025-12-29 15:29 JST
- 確認者: CodexCLI1
- RUN_ID: 20251229T062814Z
- 結果: 09 の P0/P1/P2 と突合し、A の履歴コピー/スタンプコピー、B の RP名/薬剤行、E の文書種別を追記済み。差分漏れなし。
