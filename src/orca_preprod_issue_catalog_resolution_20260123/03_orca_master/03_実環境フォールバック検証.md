...
## 前提/注意事項
- LogFilter が `userName/password` ヘッダ認証を無効化しているため、**Authorization: Basic が必須**。
- 上記理由により、`OrcaMasterResource` に Basic 認証ヘッダの解釈ロジックを追加し、
  ORCA master 系の認証経路を LogFilter の挙動に合わせた。
