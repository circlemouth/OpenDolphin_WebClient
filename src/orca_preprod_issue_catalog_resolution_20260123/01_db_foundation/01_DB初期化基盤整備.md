...
- 必須シーケンスの自動補正 SQL を追加し、search_path/必須シーケンス欠落時のみ適用。
- DB 初期化ログの出力先を追加（artifacts/preprod/db-init）。
- /api/* 500 回避の自動検証（API health check）を追加。

## 起動チェック
- `setup-modernized-env.sh` で以下を自動確認。
  - `d_users` / `d_facility` / `d_roles` / `d_audit_event`（opendolphin/public いずれか）の存在。
...
- API health check のログは `artifacts/preprod/api-health/api-health-<RUN_ID>.log` に出力。
...
- docs/preprod/implementation-issue-inventory/logs/20260123T232657Z-db-init-automation.md
...
- 結果: Modernized Server 起動・DB修復SQL適用・API health check 実行・baseline seed・初期ユーザー登録まで完了。
- 初期化ログ: `artifacts/preprod/db-init/db-init-20260123T232657Z.log`
