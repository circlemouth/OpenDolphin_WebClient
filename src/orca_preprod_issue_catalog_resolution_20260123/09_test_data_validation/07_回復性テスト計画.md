...
- ネットワーク: ブラウザ〜Vite〜dev proxy〜server-modernized の通信経路
...
- インフラ障害試験は **MSW 無効**（`VITE_DISABLE_MSW=1` / `PLAYWRIGHT_DISABLE_MSW=1`）で実施
- 証跡保存: `artifacts/validation/resilience/<RUN_ID>/` に統一
- worktree 専用コンテナを作成し、他 worktree のコンテナは操作しない

## 対象フロー（最低限）
- 受付 → Charts → 請求送信 → キュー確認
- 監査ログ/テレメトリの送出確認（runId/traceId の突合）

## 依存サービス構成（想定）
- server-modernized-dev / db / minio / web-client / dev proxy（ORCA）
- ブラウザは同一端末の Vite 経由アクセスを前提とする
...
| NET-1 | Network | ブラウザ側 offline（DevTools などで切断） | オフライン時の UI/再取得導線 | オフライン通知 + 再取得導線 | オンライン復帰後に再取得成功 |
| NET-2 | Network | dev proxy 〜 server 間の切断 | 中間断の復旧導線 | 取得失敗の説明 + 再取得導線 | 接続復旧後に正常表示 |
...
## 観測指標（最低限）
- 復旧時間: サービス復帰 → UI 正常化までの所要時間
- 再送/再取得成功率: 復旧操作後に成功へ遷移する比率
- 監査ログ欠落率: runId/traceId が監査イベントに残る割合
...
### Network
- ブラウザ offline: DevTools の offline 切替
- proxy 断: `docker network disconnect <network> <proxy-container>`
...
## 注意事項
- ORCA 実環境接続は禁止（WebORCA Trial のみ）。資格情報の変更は必ず復旧させる。
- 障害注入は単独で行い、同時注入は本計画の範囲外とする。
