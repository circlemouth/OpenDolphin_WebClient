...
- コンテナ: `jma-receipt-docker-orca-1`（未起動時はサブモジュールの `docker compose up -d` で起動）
- 疎通確認（system01dailyv2）:
  - `curl -u ormaster:change_me -H 'Content-Type: application/xml; charset=UTF-8' -H 'Accept: application/xml' -X POST --data-binary @docs/server-modernization/phase2/operations/assets/orca-api-requests/xml/44_system01dailyv2_request.xml http://localhost:8000/api/api01rv2/system01dailyv2`
  - Api_Result=00 を確認
