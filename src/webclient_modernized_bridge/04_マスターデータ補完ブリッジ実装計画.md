...
- 親RUN_ID: `20251124T000000Z`（スナップショット基線、証跡・データ取得元は親 RUN を継続利用）
- 期間: 2025-11-25 03:00 〜 2025-11-27 03:00 (JST)
- 優先度: Medium / 緊急度: High
- エージェント: Codex（指示ベースで更新）
...
### 参考資料
- `docs/server-modernization/phase2/operations/assets/orca-db-schema/error-fallback-test-matrix.md`（ORCA-05/06/08 エラー/フォールバック設計とテスト網羅表、RUN_ID=20251124T123000Z）
- `docs/server-modernization/phase2/operations/assets/orca-db-schema/field-mapping/orca-master-field-matrix.md`（DB 列 ↔ OpenAPI DTO ↔ UI 型の対応表、RUN_ID=20251124T121500Z）
- `docs/server-modernization/phase2/operations/assets/orca-db-schema/seed-plan/orca-master-seed-plan.md`（ORCA-05/06/08 シード計画・テンプレート参照、RUN_ID=20251124T130000Z）
- `docs/server-modernization/phase2/operations/orca-master-release-plan.md`（ORCA-05/06/08 リリース／ロールバック計画・フラグ設計、RUN_ID=20251124T131500Z）
- `docs/server-modernization/phase2/operations/orca-master-staging-plan.md`（Dev→Stage→Prod 段階移行とフラグ切替時系列テンプレ、RUN_ID=20251124T194500Z）
- `docs/server-modernization/phase2/operations/orca-master-resilience-plan.md`（ORCA-05/06/08 フォールトインジェクション／レジリエンス試験計画 draft、RUN_ID=20251124T174500Z）
- `docs/server-modernization/phase2/operations/orca-master-go-live-checklist.md`（ORCA-05/06/08 実サーバー接続前チェックリスト、RUN_ID=20251124T184500Z）
- `docs/server-modernization/phase2/operations/orca-master-ab-compare-plan.md`（ORCA-05/06/08 旧実装↔新 REST A/B 比較計画ドラフト、RUN_ID=20251124T151500Z）
- `docs/server-modernization/phase2/operations/orca-master-schema-drift-plan.md`（ORCA-05/06/08 DB 実体 vs 定義書/OpenAPI のドリフト検出ドラフト、RUN_ID=20251124T161500Z）
- `artifacts/api-stability/20251124T161500Z/schema-drift/results/`（スキーマドリフト検証結果置き場。本 RUN=20251124T214500Z は DSN 未提供のため README のみ）
- `docs/web-client/ux/performance/orca-master-performance-plan.md`（ORCA マスター取得のフロント性能予算と計測計画、RUN_ID=20251124T171500Z）
- `docs/web-client/ux/accessibility/orca-master-a11y-plan.md`（ORCA マスタ関連画面のアクセシビリティ改善計画ドラフト、RUN_ID=20251124T203000Z）
- `docs/web-client/ux/performance/orca-master-fcp-improvement.md`（/charts 初期描画 FCP ボトルネック分析と短期チューニング案、RUN_ID=20251124T183000Z）
- `docs/server-modernization/phase2/operations/assets/observability/orca-master-dashboard.json`（Grafana 監視ドラフト、RUN_ID=20251124T142000Z）
- `docs/server-modernization/phase2/operations/orca-master-security-privacy-checklist.md`（ORCA-05/06/08 ログ/キャッシュのセキュリティ・プライバシー分類とマスキング方針、RUN_ID=20251124T134500Z）
- `docs/server-modernization/phase2/operations/orca-master-audit-archive-plan.md`（ORCA-05/06/08 監査ログ/アーカイブ方針 draft、RUN_ID=20251124T190000Z）
- `docs/server-modernization/phase2/operations/orca-master-sprint-plan.md`（本タスクの最小スプリント計画、RUN_ID=20251124T213000Z、親=20251124T000000Z）
- `docs/server-modernization/phase2/operations/assets/ci/README.md`（ORCA マスターブリッジ CI/品質ゲート案、RUN_ID=20251124T140500Z）
- `docs/server-modernization/phase2/operations/orca-master-rollback-plan.md`（ORCA-05/06/08 ロールバック／クリーンアップ手順ドラフト、RUN_ID=20251124T160000Z）
- `docs/server-modernization/phase2/operations/orca-master-code-systems.md`（ORCA-05/06/08 コード体系レビュー／バリデーション草案、RUN_ID=20251124T163000Z）
- `web-client/.env.stage.example` / `web-client/.env.prod.example`（ブリッジ用 Stage/Prod 環境変数テンプレート、RUN_ID=20251124T213500Z）
...
## 本指示対応（RUN_ID=20251124T073245Z）
- 目的: ORCA-05/06/08 の不足マスタを ORCA DB 定義書（正式版 2024-04-26）に沿った DTO/型へ落とし込み、クライアント側の型拡張と API スケルトンを準備する。同時にサーバー側 REST 追加の設計メモをインベントリ/ギャップトラッカーへ明示し、MSW/fixture 期待値とテスト手順を計画にリンクする。
- 進捗要約: 型拡張と API スケルトンの TODO 埋め込みを実施（web-client/src/types/orca.ts, web-client/src/features/charts/api/orca-api.ts）。サーバー REST 設計メモとオーナー・優先度を `docs/server-modernization/MODERNIZED_REST_API_INVENTORY.md` / `docs/server-modernization/phase2/notes/MODERNIZED_SERVER_GAP_TRACKER_20251116T210500Z.md` に追記。DOC_STATUS・ログを RUN_ID 同期。

| タスク | 担当 | ETA (JST) | 状態 | 備考 |
| --- | --- | --- | --- | --- |
| ORCA-05/06/08 DTO 定義（DB 定義書準拠）とクライアント型拡張 | Codex | 2025-11-24 16:00 | Done | `web-client/src/types/orca.ts` に監査メタ/バージョン/必須列を追加し、ORCA-05 統合 DTO（minPrice/youhou/material/kensaSort）と ORCA-06/08 必須フィールドを明示。 |
| API スケルトン TODO 追加（bridge 呼び出し口） | Codex | 2025-11-24 16:00 | Done | `web-client/src/features/charts/api/orca-api.ts` に ORCA-05/06/08 スケルトン関数と監査メタ引数を追加（返却は暫定 placeholder、実 REST 提供後に実装）。 |
| サーバー REST 追加メモ記載（オーナー・優先度） | Codex → Server担当: Worker-B(暫定) | 2025-11-24 17:00 | Done | `MODERNIZED_REST_API_INVENTORY.md` と `MODERNIZED_SERVER_GAP_TRACKER_20251116T210500Z.md` に ORCA-05/06/08 の優先度=P1、オーナー=Worker-B（サーバー）、依存: ORCA DB 定義書を追記。 |
| MSW/fixture 期待値と契約テスト動線のリンク | Codex（手順作成） / QA(Worker-Q 想定) | 2025-11-25 10:00 | ToDo | `artifacts/api-stability/20251123T130134Z/schemas/` ベースのフィクスチャに `dataSource= snapshot` `runId=20251124T073245Z` `version` `snapshotVersion` を必須付与する手順を本計画「MSW/fixture 追加作業ステップ」に明示。 |

### MSW/fixture 追加作業ステップ（RUN_ID=20251124T073245Z）
1. **ソース確定**: `artifacts/api-stability/20251123T130134Z/schemas/` の `orca-master-*.json` を唯一の入力として扱う（ORCA-05/06/08 全件）。日付/値が欠損していても追加取得は禁止し、欠損は `missingMaster=true` で表現する。
2. **フィールド拡張**: 各エントリへ以下を追加し、`web-client/src/mocks/fixtures/orcaMaster.ts` に集約する。`dataSource: "snapshot"`, `runId: "20251124T073245Z"`, `snapshotVersion`（元 JSON の `version` またはファイル名由来の日付、無ければ `2025-11-23` を暫定設定）, `cacheHit: false`, `missingMaster`/`fallbackUsed` をスキーマに合わせて boolean で追加。
3. **MSW ハンドラ差し替え**: `web-client/src/mocks/handlers/master/*.ts` で上記フィクスチャを返却し、`dataSource`/`runId`/`snapshotVersion` を必ず透過させる。ORCA-06 住所の空レスは `fallbackUsed=true` で返す。差し替え後に `npm run test -- orca-master`（仮の契約テストスイート）を実行予定（Python スクリプト禁止のため Node/Vitest に限定）。
4. **ハッシュ出力**: フィクスチャ生成後に `artifacts/api-stability/20251124T000000Z/master-sync/<YYYYMMDD>/hashes/msw/{orca05,orca06,orca08}.hash` を作成し、本計画 §契約テスト観点のハッシュ手順に従って保存。生成日付と runId を同ディレクトリの README に追記する。
5. **証跡記録**: 作業ログと生成ファイル一覧を `docs/server-modernization/phase2/operations/logs/20251123T135709Z-webclient-master-bridge.md#msw-fixture` へ追記し、DOC_STATUS 備考に `msw fixture steps` 追記済みであることを明記する。

## UI スモーク手順（MSW 前提・新 DTO 版）
MSW を有効にした状態で警告バナー/フォールバック表示を確認する最小スモーク。実行結果は `docs/server-modernization/phase2/operations/logs/20251123T135709Z-webclient-master-bridge.md#ui-smoke` に記録し、`runId=20251124T073245Z` を必ず付与する。詳細な UI 影響分析は `src/webclient_modernized_bridge/03_ギャップ解消方針とUI影響分析.md` を参照。
- ORCA-05（薬剤/材料/用法）: `WEB_ORCA_MASTER_SOURCE=msw` でカルテのオーダ検索を開き、暫定バッジ＋InlineFeedback `tone=warning` が表示され `missingMaster=true` が監査送出されることを確認。最低薬価欠落で保存ブロックされることも確認。
- ORCA-06（保険者/住所）: 受付→保険選択ダイアログでドロップダウンがスナップショット値で埋まること、空レス時に `fallbackUsed=true` が監査へ出ること、住所は `prefCode/cityCode` 未設定で手入力許可＋warning バナー表示になることを確認。
- ORCA-08（電子点数表）: Charts の算定/再計算で点数表フィクスチャを読み込み、`tensuVersion` が UI/監査に透過していること、空ヒット時に `warning` バナーと `missingMaster=true` がセットで送出されることを確認。
- 切替テレメトリ: ブラウザリロード後に `WEB_ORCA_MASTER_SOURCE` を `server` → `msw` に戻し、`dataSourceTransition` が `mock|snapshot←→server` で 1 回ずつ送出されることを監査ログで確認。
- ログ保存: 各シナリオのコンソール/ネットワークログを `artifacts/api-stability/20251124T000000Z/ui-smoke/` に保存（JSON で良い）。ファイル名は `orca05-smoke.json` などとし、上記ログセクションから相対パスでリンクする。
| DOC_STATUS 備考更新・ログ追記 | Codex | 2025-11-24 17:30 | Done | `docs/web-client/planning/phase2/DOC_STATUS.md`、`docs/server-modernization/phase2/operations/logs/20251123T135709Z-webclient-master-bridge.md`、`docs/server-modernized/phase2/verification/00_ORCA_MASTER_DATA_GAP_REPORT.md` を RUN_ID で同期。 |
