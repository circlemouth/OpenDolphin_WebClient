...
- 派生RUN_ID: `20251124T000000Z`（同期ジョブ/アーティファクト用のベース）
...
- 証跡ログ: `docs/server-modernization/phase2/operations/logs/20251124T000000Z-webclient-master-bridge-plan.md`（派生RUN_ID）、`docs/server-modernization/phase2/operations/logs/20251124T073245Z-webclient-master-bridge.md`（RUN_ID=20251124T073245Z の進捗・証跡）
...
- RUN_ID=`20251124T073245Z` を DOC_STATUS/ログ/証跡で共通利用し、派生 RUN_ID は本値を親として明示する。

## データソース設計（RUN_ID=20251124T073245Z）[^gap][^orca_status][^api_guide]
...
`dataSourceTransition` 監査メタは `{ runId: "20251124T073245Z", masterType, from, to, reason, snapshotVersion }` を最小セットとし、
- 初回ソース決定（cold start）、
- 前回決定値と異なる経路を選択したとき、
...
| SP3 同期ジョブ & スナップショット更新 | 11/26 19:00-22:00 | `bridge-sync` npm script 追加、結果を `artifacts/api-stability/20251124T000000Z/master-sync/` へ保存、ロールバック手順追記。 | 3h | 1 回目の同期で snapshot が保存され、DOC_STATUS 備考にパスを追記。 |
...
## 実装ロードマップ（SP1〜SP4 詳細、RUN_ID=20251124T073245Z）[^gap][^api_guide]
...
| --- | --- | --- | --- | --- |
| SP1: 型/フィクスチャ整理 | Webクライアント担当（codex） | 2025-11-26 12:00 | `resolveMasterSource`/`fetch adapter` の API 面と監査フィールドを `03_ギャップ解消方針…` に一致させ、本ファイルへ反映。型/フィクスチャ diff を整理しログへ記載。 | `docs/server-modernization/phase2/operations/logs/20251124T000000Z-webclient-bridge.md#SP1` |
...
| SP2: ブリッジ実装 | Webクライアント担当（codex） | 2025-11-26 18:00 | React Query TTL=5分設定、fallback 定数組み込み、`dataSource`/`dataSourceTransition` 送出を実装。`/orca/master/address` スモークで `dataSource=server` が確認できること。 | `docs/server-modernization/phase2/operations/logs/20251124T000000Z-webclient-bridge.md#SP2` |
| SP3: 契約テスト | QA（契約テスト担当） | 2025-11-26 22:00 | MSW ↔ 実サーバーのスキーマ差分/件数差分テストが緑。監査メタ `dataSource`/`fallbackUsed`/`missingMaster` が契約テストで検証済み。 | `docs/server-modernization/phase2/operations/logs/20251124T000000Z-webclient-bridge.md#SP3` |
| SP4: 切替準備 | フロント+QA | 2025-11-27 06:00 | Feature flag で MSW↔server を往復し、主要 6 シナリオ E2E PASS。警告バナー/監査メタが UI で表示・送出されること。 | `docs/server-modernization/phase2/operations/logs/20251124T000000Z-webclient-bridge.md#SP4` |

#### SP4 チェックリスト（RUN_ID=20251124T073245Z）
- [ ] runtime flag 切替手順: `.env.local` の `WEB_ORCA_MASTER_SOURCE=mock|snapshot|server` を順番に適用し各切替後に `npm run e2e:smoke -- --runId=20251124T073245Z --tag=master-bridge` を実行、`artifacts/e2e/20251124T073245Z/sp4-runtime-flag.log` に `dataSourceTransition` が記録されることを確認。
...
- [ ] 監査メタ最終確認: Charts/Reception/Claim の主要6シナリオを E2E 実行し、`artifacts/e2e/20251124T073245Z/sp4-main-scenarios.log` に `runId` `apiRoute` `dataSource` `cacheHit` `missingMaster` `fallbackUsed` が空値なく出力され PASS であることを確認。
- [ ] E2E 結果リンク: 上記主要6シナリオログを `docs/server-modernization/phase2/operations/logs/20251124T073245Z-webclient-bridge.md#SP4` に貼付し、FAIL があれば再現手順とチケット ID を併記。
- [ ] Slack 通知テンプレ: `#modernized-webclient` へ「MSW↔server 切替 E2E 完了・監査メタ確認済 / RUN_ID=20251124T073245Z / evidence=artifacts/e2e/20251124T073245Z/sp4-main-scenarios.log / flag=WEB_ORCA_MASTER_SOURCE=server」を送信し、送信ログ/スクショパスを #SP4 に記録。

## 契約テスト観点（SP3ドラフト）
`npm run bridge-sync -- --run-id 20251124T073245Z --date 20251124 --source msw` で canonical 化した ORCA-05/06/08 を `artifacts/api-stability/20251124T000000Z/master-sync/20251124/hashes/msw/{orca05,orca06,orca08}.hash` に保存し、`--source server` で実 API 出力と `hashes/server/` の差分を比較する。scripts/bridge-sync.mjs は schema から list を抽出して指定フィールド順でソート・flatten し、sha256 を計算するため、SDLC 検証やロールバックタイムラインの基盤になる。
...
- 監査メタ送出: MSW→server 切替時に `dataSourceTransition` が `from=mock|snapshot` → `to=server` で記録されること、`runId=20251124T073245Z` が全イベントに付与されることを契約テストで確認。
...
#### server版 ↔ MSW 差分記録テンプレ（RUN_ID=20251124T073245Z）
| 実行日時 | 環境 | ハッシュ値 | 件数差 | 単価差 | 許容判定 | diffファイルパス | 監査メタ送出確認結果 |
| --- | --- | --- | --- | --- | --- | --- | --- |
|  |  |  |  |  |  |  |  |

### 付録: CI/同期ジョブ設計（RUN_ID=20251124T073245Z）
...
### SP1具体タスクリスト（RUN_ID=20251124T073245Z）
...
## 5. 成果物 & 更新先
- ドキュメント: 本ファイルと証跡ログ `docs/server-modernization/phase2/operations/logs/20251124T000000Z-webclient-master-bridge-plan.md` / `docs/server-modernization/phase2/operations/logs/20251124T073245Z-webclient-master-bridge.md`。
...
- SP3: `node scripts/bridge-sync.mjs --run-id 20251124T073245Z --date 20251124 --source server` を実行し、`artifacts/api-stability/20251124T000000Z/master-sync/20251124/hashes/{msw,server}` を更新。`diffs/server-vs-msw-orca05.json` には ORCA-05 件数が MSW=5→server=4 となった差分と、`missingMaster=false`/`fallbackUsed=false`/`dataSource=snapshot` の監査メタが記録され、ログ `docs/server-modernization/phase2/operations/logs/20251124T073245Z-webclient-master-bridge.md#SP3` へリンク済み。
- SP4: `fetchWithResolver` と `OrcaMasterFetchOptions` を runId・snapshotVersion 付きで調整し、`app/App.tsx` で `ORCA_MASTER_FETCH_TTL_MS` を React Query TTL（5分）へ同期。Charts/Reception/Claim へ `fetchOrca05/06/08 BridgeMasters` を連携し、warning banner/監査メタ付きの E2E候補を `artifacts/e2e/20251124T073245Z/sp4-main-scenarios.log` に記録、同ログと `docs/server-modernization/phase2/operations/logs/20251124T073245Z-webclient-master-bridge.md#SP4` に結果を引用。
- DOC_STATUS: `docs/web-client/planning/phase2/DOC_STATUS.md:65` に RUN_ID・bridge-sync/MSW+server ハッシュ/`artifacts/e2e/20251124T073245Z/` E2Eログ・`PHASE2_WEB_CLIENT_EXPERIENCE_MANAGER_CHECKLIST.md` 連携・未完タスクを記載し、参照チェーンと整合。
- 追加資産: `web-client/.env.sample` や `artifacts/e2e/20251124T073245Z/`・`scripts/bridge-sync.mjs` など、既存未追跡ファイルとの整合性は変化なし（今回の更新はログ・DOC_STATUS・ハッシュ関連に限定）。

## 次のアクション（引き続き RUN_ID=20251124T073245Z）
1. server raw B にてデータ更新を確認したら再度 `node scripts/bridge-sync.mjs --run-id 20251124T073245Z --date 20251124 --source server` を実行し、`hashes/server/` と `diffs/server-vs-msw-orca*.json` を上書き。差分が出た場合は `missingMaster`/`fallbackUsed`/`cacheHit` の変化を `docs/server-modernization/phase2/operations/logs/20251124T073245Z-webclient-master-bridge.md#SP3`・`docs/web-client/planning/phase2/DOC_STATUS.md:65`（Webクライアント/連携行）・`docs/managerdocs/PHASE2_WEB_CLIENT_EXPERIENCE_MANAGER_CHECKLIST.md` RUN_ID セクションへ追記して証跡化・通知する。
2. `WEB_ORCA_MASTER_SOURCE=server` ＋ `VITE_DISABLE_MSW=1` の Stage/Preview 環境で `npm run e2e:orca-master`（root）を再実行し、Playwrightログ（`runId`/`dataSource`/`cacheHit`/`missingMaster`/`fallbackUsed` 含む）を `artifacts/e2e/20251124T073245Z/` に残す。結果・警告・再現手順は `docs/server-modernization/phase2/operations/logs/20251124T073245Z-webclient-master-bridge.md#SP4` に追記し、DOC_STATUS 65 行と manager checklist RUN_ID セクションに `artifacts/e2e/20251124T073245Z/sp4-main-scenarios.log` へのリンクを追加する。
3. ワーカー報告草案を final 形式に整備し（`docs/server-modernization/phase2/notes/worker-report-master-bridge-20251124T073245Z.md`）、`docs/web-client/README.md`・`docs/web-client/planning/phase2/DOC_STATUS.md:65`・`docs/managerdocs/PHASE2_WEB_CLIENT_EXPERIENCE_MANAGER_CHECKLIST.md` の RUN_ID 行に RUN_ID・ログ・アーティファクトパス・未完タスク/次アクションを同期させる。

## 接続断ループ運用（Stage/Live 再接続前の手順）
| フェーズ | 内容 | 証跡パス | 監査メタ送出ポイント |
| --- | --- | --- | --- |
| 1. 全クライアント切断 | Stage/Live への接続再開前にすべての Web クライアントを停止（Vite/Playwright + Stage/Live 接続）し、`raw B` 停止状態を確保 | `docs/server-modernization/phase2/operations/logs/20251124T073245Z-webclient-master-bridge.md#SP3` | `dataSourceTransition`：from=`snapshot|msw` → to=`server` が再接続をまたいで発生していないか検証 |
| 2. raw B 差分取得 | `artifacts/api-stability/20251124T151500Z/ab-compare/.../raw/B/` を再確認して新しい diff を採取 | 同上 + `artifacts/api-stability/20251124T151500Z/ab-compare/20251124T153000Z/raw/B/` | `missingMaster`/`fallbackUsed`：差分で `true`→`false` になっていないか、ログで確認 |
| 3. bridge-sync 実行 | `node scripts/bridge-sync.mjs --run-id 20251124T073245Z --date 20251124 --source server` を all-clients-off 中に走らせ、`hashes/server/`・`diffs/server-vs-msw-orca*.json` を更新 | `artifacts/api-stability/20251124T000000Z/master-sync/20251124/`（hashes/diffs） | `dataSource`/`cacheHit`：snapshot→server 切替時の値と、cache miss→hit の遷移を比較 |
| 4. hash/diff 上書き & schema 再確認 | `hashes/server/{orca05,orca06,orca08}.hash` を再書き込みし、`schema missing` を含む diff を `docs/server-modernization/phase2/operations/logs/20251124T073245Z-webclient-master-bridge.md#SP3` に連携 | 同上 + `schema missing: .../raw/B/orca-master-generic-class.json`  | `missingMaster`/`cacheHit`：schema missing のタイミングで `missingMaster`=true になるか収集 |
| 5. DOC_STATUS / 管理ドキュメント同期 | DOC_STATUS（Webクライアント/連携行）、manager checklist RUN_ID 行、worker report に `hashes`・`diffs`・`artifacts/e2e/20251124T073245Z/sp4-main-scenarios.log` を追記 | `docs/web-client/planning/phase2/DOC_STATUS.md:65` / `docs/managerdocs/PHASE2_WEB_CLIENT_EXPERIENCE_MANAGER_CHECKLIST.md` / `docs/server-modernization/phase2/notes/worker-report-master-bridge-20251124T073245Z.md` | `fallbackUsed`：再同期直後に true→false でホップが切り替わっていないか、全ドキュメントに実例を残す |
| 6. Stage/Live 再接続 + E2E 実行 | 全クライアント再起動後に Stage/Live E2E（warning banner tone + dataSourceTransition）を走らせ、`artifacts/e2e/20251124T073245Z/sp4-main-scenarios.log` を上書き | `artifacts/e2e/20251124T073245Z/sp4-main-scenarios.log` / `docs/server-modernization/phase2/operations/logs/20251124T073245Z-webclient-master-bridge.md#SP4` | `dataSourceTransition`/`warning banner tone`：snapshot→server 再接続時のトーン変化と `dataSourceTransition` 記録を比較 |

監査メタ送出タッチポイント: `dataSource`、`cacheHit`、`missingMaster`、`fallbackUsed`、`dataSourceTransition`（+ runId/runContext）を上記全フェーズで確認し、それぞれの更新が Stage/Live E2E の `artifacts/e2e/20251124T073245Z/sp4-main-scenarios.log` と `docs/server-modernization/phase2/operations/logs/20251124T073245Z-webclient-master-bridge.md` に反映されていることを保証する。

## Stage/Live E2E 観測サマリ（監査含む）
Stage/Preview Stage/Live 実行（`artifacts/e2e/20251124T073245Z/sp4-main-scenarios.log`）で得た `dataSource`/`cacheHit`/`missingMaster`/`fallbackUsed`/警告バナーの組み合わせを以下に示す。PASS/FAIL 判定は Playwright そのものと監査メタの網羅状況を合わせた評価であり、次回再実行では補完すべき期待値も併せて記載しておく。

| シナリオ | dataSource | cacheHit | missingMaster | fallbackUsed | 警告バナーのトーン | 判定（PASS/FAIL） | 次回 re-run 期待値 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Charts master bridge（Live profile） | server | false | false | false | 警告（warning）バナー + recordPerfLog のメタ | PASS（監査メタ完備） | Stage/Live で `dataSourceTransition=server->server` が発生しない状態を維持しつつ、warning tone の原因（billed diff）を再検証し、`artifacts/e2e/20251124T073245Z/sp4-main-scenarios.log` を上書き |
| Reception 保険/住所 | snapshot | false | true | true | caution-tone warning（MSW snapshot） | BLOCKED（`test.skip`/`test.fixme` につき msw-only） | Live baseURL＋ `test.skip`/`test.fixme` 解除後に再実行し、`dataSourceTransition=snapshot->server` と `missingMaster=false`/`fallbackUsed=false` を確認 |
| Claim 点数/診療行為 | snapshot | false | true | true | caution-tone warning（MSW snapshot） | PENDING（Live spec 未整備） | Claim UI セレクタと baseURL が整い次第再実行し、server レスポンスで `warning` トーンの変化と `dataSourceTransition` を取得 |

各観測では `dataSourceTransition` の送出と `warning banner tone` の変化を特に注視し、監査メタが `artifacts/e2e/20251124T073245Z/sp4-main-scenarios.log` だけでなく `docs/server-modernization/phase2/operations/logs/20251124T073245Z-webclient-master-bridge.md#SP4` にも漏れなく書き出されるようにする。

## 未完タスクと想定担当
| タスク | 内容 | 担当想定 |
| --- | --- | --- |
| Reception/Claim MSW-only coverage | `test.skip`/`test.fixme` を解除し Live baseURL（例: `/reception/insurance`・`/claim/tensu`）で再実行し、`dataSourceTransition=snapshot->server`・`missingMaster=false`/`fallbackUsed=false` を確認 | 差分担当（hash/diff + fixture更新） |
| warning banner tone の検証 | Live 返却で警告バナーが表示される要因（件数/単価差・snapshot vs server）を再現し、トーンと監査メタの整合性を確認 | E2E担当（Stage/Live Playwright + UI audit meta） |
| Stage/Live E2E 再実行（dataSourceTransition） | Stage/Live で `WEB_ORCA_MASTER_SOURCE=server` + `VITE_DISABLE_MSW=1` 再実行し、`artifacts/e2e/20251124T073245Z/sp4-main-scenarios.log` / `docs/server-modernization/phase2/operations/logs/20251124T073245Z-webclient-master-bridge.md#SP4` / DOC_STATUS に warning tone + `dataSourceTransition` の変化を書き出す | E2E担当（Stage/Live automation + logs） |
| hash/diff 再取得と DOC_STATUS 同期 | `hashes/server/`・`diffs/server-vs-msw-orca*.json`・`schema missing` の再生成後、`docs/web-client/planning/phase2/DOC_STATUS.md:65`・manager checklist・worker report とリンク同期を行う | 差分担当（bridge-sync + doc同期） |

上記に加え、未完タスク/次アクション（pvt/docinfo fixture・hash diff/Stage log 上書き・Live baseURL coverage 予定）は DOC_STATUS 65 行・マネージャーチェックリスト RUN_ID セクション・worker report で同一 RUN_ID/証跡を使って管理し、リンク漏れがないことを定期的に確認する。
...
## 8. 現在の進捗（RUN_ID=20251124T073245Z）
- MSW フィクスチャを `artifacts/api-stability/20251123T130134Z/schemas/` で再生成し、`web-client/src/mocks/fixtures/orcaMaster.ts` / `msw/handlers/master/*` を snapshot/hash と同期させた。ハッシュ再計算結果と DOC_STATUS 追記は `docs/server-modernization/phase2/operations/logs/20251123T135709Z-webclient-master-bridge.md#msw-fixture` に掲載。
- `WEB_ORCA_MASTER_SOURCE`/`VITE_ORCA_MASTER_BRIDGE` で MSW→snapshot→server を切り替える source resolver を追加し、`web-client/src/features/charts/api/orca-api.ts` ほか ORCA-05/06/08 取得フックに `dataSource/dataSourceTransition/cacheHit/missingMaster/fallbackUsed/runId/snapshotVersion` を透過的に流すよう配線した。
- 単体テスト: `npm run test -- --reporter=verbose src/features/charts/api/__tests__/orca-api.test.ts`（Node 環境、SharedArrayBuffer polyfill を setupFiles へ追加）が 13/13 pass。`web-client/test-results/.last-run.json` を更新済み。
- UI スモーク: MSW headless/headful ではまだ `orca-search-input` が出現せず警告未捕捉。`docs/server-modernization/phase2/operations/logs/20251123T135709Z-webclient-master-bridge.md#ui-smoke` にリラン要件（MSW シード+server/snapshot パス確認）を記録。
- DOC_STATUS/ログ: `docs/web-client/planning/phase2/DOC_STATUS.md` と `docs/server-modernization/phase2/operations/logs/20251123T135709Z-webclient-master-bridge.md` に RUN_ID=20251124T073245Z の進捗・証跡リンクを記載済み。
