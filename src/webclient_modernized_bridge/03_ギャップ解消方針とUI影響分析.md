...
- 依存資料: `src/webclient_modernized_bridge/01_API互換レイヤー再確認とギャップ整理.md`、`src/webclient_modernized_bridge/02_ORCAマスターデータギャップ報告精査.md`、`docs/web-client/process/API_UI_GAP_ANALYSIS.md`、`docs/web-client/ux/legacy/CHART_UI_GUIDE_INDEX.md`、`docs/web-client/ux/legacy/API_SURFACE_AND_AUDIT_GUIDE.md`、`docs/server-modernization/phase2/operations/ORCA_API_STATUS.md`（2025-11-22 mac-dev 実測）
- MSW 前提の UI スモーク手順は `src/webclient_modernized_bridge/04_マスターデータ補完ブリッジ実装計画.md#ui-スモーク手順msw-前提・新-dto-版` に集約（RUN_ID=`20251124T073245Z`）。本ファイルの UI 影響表・テスト観点と合わせて参照する。
- UI スモーク（MSW + 新 DTO 版）実施ログ: `artifacts/api-stability/20251124T000000Z/ui-smoke/vitest-msw.log`（Vitest smoke, RUN_ID=`20251124T073245Z`）。警告バナー/フォールバック想定の基礎データを MSW フィクスチャ経由で供給できることを確認。
- 実画面 fetch スモーク（RUN_ID=`20251124T090000Z`, 親=`20251124T000000Z`）: MSW 有効のまま `/orca/master/*` `/orca/tensu/ten` をブラウザ上で直接叩き、レスポンスに `dataSource/runId/snapshotVersion/cacheHit/missingMaster/fallbackUsed` が含まれることを HAR/ログで確認。成果物: `artifacts/api-stability/20251124T000000Z/ui-smoke/20251124T090000Z-orca-smoke.har` / `...-orca-smoke-log.json`（#ui-smoke に記録）。スクリーンショット不要指示により未取得。
- 実画面バナー確認（RUN_ID=`20251124T094500Z`, 親=`20251124T000000Z`）: MSW runId を `20251124T090000Z` に揃え、Charts `/charts/72001` ロック状態で ORCA マスター検索 UI を操作。`初診` 検索→`/orca/tensu/name/*`、点数帯 0–300 →`/orca/tensu/ten/0-300/` が警告バナーなしで snapshot 表示されることをスクショ/HAR/コンソールログで保存（`artifacts/api-stability/20251124T000000Z/ui-smoke/20251124T094500Z-*.{png,har,json}`、master fetch=`...-orca-master-fetch.json`）。詳細は `docs/server-modernization/phase2/operations/logs/20251123T135709Z-webclient-master-bridge.md#ui-smoke`。
...
- UI/UX ガイド（`ux/ux-documentation-plan.md` と legacy 資料 `ux/legacy/CHART_UI_GUIDE_INDEX.md` / `ux/legacy/API_SURFACE_AND_AUDIT_GUIDE.md`）ではエラー/空状態トーン統一・監査ログ送出・Danger 操作の隔離が必須。暫定データでも同要件を維持する。
...
※ トーン/文言は `docs/web-client/ux/legacy/CHART_UI_GUIDE_INDEX.md`・`ux/legacy/KARTE_SCREEN_IMPLEMENTATION.md` の `neutral`/`info`/`warning`/`danger` 運用に準拠し、警告は `InlineFeedback` の `tone=warning` + `alert-triangle` アイコン、`aria-live="polite"` で統一（RUN_ID=`20251124T000000Z` 証跡）。証跡ログ: `docs/server-modernization/phase2/operations/logs/20251124T000000Z-webclient-bridge.md` にトーン準拠とテスト観点を記録。
...
1. **トーン規則の適用**: 空/再取得待ちは `neutral`、暫定データ警告は `warning`、検証失敗は `danger`、成功/復旧は `info`。`ux/legacy/CHART_UI_GUIDE_INDEX.md` の DocumentTimeline トーン運用に合わせる。
2. **Danger 操作の隔離**: 予約削除・請求再送・カルテ上書きは暫定データ利用時に確認チェックボックス＋RUN_ID 表示を追加し、`ux/legacy/API_SURFACE_AND_AUDIT_GUIDE.md` の Danger セクション運用を踏襲。
...
| SP4: UI 回帰テスト & トーン整合 | 2025-11-25 PM | E2E/スモークで UI 回帰（警告トーン、危険操作確認、監査ログ出力）。`ux/legacy/CHART_UI_GUIDE_INDEX.md` / `ux/legacy/API_SURFACE_AND_AUDIT_GUIDE.md` に暫定運用を反映。 | SP3 のフラグ付き実 API がステージングで稼働していること。 | DOC_STATUS 更新は担当者タスク。RUN_ID をテストログ/証跡に付記。 |
