...
- RUN_ID: 20251228T195634Z
- 期間: 2025-12-28 19:56 - 2025-12-28 20:20 UTC
- ステータス: done
- 進捗: 100
...
- 担当者: codex
- 更新日: 2025-12-28
- 根拠: web-client の env テンプレ追加と README 追記、MSW 無効化時の接続パスをコードで確認。
...
## 実施ログ
- 2025-12-28: 環境変数テンプレートと配信前提の整理を実施。
  - 作業内容:
    - `web-client/.env.stage.example` と `web-client/.env.prod.example` を追加し、MSW 無効化・API ベース URL・dev/preview プロキシ・ORCA bridge・観測系の整理を反映。
    - `web-client/.env.sample` に `VITE_DISABLE_MSW` / `VITE_AUDIT_ENDPOINT` / `VITE_DEV_PROXY_TARGET` / `VITE_DEV_USE_HTTPS` を追記。
    - `web-client/README.md` に本番/ステージ分離、MSW 無効化時の接続パス、CORS/証明書/プロキシ前提を追記。
  - 成果物:
  ...
  - 検証結果（コード確認）:
    ...
    - dev/preview プロキシ: `web-client/vite.config.ts` が `VITE_DEV_PROXY_TARGET` を `/api` `/api01rv2` `/orca21` `/orca12` に中継。
  - 残課題:
