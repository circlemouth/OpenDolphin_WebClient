...
- 取得した HAR:
  - `webclient-api-usage.har`: GET `/api/user/LOCAL.FACILITY.0001:dolphin` = 200、POST `/orca/claim/outpatient/mock` = 200（runId=`20251208T124645Z`, dataSource=server）、POST `/orca21/medicalmodv2/outpatient` = 415 (Content-Type/userName 無し)。
...
- サーバーログの traceId (`/api/user`: 6634187b-9732-4634-bdb2-90aefb048820, `/orca/claim/outpatient/mock`: d7e3203d-bf5d-4808-8913-8bfae96cc2d8, `/orca21/medicalmodv2/outpatient`: f134c2c2-7023-4fc5-9d49-69814112881a) で `/openDolphin/resources` に到達していることを確認。

## OpenAPI / UI 対応
- `/api/user/{facilityId:userId}`: Web クライアント Login（operationId: 既存 UserResource）。認可成功。
- `/orca/claim/outpatient/mock`: モダナイズ側スタブ（ORCA spec 未定義）→ Reception/Outpatient モックの tone 取得。200 応答に `runId/dataSourceTransition` 等を含む。
...
- operationId 対応表（OpenAPI ↔ UI）で `/orca/claim/outpatient/mock` と `/orca21/medicalmodv2/outpatient` を「ORCA spec 外スタブ」として明記し、経路リライト不要であることを併記する。
