...
- [ ] 受付（claim/outpatient, appointment/outpatient）  
  ...
  - 期待レスポンス/画面: `/api01rv2/claim/outpatient/*` が 200、`dataSourceTransition=server`、`cacheHit` はリロード時 true→false の遷移が記録される。`missingMaster=false` で警告なし、受付バナーに runId と traceId が表示。  
...
- [ ] （オプション）予約・試算（appointment/outpatient）  
  ...
  - 期待レスポンス/画面: `/api01rv2/appointment/outpatient/*` が 200、`dataSourceTransition=server`、予約一覧が UI に表示。  
