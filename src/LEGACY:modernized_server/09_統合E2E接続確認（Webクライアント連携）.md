...
- 2025-11-22 RUN では `OrcaWrapperService` の no-arg コンストラクタ欠如による `WELD-001435`/`WELD-001410` 依存解決エラーを想定していたが、最新の `docker compose -f docker-compose.modernized.dev.yml logs --no-color server-modernized-dev | rg -n WELD-0014` ではこれらのエラーが再発せず、`server_latest_logs.txt` に 23:56:26 の `WELD-000119` および 23:58:38 の `WELD-001478` だけが残っている状況である (`server.log` に `/api/user` 関連の DeploymentException や 500 も見当たらない)。したがって ORCA REST がデプロイされていないというより、Web クライアントからの `/api/user` が WildFly まで到達していない状態と考えられる。
- そのため `/api/user/<facility>:<userid>` の HTTP 500 は Vite dev/proxy 層や `httpClient` が組み立てるヘッダー・TraceContext・`auth-headers.ts` の `userName`/`password`/`clientUUID` 出力など、port 5173 → WildFly 9080 の間で発生している可能性が高い。`curl -H "userName: dolphindev"` + `-H "password: 1cc2f4c06fd32d0a6e2fa33f6e1c9164"` + `-H "X-Facility-Id: 1.3.6.1.4.1.9414.10.1"` `http://localhost:9080/openDolphin/resources/user/1.3.6.1.4.1.9414.10.1:dolphindev` が HTTP 204 を返し `server.log` 742-815 に missing credentials → 認証成功の traceId が並ぶことから、バックエンド自体は稼働している。再発時には Vite/axios のデバッグ出力 (`VITE_HTTP_LOG_BASEURL`/`DEBUG=axios`)、`auth-headers.ts`、および `server.log` の `Missing credentials headers`（行 742）を並行確認し、`docs/server-modernization/phase2/operations/logs/20251122T071146Z-e2e.md` へ証跡を載せて front-end チームと連携する必要がある。

## ワーカー指示（RUN_ID=20251122T071146Z）
- `docker compose -f docker-compose.modernized.dev.yml logs --no-color server-modernized-dev | rg -n WELD-0014` ＋ `rg -n WELD-0014 server_latest_logs.txt` を繰り返し実行し、`WELD-001410/001435` の再発が出ていたら即座に本ファイルと `docs/server-modernization/phase2/operations/logs/20251122T071146Z-e2e.md` の RUN_ID セクションへ記録。現状 `server_latest_logs.txt` の 23:56:26/WELD-000119 と 23:58:38/WELD-001478`のみが残っているので、発見済み lines を追記して再発監視のエビデンスを維持する。
- `curl -H "userName: dolphindev"` + `-H "password: 1cc2f4c06fd32d0a6e2fa33f6e1c9164"` + `-H "X-Facility-Id: 1.3.6.1.4.1.9414.10.1"` `http://localhost:9080/openDolphin/resources/user/1.3.6.1.4.1.9414.10.1:dolphindev` で HTTP 204 を取得し、同時に `server.log` の `Missing credentials headers`（行 742）および成功ログ（814-815）を抜粋して証跡化。`http://localhost:5173/api/user/1.3.6.1.4.1.9414.10.1:dolphindev` が 500 を返す再現時には `server.log` に `/api/user` や 500 の記録がないことを確認し、Vite dev サーバー/`httpClient` のデバッグ出力（`VITE_HTTP_LOG_BASEURL`・`DEBUG=axios` を有効化）と `auth-headers.ts` の header 生成を併せて記録する。
- フロントエンド側の 500 エラーについては、`docs/server-modernization/phase2/operations/logs/20251122T071146Z-e2e.md` の該当セクションに traceparent `00-11111111111111111111111111111111-2222222222222222-01`/`x-request-id: front-end-test-1` 付きで `https://localhost:5173/api/user/1.3.6.1.4.1.9414.10.1:dolphindev` にアクセスした際の `getaddrinfo ENOTFOUND host.docker.internal`（`/tmp/vite.log` 7:27:22・7:27:51）を追記し、`grep -n "/api/user" server.log | tail` が空だった事実と併せて他ワーカーが同じ証跡をたどれるようリンクを残す。必要であればこの RUN_ID 付近に `/api/user` 500 用の front-end 監視テンプレート（Vite console ⟷ `auth-headers.ts` ⟷ `server.log` の trace）を追記し、traceparent/x-request-id を添えて front-end チームと共有してください。
