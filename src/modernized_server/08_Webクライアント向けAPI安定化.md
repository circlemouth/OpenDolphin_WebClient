...
- `docs/web-client/ux/legacy/CHART_UI_GUIDE_INDEX.md` で定義される SLA に合わせ、主要エンドポイントのレスポンス時間とペイロードサイズを計測し、閾値と改善策を提示する。
...
2. `docs/web-client/ux/legacy/CHART_UI_GUIDE_INDEX.md` の SLA に対するレスポンス時間・ペイロード実測表が作成され、閾値超過時の改善案/暫定フォールバックが提示されている。
...
- **A. 対象 API と SLA の洗い出し**
  ...
  - `docs/web-client/ux/legacy/CHART_UI_GUIDE_INDEX.md` に記載の SLA（応答時間・ペイロード上限・タイムアウト/リトライ要件）を API ごとに紐付け、計測対象を確定する。
- **B. 互換レイヤー設計（破壊的変更の吸収）**
...
## リスクと留意点
- SLA 基準が UI ガイド側で更新された場合の差分検知遅延リスク → `ux/legacy/CHART_UI_GUIDE_INDEX.md` 更新日付を計測ログに記載し、差分を確認してから計測する。
