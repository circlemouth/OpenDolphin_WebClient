...
- 例外・バリデーション統一ドラフト
  ...
  - Content-Type: `/api01rv2/system01dailyv2` 等の XML 系は `application/xml; charset=UTF-8` 固定（Shift_JIS は Api_Result=91）、予約/請求の JSON 系も `Accept/Content-Type` の明示を必須化。BASIC 認証は `mac-dev-login.local.md` 記載のものを統一。
