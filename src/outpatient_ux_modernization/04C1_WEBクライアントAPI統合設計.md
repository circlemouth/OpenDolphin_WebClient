...
1. `web-client/src/libs/http/httpClient.ts` に `OUTPATIENT_API_ENDPOINTS` を追加し、`/api01rv2/claim/outpatient/*`、`/api01rv2/appointment/outpatient/*`、`/orca21/medicalmodv2/outpatient`、`/orca12/patientmodv2/outpatient` を一覧化しました。`purpose`/`auditMetadata`/`sourceDocs` を含めることで開発者が各エンドポイントに必要な `runId`/`cacheHit`/`missingMaster`/`fallbackUsed` を即座に把握できます。`docs/web-client/architecture/web-client-api-mapping.md` にテーブルと図を作成し、今後の UI 連携設計で `httpClient` との対応表を二次資料として使えるようにしています。
