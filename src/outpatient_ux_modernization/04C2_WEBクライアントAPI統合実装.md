...
- **RUN_ID=20251205T090000Z** / 期間: 2025-12-05 09:00 - 2025-12-09 09:00 JST
...
3. docs 連携: `docs/web-client/ux/reception-schedule-ui-policy.md` と `docs/web-client/ux/ux-documentation-plan.md` に接続フロー図＋差分を追記し、新 log `docs/server-modernization/phase2/operations/logs/20251205T090000Z-integration-implementation.md` へ API パス一覧・telemetry funnel/Orchestration flag 実装を記録。`docs/web-client/planning/phase2/DOC_STATUS.md` の `Web クライアント UX/Features` 行にも RUN_ID=`20251205T090000Z` を反映。

## 3. 現状・懸念

- RUN_ID=`20251205T090000Z` で `telemetryClient`/`charts/orchestration`/`OrderConsole` の `cacheHit`/`missingMaster` トリガを `funnels/outpatient` に記録する差分を適用し、`docs/server-modernization/phase2/operations/logs/20251205T090000Z-integration-implementation.md` に実装内容と API パス一覧を証跡化した。今後 Stage/Playwright で同ログを再現し、`tone=server` + `dataSourceTransition=server` の `telemetry` funnel を観測することが残る。
...
1. Stage/Playwright で `cacheHit`/`missingMaster` → `telemetry` funnel の連鎖と、`setResolveMasterSource('server')` による `tone=server` バナーの同期を再現し、そのログ・スクリーンショットを `artifacts/webclient/ux-notes/20251205T090000Z-*` に記録して `docs/server-modernization/phase2/operations/logs/20251205T090000Z-integration-implementation.md` へ追記する。
...
3. DOC_STATUS に RUN_ID=`20251205T090000Z` を維持し、manager checklist の該当行と README で本ログと `docs/web-client/ux/ux-documentation-plan.md` / `docs/web-client/ux/reception-schedule-ui-policy.md` の差分を参照できるようにする。
