# 04C4 外来 API ギャップ実装（RUN_ID=20251208T113620Z）
...
- `OutpatientClaimResource`（`/api01rv2/claim/outpatient/mock`）と `OrcaMedicalModV2Resource`（`/orca21/medicalmodv2/outpatient`）を server-modernized に追加し、`resolve_master` → `charts_orchestration` funnel へ渡す flag をデフォルトで埋め込んだ。`OrcaMedicalModV2Resource` は `SessionAuditDispatcher` で `ORCA_MEDICAL_GET` を記録。
- 契約モックを `artifacts/api-stability/20251208T113620Z/outpatient/` に配置し、`OutpatientMockPage` の fallback RUN_ID を `20251208T113620Z` に更新。
- 証跡ログ: `docs/server-modernization/phase2/operations/logs/20251208T113620Z-api-gap-implementation.md`。DOC_STATUS/`web-client-api-mapping.md` と同期済み。
...
- AuditEvent は `ORCA_MEDICAL_GET` のみ記録しており、`ORCA_CLAIM_OUTPATIENT` 側では SessionAuditDispatcher をまだ呼び出していない。監査要件を満たすには claim 側にも dispatcher 呼び出しを追加すべき。
