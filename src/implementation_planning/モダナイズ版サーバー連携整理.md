...
- server-modernized 現状
  - `/orca/appointments/list/*` ルートは未実装
...
- 監査メタ必須
  - `runId`, `dataSourceTransition`, `cacheHit`, `missingMaster`, `fallbackUsed`, `recordsReturned`
- server-modernized 現状
  - `OrcaClaimOutpatientResource` で `/orca/claim/outpatient` 実装済み
  - `queueEntries` は未返却のため、ORCA キュー警告は空配列扱い
- 追加/修正ポイント
  - 必要であれば `queueEntries` を含める（Reception のキュー状態表示と一致させる）

### Patients: ORCA 反映状態
#### `/orca/patients/local-search/*` (POST)
...
- server-modernized 現状
  - `/orca/patients/local-search/*` ルートは未実装
...
- server-modernized 現状
  - `OrcaClaimOutpatientResource` で実装済み
...
- 既存実装の補完
  - `OrcaClaimOutpatientResource` に `queueEntries` を追加するか、別 API で連携
...
- [ ] Patients の検索 API を `/orca/patients/local-search/*` または `/orca/patients/*` に整理
