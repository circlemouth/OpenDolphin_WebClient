...
- Secrets 注入チェック
  - 現状の `ops/check-secrets.sh` で `PHR_LAYER_PRIVATE_KEY_BASE64` / `PHR_LAYER_PRIVATE_KEY_PATH` の検証を実施済み。
  - base64 指定時は形式チェック + デコード検証、不正/空を検知。path 指定時は不存在/空ファイルを検知。
- 監査理由の一貫性
