...
### 2.4 ドラフト TTL / 掃除フック（本章フックに追加）
- 掃除タイミング（実行ポイントを明記）:  
  1) `AuthService` の `runId` 再発行時（再ログイン/ユーザー変更）、  
  2) `facilityId` 変更検知時、  
  3) 明示ログアウト時（`AuthService.clear()` 呼び出し）、  
  4) アプリ起動時/Charts 初期化時に `SessionContext.issuedAt + ttlHours` を超えていないかチェックするウォームアップフック、  
  5) 定期ハートビート（5分間隔）で TTL を越えたものをスイープ。
- TTL ルール: `draftExpiryHours=72` を超えたドラフトは scope 一致していても復元候補にしない。`updatedAt` が TTL を過ぎた時点で自動削除し、`DRAFT_DISCARD outcome=success reason=ttl_expired` を audit に送信。
- 掃除対象: `localStorage` キー `charts:draft:*` で始まるものと、React Query の `draft` 系 cache key。runId/facilityId/userId 不一致も同時に削除する。
- 例外: ユーザーが「あとで」を選択した直後（復元保留状態）でも TTL 超過したら削除し、再度ダイアログを出さない。
- UI 告知: TTL で削除した場合は info トースト（polite）を 1 回だけ表示。「72時間を過ぎた下書きを削除しました」。assertive では鳴らさない。

### 2.5 テスト観点（TTL）
- ケース: `updatedAt` が 72h+1m 経過した draft を持つ状態で Charts を開く → ウォームアップフックが draft を削除し、`DRAFT_DISCARD outcome=success reason=ttl_expired` が送信されること。UI は「72時間を過ぎた下書きを削除しました」を info トースト（polite）で 1 回だけ表示、復元ダイアログは出ないこと。
