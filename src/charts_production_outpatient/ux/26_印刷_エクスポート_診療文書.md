...
> 追記: `location.state` はリロードで失われるため、UX 劣化を避ける目的で **短期の sessionStorage 復元（最大10分）** を許可する。出力後/閉じるで破棄する（PHI を含むため）。
...
- 最低限含める情報:
  - **誰が**: `details.actor = facilityId:userId`（ログインにより確定した `sessionStorage(opendolphin:web-client:auth)` を優先）
  - **いつ**: `timestamp`（ISO）
