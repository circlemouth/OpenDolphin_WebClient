...
- **取得元**: 患者取得 API `/api01rv2/patient/outpatient/basic|insurance`（03 契約表 1.4/1.5）で `updatedAt`/`updatedBy` を受け取り、直近保存値として使用。保存成功後は応答の値を `newValue` として即反映。
...
## 9. リスク・未決事項
- 保険情報の差分取得 API は `/api01rv2/patient/outpatient/*` の応答項目（updatedBy/updatedAt/versions）依存。未提供なら暫定で保存応答を直近値として扱う。
...
- 差分表示: `Patients` の取得結果（`/api01rv2/patient/outpatient`）をベースラインとして before/after を表示し、強調/解除・監査ログクリック→差分フォーカスを追加。
...
### 未完/今後
- 設計通りの差分ベースライン（`/api01rv2/patient/outpatient/basic|insurance` の分離取得、`updatedAt/updatedBy`・版管理）へ移行する。
