...
3) **コンフリクト確認（documentRevision × contentHash 突合）**: サーバーの最新 `documentRevision`（`03` 契約テーブル）とドラフトの `contentHash`/`updatedAt` を比較し、古いドラフトや競合をダイアログで提示。選択肢: 「ドラフトを適用」「サーバー版を保持」「マージ（フィールド単位選択）」。
...
- **TTL 超過**: `updatedAt` から 72 時間経過したドラフトは復元候補にせず自動削除し、info トースト（polite）と `DRAFT_DISCARD outcome=success reason=ttl_expired` を送信。
...
### 付録: documentRevision × contentHash 突合アルゴリズム（擬似コード）
```ts
// inputs: serverRevision, serverUpdatedAt, serverContentHash? (optional)
//         draftRevision?, draftUpdatedAt, draftContentHash
function decideDraftConflict(input) {
  const { serverRevision, serverUpdatedAt, serverContentHash,
          draftRevision, draftUpdatedAt, draftContentHash } = input;
  // 1. 不正ドラフト
  if (!draftContentHash) return {status: 'discard', reason: 'invalid_schema'};
  // 2. TTL は上位フックが判定済み。ここでは突合のみ。
  // 3. 完全一致（ハッシュ一致 or リビジョン一致）
  if (serverContentHash && serverContentHash === draftContentHash) {
    return {status: 'skip', reason: 'identical'}; // 復元不要
  }
  if (draftRevision && serverRevision && draftRevision === serverRevision) {
    return {status: 'restore', reason: 'same_revision_but_local_changes'};
  }
  // 4. サーバーが新しい場合（revision/updatedAt 比較）
  const serverNewer = serverRevision && draftRevision
    ? serverRevision > draftRevision
    : (serverUpdatedAt && draftUpdatedAt ? serverUpdatedAt > draftUpdatedAt : false);
  if (serverNewer) {
    return {status: 'conflict', reason: 'server_newer'};
  }
  // 5. ローカルが新しい
  return {status: 'restore', reason: 'draft_newer'};
}
```

### 競合 UI を出す条件
- `decideDraftConflict(...).status === 'conflict'`（サーバーが新しい）または `status === 'restore'` かつ `reason !== 'identical'` の場合に復元モーダルを表示。
- `status === 'skip'` はモーダル無し（復元しない）。`status === 'discard'` はトースト＋監査のみ。
- モーダル内では以下を並べて表示し、フィールド別マージを許可する:  
  - サーバー版: revision / updatedAt / 差分ハイライト  
  - ドラフト版: updatedAt / contentHash  
  - CTA: 「ドラフトを適用」「サーバー版を保持」「フィールドを選んで適用」
