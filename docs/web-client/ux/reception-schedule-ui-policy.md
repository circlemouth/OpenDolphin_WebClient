...
- **接続フロー**: `resolveMasterSource` で `dataSourceTransition=server` になったタイミングで `httpClient` 経由の外来 API（`/orca/appointments/list/*`, `/orca/visits/list/*`, `/orca21/medicalmodv2/outpatient` など）が実行され、キャッシュ命中／未命中フラグ (`cacheHit`/`missingMaster`) が `telemetryClient` の `funnels/outpatient`（RUN_ID=`20251205T150000Z` + `docs/server-modernization/phase2/operations/logs/20251205T150000Z-integration-implementation.md`）へ送出される。AuthServiceProvider は `recordOutpatientFunnel('resolve_master', …)` で最初のステージをキャプチャし、`handleOutpatientFlags` で `charts_orchestration` を記録したタイミングで `setResolveMasterSource('server')` を呼び出して Reception/Charts の `tone=server` + `dataSourceTransition=server` 表示を同期する。
- **接続図 (概要)**:
```
       [resolveMasterSource]
               |
