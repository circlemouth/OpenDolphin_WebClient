...
│ ・プロブレム/病名(現症)     │ ・SOAPノート(HPI/ROS/PE/A/P)     │ 〔処置〕〔紹介/文書〕   │
...
* 内容：**要約メモ**（直近3回のA/P抜粋、慢性疾患の管理方針、禁忌、前回の保留課題など）。
* 操作：医師が**ピン留め**、**版管理/注釈**、**更新日時**。

> 一目で臨床の**骨子**を掴ませ、毎回の読込負荷を削減。NISTは注釈機能や内容の整理・現在値強調・スクロール統一などを推奨。([NIST Publications][1])
...
* **最新値を強調**し、前回比の矢印/色で変化を示す（**現在値の区別**）。([NIST Publications][1])
...
**3) Assessment & Plan（A/P）—“オーダのハブ”**

* 各Assessment行の右端に**〔処方〕〔検査〕〔画像〕〔処置〕**の**ワンクリック**ボタン。**選択と同時に右レールへプリセット**され、**署名前レビュー**だけで確定。
* **オーダセット**（疾患別）と**お気に入り**で高速化。

> **CPOE（電子オーダ入力）**を**臨床文脈のすぐ横**に置くのが外来では最も速い。CPOEは紙より安全だが、**ワークフローと一体化**して初めて真価を発揮。([PSNet][4])

---

### D. 右レール：オーダ & 結果・会計（幅 約22%）

**タブ構成**：〔処方〕〔検査〕〔画像〕〔処置〕〔紹介/文書〕〔会計/コード〕

* **処方**：候補（前回/疾患別）、**相互作用・アレルギーチェック**、剤形・日数・用法の**ショートカット**。
* **検査/画像/処置**：**クイックセット**（例：上気道炎セット、健診フォローセット）。
* **結果ビュー**：**直近結果**は常時表示、時系列比較はワンクリックで展開。
* **会計/コード**：A/Pから**ICD・手技**を自動提案。

> **知識・意思決定支援**（相互作用、リマインダ、ガイドライン）は**リアルタイム表示**で安全性を高める。([NCBI][2])
> **外部システム起動を最小化**し、EHR内で完結することはワークフローを単純化。([NIST Publications][1])
...
| **バイタル（現値/トレンド）**    | **左レール中段**                      | **現在値強調**・変化の即時把握。([NIST Publications][1])             |
| **プロブレム/病名（現症）**     | **左レール上段**                      | 重要度順で迅速にA/Pへ。IOMのコア情報。([NCBI][2])                      |
| **処方・検査等オーダ**        | **右レール**（A/P横で起動）               | **CPOEをA/Pに隣接**し、ワンクリックで発注→署名。([PSNet][4])             |
...
1. **オーダはA/Pの文脈から**：Assessment行の右端ボタンで**1クリック発注→右レールで即編集→フッタで署名**。紙より安全、かつ外来の短時間に適合。([PSNet][4])
...
* 問診カードを**取込**→テンプレートから**PE**→**A/P**→**処方**（相互作用・アレルギー自動チェック）→**患者向け指導文**を右レール〔文書〕で即印刷/ポータル送信。([NCBI][2])
...
* **患者サポート**（事前問診/教育資料/ポータル配信）：中央問診カード→右レール〔文書〕。([NCBI][2])
* **事務プロセス**（スケジュール/適格性/会計）：右レール〔会計/コード〕で**医師の動線を阻害しない最小限 UI**。([NCBI][2])
* **報告・集団健康**（ダッシュボード/公的報告）：右レールの**レポート**やバックエンド出力に接続。([NCBI][2])
