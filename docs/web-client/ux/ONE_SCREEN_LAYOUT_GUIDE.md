...
* **AppShell 本体はウィンドウ幅へ追従**: `AppShell` のコンテンツコンテナ（患者一覧・受付一覧・ChartsPage など共通）は `contentMaxWidth` 制限を撤廃し、ウィンドウを広げた分だけ中央カラムが連続的に拡張する。大画面での余白空間は左右レールの固定幅と外周パディングのみとなる。
* **カルテ未選択時のヘッダー圧縮**: ChartsPage では患者未選択のケースでヘッダーを約 60px のコンパクトモードに切り替え、トップメニューとの垂直余白を最小化する（患者選択後は通常モードの高さへ復帰）。



### 23インチ(1920px)ディスプレイ前提の寸法指標
* `grid-template-columns` は `minmax(264px, 21vw) / minmax(var(--charts-central-min-width), 1fr) / minmax(264px, 21vw)` を前提に、左右レールの最小幅 264px（ブレークポイント毎に 252px → 236px へ縮退）と中央カラムの残余幅全取得を両立する。ヘッダー 80px + ステータスバー 48px を差し引いた高さで中央・右ペインの固定表示を維持する。
* 1600px を超える環境でも中央カラムに上限を設けず、左右レールを最大 288px に抑えたまま中央がウィンドウ全幅に追従する。1280px／1100px ブレークポイントでは左右レールを段階的に縮め、1000px 未満は右レールを折りたたみ＋ホバー展開に制限して入力欄の横幅を確保する。
* 中央カラムのスクロール領域は常に画面全幅から左右レール・列間ギャップ・外周パディングを差し引いた値となり、23インチ FHD では約 1,256px（右レール展開時）／約 1,488px（右レール折りたたみ時）を確保して SOAP 記載とタイムライン／オーダ操作を同時視認できることを手動・コード両方で検証済み。

#### 2025-11-01 追記: Swing 版レイアウトとの寸法合わせ方針（担当: Codex）
* 参考スクリーンショット（Swing 版: 1280×720 同等構成）では左パネル約280px・中央ノート約720px・右パネル約280pxで全幅 1280px を構成している。Web 版ではこの分割比を `clamp()` とドッキング制御で再現する。
* 基準ブレークポイントを 1366px に設定し、`grid-template-columns` を `minmax(264px, 21vw) / minmax(var(--charts-central-min-width), 1fr) / minmax(264px, 21vw)` とする。中央カラムに上限を設けず、ビューポート幅に応じて残余幅をそのまま割り当てる（例: 1366px 幅 ≒ 763px、1600px 幅 ≒ 943px、1920px 幅 ≒ 1,256px）。
* 右ペインは Swing 版の縦アイコンバーと同寸（48px幅ボタン）＋内容領域 212px の二段構成に変更する計画。1000px 未満ではアイコンバーのみ表示し、内容はモーダル／スライドアウトで提供して幅超過を防ぐ。
* 左レールのカードは上下マージン 12px／内側パディング 12px に統一し、カード幅 264px 内で最大 6 行のテキストを想定する。Swing 版で「主病名リスト」と「履歴」セクションが同列に並ぶ密度に合わせた視認性を確保する。
* 中央カラムは最小幅 640px を死守しつつ、SOAP ノートの本文フォントを 13px 相当（`0.82rem`）まで縮小して 14 行以上の視認性を確保。既存コンポーネントは維持し、サイズの再設定のみで実現する。
