...
- 備考: MSW OFF 時は Playwright で `/orca21/medicalmodv2/outpatient` `/orca12/patientmodv2/outpatient` `/api/admin/config|delivery` `/api/orca/queue` をフィクスチャで `dataSourceTransition=server, cacheHit=true, missingMaster=true/false` 応答に差し替え、トーンと `aria-live` を強制的に検証する。
