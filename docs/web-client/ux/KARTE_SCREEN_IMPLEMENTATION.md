...
- 右カラムの「患者検索」カードは漢字氏名／カナ氏名／患者ID／数字検索（下4桁など）の4入力欄を横2列で配置し、入力済みのすべての条件に一致する患者のみを一覧表示する。検索対象タブは廃止し、カード全体はギャップ 12px のグリッドでコンパクトに保つ。「条件をクリア」ボタンで入力値を即座にリセットでき、検索ボタンの左側に配置してキーボードフォーカスの流れを維持する。
