...
- トースト/バナーは `docs/web-client/ux/CHART_UI_GUIDE_INDEX.md` にあるカテゴリ分け `neutral/info/warning/danger` を踏襲し、`management-ui-guide` に従って `warning` バナーは ORCA master fallback ・ `danger` toast は保存エラーと紐づける。Badge/Toast/Banner は同じ `tone` を共有することで監査ログとの整合性を保つ。
