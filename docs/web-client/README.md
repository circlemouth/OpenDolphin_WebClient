...
- [`ux/patients-admin-ui-policy.md`](ux/patients-admin-ui-policy.md): Patients 系と Administration 系の UI/ORCA 連携・トーン管理・監査を整理し、PHR/ORCA master bridge のブロックチェックと `missingMaster`/`fallbackUsed` による警告パターンを記述（RUN_ID=`20251127T133000Z`）。20251127T170500Z には `PatientEditorPanel` を `SurfaceCard`/`StatusBadge` で ORCA master readiness を可視化し、住所/保険情報が揃うまでは保存ボタンを `disabled` にして `warning` バナーを出す実装を追加。証跡: `web-client/src/features/patients/components/PatientEditorPanel.tsx`。
...
- 2025-11-27 (担当: Codex): `PatientEditorPanel` に ORCA master readiness を示す `SurfaceCard`/`StatusBadge` を追加し、住所・郵便番号・保険者番号/記号/番号/区分コードが揃うまでは保存ボタンを `disabled` にして `warning` バナーを出すことで `masterRequiredFields` を強制する実装をデプロイ。RUN_ID=`20251127T170500Z`、証跡: `web-client/src/features/patients/components/PatientEditorPanel.tsx` + `docs/web-client/ux/patients-admin-ui-policy.md`。
