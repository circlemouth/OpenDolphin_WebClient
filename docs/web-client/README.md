...
- 2025-11-08 (担当: Codex): `docker/orca/jma-receipt-docker` サブモジュールを追加し、WebORCA コンテナを ORCA↔OpenDolphin 通信テスト用に定義。起動・ネットワーク接続・`claim.host=orca` 設定・ヘルスチェック手順を `docs/server-modernization/phase2/operations/EXTERNAL_INTERFACE_COMPATIBILITY_RUNBOOK.md#44-weborca-テストコンテナ2025-11-08-追加` に追記し、証跡を `artifacts/orca-connectivity/20251108T075913Z/` へ保存。
