...
- 2025-11-14 (担当: Codex): WebORCA 連携方針をローカルコンテナから WebORCA クラウド本番（`https://weborca.cloud.orcamo.jp:443`）へ切り替え。`docs/server-modernization/phase2/operations/ORCA_CONNECTIVITY_VALIDATION.md#44-weborca-クラウド接続2025-11-14-更新` と `operations/EXTERNAL_INTERFACE_COMPATIBILITY_RUNBOOK.md#44-weborca-クラウド接続2025-11-14-更新` に証明書管理・`curl --cert-type P12`・`ServerInfoResource` の手順を統一し、証跡ディレクトリを `artifacts/orca-connectivity/20251113TorcaProdCertZ1/` へ集約。
