...
| [`docs/server-modernization/phase2/PHASE2_PROGRESS.md`](../../../server-modernization/phase2/PHASE2_PROGRESS.md) | モダナイズ/運用 | Active | 2025-11-10 | CLAIM/DIAGNOSIS/MML 再検証節に 23:18Z 実測（`diseaseHelper` 再同梱→DLQ 0件→`d_diagnosis` 追加）の更新内容と残課題（`d_diagnosis_seq` 補正、Legacy 403、レスポンス仕様化）を追記。 |
...
| [`docs/server-modernization/phase2/operations/TRACE_PROPAGATION_CHECK.md`](../../../server-modernization/phase2/operations/TRACE_PROPAGATION_CHECK.md) | モダナイズ/運用 | Active | 2025-11-10 | §5.2 で RUN_ID=`20251110T035118Z` の `modernized-dev` プロファイルにおける `curl: (6)` / `docker` 未接続ブロッカーと回避案を記録し、§6 では RUN_ID=`20251110T002045Z` の Trace Harness 実行結果（`trace_http_*` / `trace-schedule-jpql` / `trace-appo-jpql` の status・SessionOperation/traceId ログ・既知ブロッカー）を整理。証跡は `artifacts/parity-manual/TRACEID_JMS/20251110T035118Z/` / `20251110T002045Z/`、`TRACEID_JMS_RUNBOOK.md` へリンク済み。 |
...
| [`docs/server-modernization/phase2/notes/domain-transaction-parity.md`](../../../server-modernization/phase2/notes/domain-transaction-parity.md) | モダナイズ/ドメイン | Active | 2025-11-10 | RUN_ID=`20251110T002045Z` の Trace Harness 表と `trace_http_*` / `trace-schedule-jpql` / `trace-appo-jpql` の現状に更新。§3 では Flyway V0223 + persistence.xml 更新で UnknownEntityException を解消しつつ、Modernized 側で `remoteUser=anonymous` による `list=null`、`AppointmentModel` seed 未整備による 500 を TODO に記録。`rest_error_scenarios.manual.csv` の該当行に再現日時を追記。 |
