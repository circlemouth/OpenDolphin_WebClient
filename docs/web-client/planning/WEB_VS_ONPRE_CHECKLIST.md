...
## 5. 未実装（要フォローアップ）
- [ ] **CareMap（治療履歴カレンダー）**: オンプレ版 `CareMapDocument` が提供するカレンダービュー、予約・検査履歴連動、画像履歴の統合表示を Web クライアントへ移植する。
  - 未着手: カルテ画面のコンポーネントに CareMap 相当のモジュールが存在せず、治療履歴カレンダー UI が欠落している。
- [ ] **ラボ検査履歴ビューア**: `LaboTestPanel` が担う検査結果一覧・グラフ表示・PDF 出力を Web クライアントにも実装する。
  - 未着手: カルテ画面に検査結果を扱うモジュールがなく、ラボデータの参照や可視化機能が不足している。
...
- [ ] **施設全体の予約一覧（PatientSchedule）**: `PatientScheduleImpl` と同様に日付・担当医で施設全体の予約を俯瞰できるビューを追加する。
  - 未着手: Web クライアントは来院患者ごとの予約編集に留まり、全体スケジュールを横断的に参照する画面が未提供。
