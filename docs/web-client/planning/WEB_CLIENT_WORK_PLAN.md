# Webクライアント開発 実装計画 (更新: 2026-05-24)
...
- **タスク**
  ...
  - [x] `/patient/*` API を利用した検索・一覧 UI と安全情報（アレルギー/危険サイン）の常時表示を実装。
    ...
    - 2026-05-03: `PatientsPage` に患者情報編集カードを追加し、`/patient` POST/PUT を利用した新規登録・更新と健康保険モジュール統合を実装。フォームバリデーション、監査ログ、React Query キャッシュ再取得を連動させ、受付→編集→保存の業務フローを Web でも完結させた。
...
- **タスク**
  - [x] `/stamp` 系 API を利用したスタンプ閲覧・検索・挿入 UI とお気に入り機能を実装。
    ...
    - 2026-05-03: `/stamp/id/{stampId}` から取得した `ModuleModel` を `orderModules` として保持し、Plan カードにオーダ概要を表示した上でカルテ保存時にモジュールを送信する実装へ移行。
  - [x] `/orca/tensu/*`, `/orca/disease/*`, `/orca/general/*` 等のマスター検索 UI を中央/右カラムに統合し、1 クリック発注導線を構築。
    ...
    - 2026-05-03: `OrcaOrderPanel` から `/orca/stamp/{code},{name}` を呼び出して `ModuleModel` を生成し、そのままカルテへ保存できる「カルテに追加」操作を追加。保存前に `orderModules` へ編成し Claim フラグを更新。
...
- **タスク**
  - [x] HTTPS 証明書配備、CORS、CSP、CSRF 対策を適用し、認証ヘッダー保護方針を文書化。
    - 2026-03-30: `initializeSecurityPolicies` を追加して HTTPS リダイレクトと CSP/Referrer-Policy を強制。`httpClient` へ CSRF トークン自動付与と監査ログ除外ルールを実装し、`features/PHASE4_SECURITY_AND_QUALITY.md` に運用方針を記載。
  - [x] フロントエンドのキャッシュ制御を設計し、患者情報がブラウザキャッシュやローカルストレージに残留しないよう HTTP ヘッダー／Storage ポリシーを適用。
    - 2026-03-30: 全リクエストへ `Cache-Control: no-store` を付与し、認証セッションは `sessionStorage` / インメモリのみに限定。キャッシュ残留防止策をドキュメント化。
  - [x] 監査ログ/操作ログの設計と出力（診療履歴・請求操作）を実装、SIEM 連携計画をまとめる。
    - 2026-03-30: `libs/audit` で HTTP/業務ログをバッファリングし `/audit/logs` へ送信。ログイン・患者検索・カルテ取得・ORCA チェックの操作記録を追加。
  - [x] Lighthouse/axe 等によるアクセシビリティ評価を実施し、キーボード操作やスクリーンリーダ対応を修正。
    - 2026-03-30: `vitest-axe` を導入し `AppShell` の a11y テストを追加。通知まとめに `role="status"` を付与し、Skip Link の挙動を検証。
  - [x] ORCA マスター検索やカルテ描画のレスポンス測定を行い、目標 (3 秒/5 秒) を満たすよう最適化。
    - 2026-03-30: `measureApiPerformance` で患者検索/カルテ取得/ORCA API を計測し、閾値超過を監査ログに記録。
  - [x] 30 クライアント同時接続を想定した負荷テストを自動化し、ボトルネック改善計画を策定。
    - 2026-03-30: `npm run test:load` で実行できるシナリオスクリプトを作成し、P50/P90/P99 を収集する仕組みを整備。
  - [x] シフト制オペレーション向けにアラート疲労対策のルール（重大通知のみに遮断を適用）を反映。
    - 2026-03-30: ORCA 併用禁忌チェックに重大/注意/参考の分類とトグル UI を追加し、重大通知のみ遮断対象とする運用を実装。
- **完了条件**
...
- **UI ギャップ解消計画**: `planning/UNIMPLEMENTED_API_UI_PLACEMENT.md` を参照し、未整備 API に紐づく UI の配置とフェーズを四半期ごとのスプリント計画へ反映。フェーズ5以降での管理系 UI 追加時はアクセス制御と監査ログポリシーを必須とする。
