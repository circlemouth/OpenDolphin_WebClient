...
- `http://localhost:5173/api/user/1.3.6.1.4.1.9414.10.1:dolphindev` へのアクセスが依然 500 を返す点は Vite dev/proxy の DNS 解決 (`host.docker.internal`) に起因しているため、`DEBUG=axios,VITE_HTTP_LOG_BASEURL` + `VITE_DEV_PROXY_TARGET=http://host.docker.internal:9080/openDolphin/resources` のローカル dev サーバーを立ち上げ、`https://localhost:5173/api/user/1.3.6.1.4.1.9414.10.1:dolphindev` に `traceparent: 00-11111111111111111111111111111111-2222222222222222-01`/`x-request-id: front-end-test-1` を付けてアクセスしたところ、`/tmp/vite.log` 7:27:22・7:27:51 に `http proxy error: /user/…`＋`Error: getaddrinfo ENOTFOUND host.docker.internal` が出力され、`grep -n "/api/user" server.log | tail` は空で WildFly には到達していないことが確認できた。`docs/server-modernization/phase2/operations/logs/20251122T071146Z-e2e.md` の該当箇所に trace/ログを添えて front-end チームとこの RUN_ID セクションへ共有し、同じ traceparent/x-request-id を使って再現できるように監視テンプレートを補完してください。フロントエンド側は `VITE_DEV_PROXY_TARGET` を Docker network 上で解決可能なホスト（例: `http://opendolphin-server:9080/openDolphin/resources` 又は `server-modernized-dev` などの compose サービス名）または `extra_hosts` で `host.docker.internal` エイリアスを追加した設定へ切り替え、再度 `DEBUG=axios,VITE_HTTP_LOG_BASEURL` + trace parent/X-Request-Id を付加したリクエストで HTTP 500 が出るか・`getaddrinfo` エラーが消えるかを確認してください。その結果（TraceId/request-id、getaddrinfo の有無、WildFly への `/api/user` 到達可否）をこのファイルと DOC_STATUS・`docs/server-modernization/phase2/operations/logs/20251122T071146Z-e2e.md` で RUN_ID=20251122T071146Z に統一して共有してください。
