...
- 旧サーバーの移行計画は `docs/server-modernization/legacy-server-modernization-checklist.md` にまとめられており、Jakarta EE 10 化・依存更新・WAR 配備手順が網羅されている。
...
| Legacy 計画 | `legacy-server-modernization-checklist.md` | 旧サーバー全体の移行計画と進捗。 | Java 11/17, WildFly 26, REST parity などを網羅。 |
