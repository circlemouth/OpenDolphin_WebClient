...
> RUN_ID=`20251209T071955Z` で 04C5 再検証（MSW OFF ローカル）。`/api01rv2/claim/outpatient/mock` / `/orca21/medicalmodv2/outpatient` へ curl POST し、いずれも **200 OK**（`dataSourceTransition=server`, cacheHit={false,true}, missingMaster=false, runId=20251208T124645Z）。dev proxy `http://100.102.17.40:8000/...` と `https://100.102.17.40:{443,8443}/...` はすべて TCP timeout（HTTP 000, ヘッダ/ボディ無）。証跡: `docs/server-modernization/phase2/operations/logs/20251209T071955Z-integration-gap-retest.md`, `artifacts/webclient/e2e/20251209T071955Z-outpatient-gap/`（stage_connectivity_20251209T0730Z.txt 追加）、doc 更新: `src/outpatient_ux_modernization/04C5_outpatient_api_gap_retest.md` / `04C3_WEBクライアントAPI接続検証.md`。UI 巡回は未実施（ネットワーク復旧後に同 RUN_ID で tone/telemetry 取得予定）。
