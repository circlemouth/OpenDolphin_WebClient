...
> RUN_ID=`20251205T090000Z` で `web-client/src/libs/http/httpClient.ts` の `OUTPATIENT_API_ENDPOINTS` に `/api01rv2/patient/outpatient/*` を追加し、`web-client/src/libs/telemetry/telemetryClient.ts` と `web-client/src/features/charts/orchestration.ts` を実装、`OrderConsole` の `cacheHit`/`missingMaster` トグルから `funnels/outpatient` へ `resolve_master`/`charts_orchestration` telemetry を送り出したことを `docs/server-modernization/phase2/operations/logs/20251205T090000Z-integration-implementation.md` に記録し、DOC_STATUS と本チェックリストの Web クライアント UX/Features 行への RUN_ID 更新と doc へのリンク整備を完了した。
