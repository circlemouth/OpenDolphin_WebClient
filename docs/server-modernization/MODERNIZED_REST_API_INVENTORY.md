...
| --- | --- | --- | --- | --- | --- | --- |
| 2 | P0 | `/orca14/appointmodv2` | `POST /orca/appointments/mutation` | ◎ 実装完了 | RUN_ID=20251121PostOpenCheckZ1 実測（`docs/server-modernization/phase2/operations/logs/20251121PostOpenCheckZ1-api-contract.md#orca14appointmodv2`） | 予約登録/取消/新規患者番号割当。`AppointmentMutationRequest/Response` と `OrcaAppointmentMutationFacade` で `AppoServiceBean` を同期し、警告は SSE へ連携。2025-11-21 ORMaster (`http://100.102.17.40:8000`) は `HTTP 405 (Allow: OPTIONS, GET)` のまま（`20251121T153200Z` → `20251121PostOpenCheckZ1` で変化なし）。Evidence: `artifacts/orca-connectivity/20251121PostOpenCheckZ1/crud/appointmodv2/headers_*.txt`。 |
