...
| **Webクライアント互換ラッパー（独自）** | 削除済み（旧互換ラッパー） | 廃止済み | **廃止完了** |
...
### 4.1 移行対象の確定（即日）
* 互換ラッパー3本は削除済み（旧互換ラッパー）。
* ORCA公式パススルーと誤認しないよう、削除禁止ラインを維持する。
...
3. **請求系（要実装）**
   * 旧互換ラッパーのロジックを **`/orca/claim/outpatient/*`** に移植する。
   * `ClaimOutpatientResponse` のメタ項目（`runId`/`dataSource`/`claimBundles` 等）は維持。
4. **患者検索（ローカル）**
   * 旧互換ラッパーのローカル検索は **`/orca/patients/local-search`** に移植する。
...
| :--- | :--- | :--- | :--- |
| 予約一覧 | 削除済み | `/orca/appointments/list` | 既存実装あり |
| 予約登録 | 削除済み | `/orca/appointments/mutation` | 既存実装あり |
| 受付一覧 | `/api01rv2/visitptlstv2` | `/orca/visits/list` | ORCA公式は存続、Webクライアントは `/orca/visits/list` を使用 |
| 請求バンドル | 削除済み | `/orca/claim/outpatient/*` | **新規実装** |
| 患者検索（ローカル） | 削除済み | `/orca/patients/local-search` | **新規実装** |
