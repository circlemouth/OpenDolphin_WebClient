# ORCA Project における CLAIM 通信廃止（一次まとめ）

## 目的
ORCA 側での CLAIM 通信廃止方針を、本リポジトリ内で参照可能な形に整理する。

## 重要告知（原文整理）
**【重要】日医標準レセプトソフト CLAIM 通信機能廃止のお知らせ**
- 電子カルテ等システムとの連携で実装されている **CLAIM 通信機能は廃止予定**。
- **廃止日: 2026 年 3 月末**
- CLAIM 通信で実現していた機能は **API / PushAPI で提供**されているため、
  **CLAIM 利用中の場合は API へ切替が必要**。
- 廃止日以降は **即時停止しない**が、**不具合修正や機能追加は行わない**。
...
## 1. 日レセCLAIM通信について（原文整理）
### 1-1. 通信
- MedXML コンソーシアムが管理するデータ交換仕様 CLAIM を使用。
- 日レセとの接続は **TCP ソケット通信**（FTP 等は未実装）。
- 仕様バージョン:
  - MML: Version 2.3
...
### 1-2. 通信の流れ
- **受付時等**: 日レセ → 電子カルテ
  - socket open → データ送信 → EOT(0x04)
  - 応答: ACK(0x06) / NAK(0x15)
  - socket close
- **処方・病名**: 日レセ ← 電子カルテ
  - socket open ← データ送信 ← EOT(0x04)
  - 応答: ACK(0x06) / NAK(0x15)
...
### 1-3. 運用形態
1. **受付・患者登録**は日レセ側。
2. **病名・処方オーダ**は電子カルテ側から送信。
3. 日レセ側で会計処理を行い、
   **点数金額情報を電子カルテへ送信**（試用版）。
4. これ以外の双方向連携は非対応。外来のみ本運用対象。

### 1-4. 制限事項
- 予約請求モジュールの claim:bundle / claim:item は **max 20 件**。
- 診断履歴モジュールの受信は **1 度に 50 病名まで**（4.4.0 までは 20）。

### 1-5. 日レセとCLAIM通信を行うために
**日レセ 5.1 以前**
- `sudo dpkg-reconfigure jma-receipt`
- “jma-receipt 用に claim server をスタートしますか?” → Yes

**日レセ 5.2 以降**
- `sudo systemctl enable claim.service`
- `sudo systemctl start claim.service`
- ポート変更: `/etc/jma-receipt/jma-receipt.env` に `CLAIM_PORT=ポート番号`

---

## 2. 処理別送受信モジュール説明（原文整理）
以下、原文に沿って整理。詳細は付録の原文を参照。

### 2-1. 日レセ送信時
#### 2-1-1. 受付時
- 送信モジュール: 患者情報 / 健康保険情報 / 予約請求(初再診・メモ)
- 保険種別の mmlHi:ClassCode 設定などの詳細は原文参照。

#### 2-1-2. 患者登録
- 受付を行わない施設向け。
- 患者基本情報・保険情報を送信。
- claim:information の status を `info` で送信。

### 2-2. 日レセ受信時
- 診断履歴情報モジュール / 予約請求モジュール（処方）を取り込み。

#### 2-2-1. 共通項目
- 施設ID / 診療科 / 患者番号 の設定。

#### 2-2-2. 診断履歴情報モジュール
- 病名の追加・更新・削除ルール、転帰・入外区分の扱い等は原文参照。

#### 2-2-3. 予約請求モジュール
- ドクターコード / claim:information / claim:bundle の取込ルール、
  特殊コード・コメント・セットコードの扱いは原文参照。
...
- 入院版 CLAIM 規格案に基づく実装。
- 外来と同様の通信方式。
- 入院登録、転棟、外泊などのサンプルは原文参照。

---

## 4. 点数金額モジュール（参考提供）
- 自費項目 / 請求金額情報 / 保険情報の追加。
- claimA:classCode などの設定は原文参照。

---

## 5. 改定内容（原文整理）
- コメントコード対応、診療報酬改定対応など。
- 更新履歴は原文参照。

---

## 付録: 原文（ユーザー提供）
以下はユーザー提供の原文をそのまま保存したもの。
...
$ sudo systemctl enable claim.service
$ sudo systemctl start claim.service 
claimの待受ポートを変更する場合は/etc/jma-receipt/jma-receipt.envに以下のように'CLAIM_PORT=ポート番号'を追記してください。
...
(なお日レセver 4.0.0 以降では、医療機関IDが必須設定となりますので、CLIAMインスタンスのグループ診療対応 [PDF]をご確認ください。)
...
mmlHi:ClassCode保険の種類
Rx労災・自賠(x：該当の保険番号マスタの保険番号の3桁目)
Zx自費(xは同上)
Ax治験 90x(xは同上)　　(ver 4.5.0以降)
Bx治験 91x(xは同上)　　(ver 4.5.0以降)
K5公害
39後期高齢者
40後期特療費後期高齢者医療特別療養費()
09協会けんぽ
...
a.医療機関IDMmlHeaderの施設情報の施設ID(mmlCm:Id)に設定します。
b.診療科MmlHeaderの診療科情報の診療科ID(mmlCm:Id)に設定します。
c.患者番号MmlHeaderのmasterIdに設定します。
...
「胃炎」(疑いフラグ設定なし)の状態で"MML0015"suspectedDiagnosis を設定して送信すると「胃炎」に対して疑いフラグが設定されます。
また「胃炎」 疑いフラグ設定あり又は「胃炎の疑い」に対して"MML0015"suspectedDiagnosis を設定せずに送信すると「胃炎」 疑いフラグ設定なしの状態で設定されます。
主病名(mainDiagnosis)も同様の動作となります。
4.「の疑い」修飾語文字修飾語病名コード()の情報「の疑い」に関しては上記3と同様の動作となります。(付け外しが可能)
...
outpatient外来
inpatient入院
※　但し、日レセ側に、同一内容で入外それぞれに病名が存在している場合に空白を設定しても、対象の病名が確定できないため更新はしません。
...
<claim:item claim:subclassCodeId="Claim003" claim:code="P01022" claim:tableId="tbl_tensu">
...
設定サンプル
一般名指示の場合
<claim:item claim:subclassCodeId="Claim003" claim:code="612140711" claim:tableId="tbl_tensu">
...
入院版CLAIM規格(案)… http://www.medxml.net/ の入院版CLAIM規格(案)を参照
...
claimA:className「自費消費税あり()」 or 「自費(消費税なし)」
claimA:claimBundleRate合計金額を設定
claimA:code自費〜110の該当する番号を4桁で設定
claimA:name自費〜110の該当する名称(文書料等)を設定
claimA:claimRate自費〜110の該当する金額を設定
...
claimA:claimBundlePoint診察料等の合計点数
claimA:claimBundleRate伝票番号を設定
claimA:codeからの連番を14桁で設定
0001 〜 0010については「初・再診料」、「医学管理等」等を設定(これらについては、claimA:claimRateに各点数を設定)
0011 〜については「負担金額」等設定(これらについてはclaimA:claimRateに各金額を設定)
※訂正時にはclaimA:code="0018"に「訂正分請求金額」を設定し、claimA:claimRateに前回請求額との差分を+-で設定します。
...
平成30年4月診療報酬改定
2018年4月CLAIM改正対応 [PDF] (2018-04-03)
更新履歴
