...
- [ ] Micrometer メトリクス出力（`DatasourceMetricsRegistrar`, `RequestMetricsFilter`）の計測内容を収集し、Prometheus 連携可否を確認する（備考: `docs/server-modernization/phase2/notes/ops-observability-plan.md` に REST/JDBC/SSE/監査メトリクス一覧と `operations/OBSERVABILITY_AND_METRICS.md` 準拠の突合フローを追記済み。`docker compose` / Podman が利用できず `curl http://localhost:9080/actuator/{health,metrics,prometheus}` も失敗しているためメトリクス採取と Evidence 更新は未実施。コンテナ起動手段の復旧待ち）
- [ ] Trace ID が JMS / セッション層ログへ伝搬するか end-to-end で確認する（備考: `ops/tests/api-smoke-test/test_config.manual.csv` から `/serverinfo/version`,`/user/doctor1`,`/chart/WEB1001/summary` を代表 API に選定し、`docs/server-modernization/phase2/notes/ops-observability-plan.md` と `PHASE2_PROGRESS.md` に HTTP/JMS/Session の採取手順・保存先 (`artifacts/parity-manual/TRACEID_JMS/trace/`) を追記済み。`docker compose` 不在で `scripts/start_legacy_modernized.sh` が起動せず実ログ採取は未実施）
- [ ] REST 例外処理共通化（`AbstractResource`）をレビューし、レスポンスフォーマットとログ整合性を検証する（備考: `docs/server-modernization/phase2/notes/ops-observability-plan.md` に例外ハンドリング調査結果・ApiProblem 案・LogFilter 改善案を追記済み。`docker compose` / `mvn -pl server-modernized wildfly:run` が失敗しており 400/401/500 の実機検証・ログ採取はブロッカー解消待ち）

## フェーズ9: 回帰・API 同等性検証

- [ ] スモークテストスクリプト (`ops/tests/api-smoke-test/`) を用いて Legacy・Modernized のレスポンス比較を実施し、成果物ディレクトリを保存する（備考: `test_config.manual.csv` から優先 API を抽出したものの、`curl http://localhost:{8080,18080}/serverinfo/version` がいずれも Connection refused。`docker compose` 不在で `scripts/start_legacy_modernized.sh` が起動せず、実リクエストと成果物生成は未実施）
- [ ] API パリティチェッカー (`scripts/api_parity_response_check.py`) で代表 API の差分を測定し、結果を共有する（備考: `ops/tools/send_parallel_request.sh` に `--config` モードを追加し、`scripts/api_parity_targets.touch.json`＋`scripts/api_parity_payloads/*.json` で `tmp/parity-touch/20251107T130558Z/` に diff.txt を生成済み。実サーバー未起動のため HTTP モックによる差分のみで、本番環境での再取得が必要）
- [ ] 監査ログ・外部副作用の検証（例: `/20/adm/factor2/*` 実行後の `d_audit_event`）を含む総合回帰テストを設計する（備考: `docs/server-modernization/phase2/notes/test-data-inventory.md` に「監査ログ・外部副作用テスト設計」節を追加し、`artifacts/parity-manual/factor2_*` と `artifacts/manual/audit_log.txt` に接続失敗ログを保存済み。`server-modernized/tools/flyway/sql/V0001__baseline_tag.sql` が空で `d_audit_event` を作れず、Codespaces では Docker/DB が起動できないため実測取得はブロック中）
