...
- [ ] 主要サービス (`KarteServiceBean`, `PatientServiceBean` など) のトランザクション境界と JPQL をレビューし、Legacy との差異を網羅表にまとめる（備考: 2025-11-09 に `/patient/id/0000001`, `/karte/pid/0000001,2024-01-01`, `/schedule/pvt/2025-11-09`, `PUT /appo` を Legacy / Modernized 両方で採取。`artifacts/parity-manual/JPQL/20251109T201157Z/<service>/` と `domain-transaction-parity.md §3` に差分とリスク（患者データ未投入 / `PatientVisitModel`・`AppointmentModel` が persistence unit 未登録）を整理済み。`rest_error_scenarios.manual.csv` へ Schedule/Appo の再現ケースを追加。`/chart/WEB1001/summary` などカルテ/PVT 系は 1.3 系サンプルデータ投入後に継続。）
