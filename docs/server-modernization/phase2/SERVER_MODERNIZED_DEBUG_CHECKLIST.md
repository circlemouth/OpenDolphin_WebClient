...
- [ ] 主要サービス (`KarteServiceBean`, `PatientServiceBean` など) のトランザクション境界と JPQL をレビューし、Legacy との差異を網羅表にまとめる（備考: 2025-11-09 取得分を `artifacts/parity-manual/JPQL/20251109T201157Z/<service>/` に保存済み。2025-11-10 は `ops/tools/send_parallel_request.sh --profile compose` で再実行したが Docker Desktop 不在により `curl (7)` で失敗し、結果ログを `artifacts/parity-manual/JPQL/20251110T034844Z/README.md` と `*_request.log` に記録。`domain-transaction-parity.md §3.2` に Legacy/Modernized の TX/JPQL/ persistence 差分マトリクスを追加し、`rest_error_scenarios.manual.csv` へ `rest_error_chart_summary_seed_gap` を追記。次回は `persistence.xml` へ `PatientVisitModel` / `AppointmentModel` を登録 → `WEB1001` シード投入 → 同 README の手順で d_audit_event/Hibernate SQL を再採取する。）
...
- [ ] `ClaimSender` / `DiagnosisSender` / `MMLSender` の設定値差分とエラーハンドリングを確認する（2025-11-09 更新: `claimHelper.vm` を `server-modernized/src/main/resources/` へ同梱し、`ops/tools/send_parallel_request.sh`（ヘルパーコンテナ経由）で `PUT /20/adm/eht/sendClaim 20251109T201826Z_CLAIM` が 200 になることを確認。証跡は `artifacts/parity-manual/CLAIM_DIAGNOSIS_FIX/20251109T201846Z/claim_send/` と `logs/docker_logs_opendolphin-server-modernized-dev.txt:245-255`。一方で `POST /karte/diagnosis/claim` は `diseaseHelper.vm` 不在により JMS DLQ へ落ち（`logs/jms_DLQ_list-messages.txt`）、`d_diagnosis` には新規登録されない。`PUT /mml/send` は `payload`/`sha256` を返すところまで成功。`docs/server-modernization/phase2/notes/messaging-parity-check.md` §8 に HTTP/JMS/DB サマリと `MessagingHeaders` 図、テンプレ差分を追記済み。未対応タスク: (1) `diseaseHelper.vm` の WAR 取り込みと DLQ evac、(2) Diagnosis API 200→DB 書き込みの実測、(3) Legacy 403 の根本原因整理、(4) ホスト→9080 の疎通改善または恒久的な CLI 実行手順、(5) MmlSender Smoke Test（`tmp/mvn-mml.log`）の Gate #44 運用。）
...
- [ ] Trace ID が JMS / セッション層ログへ伝搬するか end-to-end で確認する（備考: `ops/tests/api-smoke-test/test_config.manual.csv` / `rest_error_scenarios.manual.csv` の trace-id 列に沿って 200/400/401/500 ケースをローカル合成ベースライン上で実施し、`artifacts/parity-manual/TRACEID_JMS/` に 4xx/5xx のログを追加する。`ops/tools/send_parallel_request.profile.env.sample` へ `modernized-dev` プロファイルを追加し、Docker ネットワーク内の `opendolphin-server(-modernized-dev)` 宛て CLI も共通化した。2025-11-10 は WSL 環境に Docker Desktop が入っておらず `docker ps` が失敗したため、`--profile modernized-dev` 実行時にホスト名解決ができず全ケース `curl: (6)` で終了（RUN_ID=`20251110T035118Z`, 証跡: `artifacts/parity-manual/TRACEID_JMS/20251110T035118Z/`). `TRACEID_JMS_RUNBOOK.md` を参照し、Docker/WSL 連携を有効化するか `compose` プロファイルで `localhost` URL を使って再実行する。）
