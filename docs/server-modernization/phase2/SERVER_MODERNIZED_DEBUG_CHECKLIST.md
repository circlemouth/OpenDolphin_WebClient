...
- [ ] Legacy/Modernized Postgres ベースライン復旧計画を Runbook 化し、DB 欠損が発生した際の Gate と証跡フローを整備する（備考: `docs/server-modernization/phase2/operations/POSTGRES_BASELINE_RESTORE.md` に Secrets/VPN 受領フローと 2025-11-08 実測ログを追記。`artifacts/parity-manual/db-restore/20251108T062436Z/` へ Compose/psql/flyway ログを保存し、`flyway` が `V0002__performance_indexes.sql` で `relation "appointment_model" does not exist` により停止することを証跡化。2025-11-08T07:43Z 再試行として `artifacts/parity-manual/db-restore/20251108T074337Z/` に Secrets 未受領ログ（`ls ~/Secrets` 失敗）と docker/psql/flyway 未実施の理由、Ops/DBA 依頼テンプレ参照先（`.../20251108T062436Z/baseline_search.log:6`）を記録済み。Secrets Storage からのベースライン dump が未配布のため Gate は未完で、(1) Ops/DBA から `legacy-server/db-baseline/*.sql` / `server-modernized/db-baseline/opendolphin-modern-schema.sql` を受領して `~/Secrets/` に配置、(2) 受領後に `psql -f` → `flyway baseline/migrate` を再実行し成功ログを `artifacts/parity-manual/db-restore/<新UTC>/` に残す、(3) Gate クローズ時に Runbook/Checklist/`PHASE2_PROGRESS.md`/`DOC_STATUS.md` の備考を成功済みに切り替える──の 3 手順を完了させてクローズする）
...
- [ ] `ClaimSender` / `DiagnosisSender` / `MMLSender` の設定値差分とエラーハンドリングを確認する（備考: WebORCA コンテナ (`docker/orca/jma-receipt-docker`) を `docs/server-modernization/phase2/operations/EXTERNAL_INTERFACE_COMPATIBILITY_RUNBOOK.md#44-weborca-テストコンテナ2025-11-08-追加` の手順で起動し、`ServerInfoResource` で到達性を確認したうえで CLI 経由の送信 API（`ops/tools/send_parallel_request.sh POST /claim/send` 等）を実行する。成功/失敗レスポンスと `server.log` を `artifacts/parity-manual/CLAIM_*` へ保存し、`ops/shared/docker/custom.properties`（`claim.host=orca`, `claim.send.port=8000`, `claim.send.encoding=MS932`）と `docs/server-modernization/phase2/domains/RESERVATION_BATCH_MIGRATION_NOTES.md` のエラーコード表を突き合わせる）
