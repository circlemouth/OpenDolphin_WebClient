...
- **テストエントリ**: `server-modernized/src/test/java/open/dolphin/adm/AdmConverterSnapshotTest.java` を追加し、`IPatientModel` (touch/adm10/adm20) の JSON を `ops/tests/fixtures/adm/admxx/patient_model.json` に固定した（`tmp/legacy-fixtures/admxx/*.json` には stub だけを残し、新パスを案内）。`touch` 版の JSON をレガシー基準とし、ADM 系 DTO のマッピング漏れを自動検出する。
- **再現コマンド**: `mvn -f server-modernized/pom.xml test -Dtest=AdmConverterSnapshotTest -DskipITs=false`。フィクスチャを更新したい場合のみ `-Dadm.snapshot.update=true` を付与し、生成された JSON をレビューしてからコミットする（Surefire には `adm.snapshot.fixtureDir=${project.basedir}/../ops/tests/fixtures/adm` を渡している）。
- **サンプル payload**: `ops/tests/fixtures/adm/adm10/patient_model.json` / `ops/tests/fixtures/adm/adm20/patient_model.json` に `reserve1`〜`reserve6` と `healthInsurances[*].publicItems[*]` を含む JSON を保存。`reserve` 系フィールドの一例を抜粋。
...
- **2025-11-08 追記 (VisitPackage/Labo/Diagnosis)**: `AdmConverterSnapshotTest` へ `visit_package` / `labo_item` / `registered_diagnosis` の 3 シナリオを追加し、現在は `ops/tests/fixtures/adm/adm10|adm20/<scenario>.json` に `IVisitPackage` / `ILaboGraphItem` / `IRegisteredDiagnosis` の touch 出力を保存している。`patientMemo` にはダミー `karteBean`/`userModel` を埋め込み、病名・検査項目は 2 件ずつで差分検出しやすい構成にしてある。
...
- **2025-11-11 再検証**: RUN_ID=`20251111T161746Z`。`mvn -f reporting/pom.xml install -DskipTests` で `opendolphin-reporting` JAR をローカルに配置したうえで、`mvn -f server-modernized/pom.xml test -Dtest=AdmConverterSnapshotTest -DskipITs -Dsurefire.failIfNoSpecifiedTests=false` を実行。`patient_model`/`visit_package`/`labo_item`/`registered_diagnosis` の 4 シナリオはいずれも `adm.snapshot.update=false` のままグリーンで、`ops/tests/fixtures/adm/adm10|adm20/*.json` に変更は発生せず、新規 `artifacts/parity-manual/adm-snapshots/` も未生成。Surefire ログは `server-modernized/target/surefire-reports/open.dolphin.adm.AdmConverterSnapshotTest.txt` および `.xml` に保存されている。
- **2025-11-12 定常実行**: RUN_ID=`20251116TadmSnapshotZ1`。`RUN_ID` を環境変数にセットしたうえで `mvn -f server-modernized/pom.xml test -Dtest=AdmConverterSnapshotTest -DskipITs=false` を実行し、`ops/tests/fixtures/adm/adm{10,20}/*.json` を Git 管理で参照している状態でも差分無しで通過することを確認。Surefire 証跡は `server-modernized/target/surefire-reports/open.dolphin.adm.AdmConverterSnapshotTest.{txt,xml}` に、実行メモは `artifacts/parity-manual/adm-snapshots/20251116TadmSnapshotZ1/README.md` に保存した。差分ディレクトリの生成は無く `adm.snapshot.update=false` のまま据え置き。
- **CI 取り込み設計（2025-11-11）**:
  1. **必須ステップ**: CI でも `mvn -f reporting/pom.xml install -DskipTests` → `mvn -f server-modernized/pom.xml test -Dtest=AdmConverterSnapshotTest -DskipITs=false` を順に実行し、`reporting` JAR 依存を解決する。`ADM_SNAPSHOT_RUN_ID=$(date -u +%Y%m%dT%H%M%SZ)` を事前にエクスポートし、ジョブ全体の RUN_ID として利用する。
  2. **スナップショット保存案**: テストは差分発生時のみ `artifacts/parity-manual/adm-snapshots/<timestamp>/scenario/target/` を自動生成するため、CI 側では `ARTIFACT_UPLOAD_DIR=artifacts/parity-manual/adm-snapshots/${ADM_SNAPSHOT_RUN_ID}` を用意し、`if compgen -G "artifacts/parity-manual/adm-snapshots/20??????????????Z"` で出力ディレクトリを検出したうえで `mv artifacts/parity-manual/adm-snapshots/20* ${ARTIFACT_UPLOAD_DIR}` → アーティファクト保存（例: CI の `upload-artifact` ステップ）を行う。差分が無い場合も `server-modernized/target/surefire-reports/open.dolphin.adm.AdmConverterSnapshotTest.{txt,xml}` を `adm-snapshot-ci-${ADM_SNAPSHOT_RUN_ID}` で保存し、実行証跡とする（フィクスチャ本体は `ops/tests/fixtures/adm/` にあるため Git 追跡のみで十分）。
...
- **フィクスチャ管理方針メモ（2025-11-12 時点）**:
  - サイズ: `ops/tests/fixtures/adm/adm10` / `adm20` は各 20 KB 程度で、4 つの JSON（patient_model / visit_package / labo_item / registered_diagnosis）を保持。`tmp/legacy-fixtures/adm10|adm20/*.json` は Stub（`movedTo` メッセージ）であり、旧パス参照の Runbook から新ディレクトリを案内する。
  - 更新頻度: 2025-11-08 のスナップショット追加以降、11-12 定常実行まで差分なし。今後も ADM DTO 追加時や `adm.snapshot.update=true` 実行時のみ更新が発生する想定。
  - 再生成コマンド: `mvn -f reporting/pom.xml install -DskipTests` → `mvn -f server-modernized/pom.xml test -Dtest=AdmConverterSnapshotTest#<scenario> -Dadm.snapshot.update=true`（シナリオ別実行可）。CI 常時実行時は `-Dadm.snapshot.update=false` かつ `adm.snapshot.fixtureDir=${project.basedir}/../ops/tests/fixtures/adm` で固定し、差分検出のみを目的とする。
  - 選択肢: (A) Git 管理に昇格して CI/ローカル双方で常に同一 JSON を参照する（今回実施）、(B) Runbook 化して `adm.snapshot.update=true` 実行結果を CI の前段で取得する。現状は (A) を採用し、追加資料は `docs/web-client/planning/phase2/DOC_STATUS.md` / 本節へ逐次反映する。
