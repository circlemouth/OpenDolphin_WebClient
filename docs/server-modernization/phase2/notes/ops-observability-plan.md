...
| SYSAD header 欠落（Compose health check） | Blackbox exporter プローブ `probe_success{job="sysad-actuator"}` を `/actuator/{health,prometheus}`（ヘッダー: `ops/tests/api-smoke-test/headers/sysad-actuator.headers`）で実行し、`==0` が 2 回続いたら `ALERT sysad_actuator_probe_failure` を発火。 | `modernized_wildfly::actuator_probe`。`probe_success` とレスポンスタイムを並べ、PagerDuty 連携のトリガー値を表示。 | PagerDuty サービス `Modernized-WildFly`（Ops L1 初動 → Ops L2 15 分以内にヘルス差分確認）。 | 正常・異常系 `curl` は `artifacts/parity-manual/secrets/20251108T204806Z/{baseline,missing_sysad}/curl_actuator_*.log` に保存。Compose 側の既定ヘッダーが実際にはブランクを許容しないため、プローブ経路で検出する。 |
