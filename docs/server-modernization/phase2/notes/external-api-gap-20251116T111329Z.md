...
- **2025-11-16 RUN_ID=`20251116T210500Z-E2` 実測結果**:
  - `docs/server-modernization/phase2/operations/logs/20251116T210500Z-E2-appointmod.md` / `...-acceptmod.md` に Trial (`curl -u <MASKED>:<MASKED> .../orca14/appointmodv2`, `/orca11/acceptmodv2`) の 405 応答を記録。`artifacts/orca-connectivity/20251116T210500Z-E2/appointmodv2/trial/` と `.../acceptmodv2/trial/` にヘッダー・body・curl verbose を保存。
...
- **採取手順**:
  1. Trial/ORMaster 共通で `curl -u <MASKED>:<MASKED> https://weborca-trial.orca.med.or.jp/mml/letter/list` を実行し、`{"Code":405}` を Blocker 証跡として `docs/server-modernization/phase2/operations/logs/20251116T111329Z-external-api-gap.md` に追記済み。
