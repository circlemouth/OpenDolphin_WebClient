...
- **採取手順**:
  ...
  3. ORMaster 接続が許可されたタイミングで `curl -u <ormaster> https://<host>/mml/letter/json/{pk}` など 4 エンドポイントを取得し、Legacy との差分を `diff -u legacy.json modern.json` で生成。結果は `artifacts/external-interface/mml/<RUN_ID>/{letter_json,labtest_json}` へ格納。  
  4. 取得完了後に `STAMP_LETTER_MML_ORCA_ALIGNMENT_PLAN.md` / `API_PARITY_MATRIX.md` の 4 行を `[x]/◎` へ更新し、`DOC_STATUS` W22 備考を `[証跡取得済]` に切り替える。
