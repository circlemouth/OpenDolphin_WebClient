...
- 共通: `curl -u user:pass -H 'Content-Type: application/xml; charset=UTF-8' -X POST --data-binary @docs/server-modernization/phase2/operations/assets/orca-api-requests/44_system01dailyv2_request.xml '<URL>/api/api01rv2/system01dailyv2?class=00'` を実行すると HTTP 200 (`Api_Result=00`) となる。Shift_JIS 送信では `Api_Result=91`。Evidence: `artifacts/orca-connectivity/20251119TorcaTrialCrudZ1/httpdump/system01dailyv2/` を取得予定。
