...
## 0. 運用前提（2025-11-19 更新）
- ORCA 連携検証は WebORCA トライアルサーバー（`https://weborca-trial.orca.med.or.jp/`、Basic 認証 `trial/weborcatrial`）のみを対象とし、ローカル WebORCA コンテナや `curl --cert-type P12` は使用しない。Trial 環境では「新規登録／更新／削除 OK（トライアル環境でのみ）」として CRUD 実測を行う。
- `assets/seeds/*.sql` や本メモで言及する「seed」は **参考アーカイブ** であり、フィールド条件の確認にだけ用いる。Trial でデータが欠落している場合は API/ORCA UI を使って CRUD を行い、`docs/server-modernization/phase2/operations/logs/2025-11-19-orca-trial-crud.md` へ記録する。
...
- 共通: `curl -u trial:weborcatrial -H 'Content-Type: application/xml; charset=UTF-8' -X POST --data-binary @docs/server-modernization/phase2/operations/assets/orca-api-requests/44_system01dailyv2_request.xml 'https://weborca-trial.orca.med.or.jp/api/api01rv2/system01dailyv2?class=00'` を実行すると HTTP 200 (`Api_Result=00`) となる。Shift_JIS 送信では `Api_Result=91`。Evidence: `artifacts/orca-connectivity/20251119TorcaTrialCrudZ1/httpdump/system01dailyv2/` を取得予定。
...
- **push-exchanger（PUSH/帳票 API）設定**
  ...
  - メモ: `:use_weborca: true` と `:api_server=weborca-trial.orca.med.or.jp` を指定し、Basic 認証 `trial/weborcatrial` を `.env` に設定する。Trial 切替後は PKCS#12 を使わず、`curl -u trial:weborcatrial` → TLS ハンドシェイク（`openssl s_client weborca-trial.orca.med.or.jp:443`）の順で確認する。
...
> **2025-11-19（RUN_ID=`20251119TorcaTrialCrudZ1` 着手）:** WebORCA トライアルサーバーへ接続を切り替え、病棟/食事/ADL/ユーザー権限は Trial UI から直接 CRUD する前提に変更した。`assets/seeds/api21_medical_seed.sql` や `operations/assets/seeds/inpatient_master_seed_20251114.md` はコード確認用のアーカイブとして残し、実データは Trial で再作成する。以降の RUN_ID は `curl -u trial:weborcatrial` で採取し、`artifacts/orca-connectivity/<RUN_ID>/trial/` に POST/DELETE 証跡を保存する。
...
- **RUN_ID / 次アクション:** `RUN_ID=20251119TorcaTrialCrudZ1` で Trial 向け UTF-8 XML（`<data><private_objects>`）を `curl -u trial:weborcatrial 'https://weborca-trial.orca.med.or.jp/api01rv2/hsconfbasev2?class=01'` へ送信する。レスポンスと `Api_Result` を `artifacts/orca-connectivity/20251119TorcaTrialCrudZ1/httpdump/19_hsconfbasev2/` に保存し、Trial CRUD ログにも記載する。
- **XML/Trial 参考:** `operations/assets/orca-api-requests/xml/19_hsconfbasev2_request.xml`（UTF-8）は seed の値域説明として参照し、実データの `Base_Date` は Trial UI の施設開始日に合わせて登録する。証跡は `artifacts/orca-connectivity/<RUN_ID>/inpatient/19_hsconfbasev2/` へ格納する。
...
- **RUN_ID / 次アクション:** `RUN_ID=20251119TorcaTrialCrudZ1` で Trial へ `Request_Number=1/2` の XML を送信し、病棟/病室を新規作成→取得→削除まで一連で確認する。`curl -u trial:weborcatrial 'https://weborca-trial.orca.med.or.jp/api01rv2/hsconfwardv2?class=01'` を利用し、`trial/20_hsconfwardv2/` に CRUD 証跡を保存する。
- **XML/Trial 参考:** `operations/assets/orca-api-requests/xml/20_hsconfwardv2_request.xml` はコード例の記録として参照し、`Ward_Number`/`Room_Number` は Trial UI の病棟マスタ画面で登録した値を使用する。証跡は `artifacts/orca-connectivity/<RUN_ID>/inpatient/20_hsconfwardv2/` にまとめる。
...
- **RUN_ID / 次アクション:** `RUN_ID=20251119TorcaTrialCrudZ2` で `InOut=1/2` の XML を Trial へ送信し、カルテ一覧パネルと API レスポンスの差分を比較する。レスポンスは `artifacts/orca-connectivity/20251119TorcaTrialCrudZ2/httpdump/21_tmedicalgetv2/` に保存する。
- **XML/Trial 参考:** `operations/assets/orca-api-requests/xml/21_tmedicalgetv2_request.xml` はパラメータ例として参照し、`Patient_ID`/`Department_Code` は Trial で登録した患者・診療科の値を使用する。旧 seed 値は参考のみとし、Trial で CRUD した履歴を `docs/server-modernization/phase2/operations/logs/2025-11-19-orca-trial-crud.md` へ残す。
...
- **RUN_ID / 次アクション:** `RUN_ID=20251119TorcaTrialCrudZ2` で 2 パターン（保険者番号のみ／保険番号のみ）を Trial へ送信し、`curl -u trial:weborcatrial 'https://weborca-trial.orca.med.or.jp/api01rv2/insprogetv2?class=01'` のレスポンスを取得する。結果は `httpdump/22_insprogetv2/` へ保存し、`DOC_STATUS` と同期。
- **XML/Trial 参考:** `operations/assets/orca-api-requests/xml/22_insprogetv2_request.xml` のコードは seed の参考値とし、実際の `InsuranceProvider_Number` は Trial UI で登録した番号を使用する。証跡は `artifacts/orca-connectivity/<RUN_ID>/inpatient/22_insprogetv2/` にまとめる。
...
- **RUN_ID / 次アクション:** `RUN_ID=20251119TorcaTrialCrudZ3` で Trial 入院患者を対象に `Perform_Month` を指定した XML を送信し、ベッド画面との突合情報を `docs/server-modernization/phase2/operations/logs/2025-11-19-orca-trial-crud.md` へ追記する。
- **XML/Trial 参考:** `operations/assets/orca-api-requests/xml/23_hsmealv2_request.xml` はテンプレ参照用。`Patient_ID`/`保険組合せ`/`Meal_Type` は Trial UI で登録した値を使用し、旧 seed 値は参照のみ。レスポンスは `artifacts/orca-connectivity/<RUN_ID>/inpatient/23_hsmealv2/` に保存する。
...
- **RUN_ID / 次アクション:** `RUN_ID=20251119TorcaTrialCrudZ3` で ADL 登録済み患者を Trial UI から作成し、XML を送信して `hsptevalmodv2` との往復試験を行う。レスポンスは `httpdump/24_hsptevalv2/` へ保存する。
- **XML/Trial 参考:** `operations/assets/orca-api-requests/xml/24_hsptevalv2_request.xml` はコード表確認用。ADL/医療区分コードは Trial UI（入院基本設計書の Appendix を参照し再登録）で投入し、証跡は `artifacts/orca-connectivity/<RUN_ID>/inpatient/24_hsptevalv2/` にまとめる。
...
- **RUN_ID / 次アクション:** `RUN_ID=20251119TorcaTrialCrudZ4` で Trial 入院患者（転棟/他院歴あり）を登録し、XML を取得して `Ward_Number`/`Room_Number` と UI の一致を確認する。
- **XML/Trial 参考:** `operations/assets/orca-api-requests/xml/25_hsptinfv2_request.xml` はテンプレ。`Patient_ID`・`Admission_Date`・病棟コードは Trial UI で登録した値を使い、seed 値は参照のみ。証跡は `artifacts/orca-connectivity/<RUN_ID>/inpatient/25_hsptinfv2/` へ保存する。
...
- **RUN_ID / 次アクション:** `RUN_ID=20251119TorcaTrialCrudZ4` で Trial 入院患者の退院予定日を指定し、`curl -u trial:weborcatrial 'https://weborca-trial.orca.med.or.jp/api01rv2/hsacsimulatev2?class=01'` を送信して `hsacctmodv2` の会計結果と比較する。
- **XML/Trial 参考:** `operations/assets/orca-api-requests/xml/26_hsacsimulatev2_request.xml` は入力例。退院日／保険／食事設定は Trial UI で登録した値を利用し、seed 値は参照のみ。証跡は `artifacts/orca-connectivity/<RUN_ID>/inpatient/26_hsacsimulatev2/` に保存する。
...
- **RUN_ID / 次アクション:** 未実測（Trial 再測定キュー: `RUN_ID=20251119TorcaTrialCrudZ#`。`curl -u trial:weborcatrial` で取得予定）。月次パターン（`Perform_Month`）と年次パターン（`Perform_Year`）を切り替えて XML 投稿し、会計 UI と金額突合を行う。
- **XML/Trial 参考:** `operations/assets/orca-api-requests/xml/27_incomeinfv2_request.xml` は月次指定のみを送るテンプレ。収納 seed は `assets/orca-tec-index/raw/index.md` にリンクされる「収納登録（orca_bd_rct_060.pdf）」のコード表、または ORCA UI で現金収納を登録してから `artifacts/orca-connectivity/<RUN_ID>/inpatient/27_incomeinfv2/` へ保存する。
...
- **RUN_ID / 次アクション:** 未実測（Trial 再測定キュー: `RUN_ID=20251119TorcaTrialCrudZ#`。`curl -u trial:weborcatrial` で取得予定）。`ServerInfoResource` の時刻と合わせた XML を送信し、Legacy/Modernized 間の差分を記録する。
- **XML/Trial 参考:** `operations/assets/orca-api-requests/xml/28_systeminfv2_request.xml` を `ServerInfoResource` 取得直後（±30 分以内の `Request_Date/Time`）で送信。結果は `artifacts/orca-connectivity/<RUN_ID>/inpatient/28_systeminfv2/` に保存し、`ServerInfoResource` ログと突合する。
...
- **RUN_ID / 次アクション:** 未実測（Trial 再測定キュー: `RUN_ID=20251119TorcaTrialCrudZ#`。`curl -u trial:weborcatrial` で取得予定）。公式コードに合わせた XML を作成し、`logs/ORCA_HTTP_404405_HANDBOOK.md` 手順で 405 証跡→サポート問い合わせ用のテンプレを整備する。
- **XML/Trial 参考:** `operations/assets/orca-api-requests/xml/29_hsptinfmodv2_request.xml` は入院登録（仮）を想定。`Doctor_Code` や保険 seed は `assets/seeds/api21_medical_seed.sql` と同一患者を流用し、病棟/室料コードは `hsconfwardv2` で登録した ID を使う。405 取得時も `artifacts/orca-connectivity/<RUN_ID>/inpatient/29_hsptinfmodv2/` に `request.http`/`response.http` を残す。
...
- **RUN_ID / 次アクション:** 未実測（Trial 再測定キュー: `RUN_ID=20251119TorcaTrialCrudZ#`。`curl -u trial:weborcatrial` で取得予定）。用途毎に XML へ変換し、`/orca31` POST 405 を `ORCA_HTTP_404405` テンプレで採取する。用途の違いは `orca-api-matrix` の備考で整理する。
- **XML/Trial 参考:** `operations/assets/orca-api-requests/xml/30_hsacctmodv2_meal_request.xml`（Request_Number=食事）と `xml/38_hsacctmodv2_create_request.xml`（会計作成）を維持。食事/外泊区分コードは `raw/hospshokuji.md` の `Meal_Time`/`Meal_Type` 一覧を参照し、患者 seed は `api21_medical_seed.sql` を使う。405 証跡・エラー応答を `artifacts/orca-connectivity/<RUN_ID>/inpatient/30_hsacctmodv2/` / `38_hsacctmodv2/` に格納する。
...
- **RUN_ID / 次アクション:** 未実測（Trial 再測定キュー: `RUN_ID=20251119TorcaTrialCrudZ#`。`curl -u trial:weborcatrial` で取得予定）。`hsptevalv2` で取得した最新値を XML に落とし込み、`/orca32` POST 405 の証跡と併せて ORCA へルート公開を依頼する。
- **XML/Trial 参考:** `operations/assets/orca-api-requests/xml/31_hsptevalmodv2_request.xml` を `Medical_Condition`/`ADL_Score` CSV 付きで利用。値は `hsptevalv2` 応答に合わせ、患者 seed は `api21_medical_seed.sql` の `Patient_ID=000019` を流用。405 応答は `artifacts/orca-connectivity/<RUN_ID>/inpatient/31_hsptevalmodv2/` で保管する。
...
- **RUN_ID / 次アクション:** 未実測（Trial 再測定キュー: `RUN_ID=20251119TorcaTrialCrudZ#`。`curl -u trial:weborcatrial` で取得予定）。ユーザー一覧（01）と登録（02）の XML テンプレを作り、`/orca101` POST 405 をログ化→管理者 seed 状況を `DOC_STATUS.md` で追跡する。
- **XML/Trial 参考:** `operations/assets/orca-api-requests/xml/32_manageusersv2_request.xml`（Request_Number=02）を利用し、`Menu_Item_Number` などは `raw/userkanri.md` のコード表で補完する。証跡は `artifacts/orca-connectivity/<RUN_ID>/inpatient/32_manageusersv2/` へ保存し、ユーザー seed は `assets/orca-tec-index/raw/index.md` でリンクされる「患者登録」/「ユーザー管理」資料に従い ORCA UI で投入する。
...
- **RUN_ID / 次アクション:** 未実測（Trial 再測定キュー: `RUN_ID=20251119TorcaTrialCrudZ#`。`curl -u trial:weborcatrial` で取得予定）。`ModuleListConverter` と突合できるよう XML に変換し、`/orca21` POST 405 を採取する。
- **XML/Trial 参考:** `operations/assets/orca-api-requests/xml/33_medicalsetv2_request.xml`（Request_Number=01）を使用。`Medical_Class`・`Medication_Code` は `raw/setcode.md` / `assets/orca-tec-index/raw/index.md` の診療行為資料を参照し、405 応答を `artifacts/orca-connectivity/<RUN_ID>/inpatient/33_medicalsetv2/` に残す。
...
- **RUN_ID / 次アクション:** 未実測（Trial 再測定キュー: `RUN_ID=20251119TorcaTrialCrudZ#`。`curl -u trial:weborcatrial` で取得予定）。照会/登録の 2 ステップを XML に起こし、`/orca31` POST 405 を `ORCA_HTTP_404405` 手順で取得する。
- **XML/Trial 参考:** `operations/assets/orca-api-requests/xml/34_birthdeliveryv2_request.xml`（Request_Number=02）を使い、患者/保険 seed は `assets/seeds/api21_medical_seed.sql` の `Patient_ID=000020` を流用。入院費用・出産区分は `raw/childbirth.md` のコードで整備し、405 証跡を `artifacts/orca-connectivity/<RUN_ID>/inpatient/34_birthdeliveryv2/` に保管する。
...
- **RUN_ID / 次アクション:** 未実測（Trial 再測定キュー: `RUN_ID=20251119TorcaTrialCrudZ#`。`curl -u trial:weborcatrial` で取得予定）。XML 化と 200 件超えテストを行い、患者登録画面の保険タブとレスポンスを突合させる。
- **XML/Trial 参考:** `operations/assets/orca-api-requests/xml/35_patientlst6v2_request.xml` を `Reqest_Number=01` で利用し、患者/保険 seed は `api21_medical_seed.sql` の `tbl_ptnum` / `tbl_pthkninf` を適用。応答は `artifacts/orca-connectivity/<RUN_ID>/inpatient/35_patientlst6v2/` で保管する。
...
- **RUN_ID / 次アクション:** 未実測（Trial 再測定キュー: `RUN_ID=20251119TorcaTrialCrudZ#`。`curl -u trial:weborcatrial` で取得予定）。ORCA 側 405 ルートのため XML へ直したうえで `ORCA_HTTP_404405` テンプレに沿った証跡を残し、スタンプ連携と整合させる。
- **XML/Trial 参考:** `operations/assets/orca-api-requests/xml/36_diseasev2_request.xml` で `Disease_Code=8830040` を送信。病名コード seed は `assets/orca-tec-index/raw/index.md` の「病名登録（orca_bd_nds_050.pdf）」や `raw/diseasemod.md` を参照し、405 証跡を `artifacts/orca-connectivity/<RUN_ID>/inpatient/36_diseasev2/` へ保存する。
...
- **RUN_ID / 次アクション:** 未実測（Trial 再測定キュー: `RUN_ID=20251119TorcaTrialCrudZ#`。`curl -u trial:weborcatrial` で取得予定）。`diseasev2` と同様 XML 化＋405 証跡→サポートエスカレーションを行い、v2/v3 の差分を Runbook に追記する。
- **XML/Trial 参考:** `operations/assets/orca-api-requests/xml/37_diseasev3_request.xml` で `Disease_Supplement_Name` など v3 専用項目を埋める。コード表は `raw/diseasemod2.md`、患者 seed は `api21_medical_seed.sql` を参照し、405 応答を `artifacts/orca-connectivity/<RUN_ID>/inpatient/37_diseasev3/` へ。
...
- **RUN_ID / 次アクション:** 未実測（Trial 再測定キュー: `RUN_ID=20251119TorcaTrialCrudZ#`。`curl -u trial:weborcatrial` で取得予定）。`Request_Number=9` の XML を用意し、`/orca31` POST 405 のログを取得→既存会計と突合する準備を進める。
- **XML/Trial 参考:** `operations/assets/orca-api-requests/xml/38_hsacctmodv2_create_request.xml` を `Request_Number=9` 想定のテンプレとして使用。食事/室料 seed は `raw/hosp_kaikeimod.md` のコード、患者/保険 seed は `api21_medical_seed.sql` を参照し、405 証跡を `artifacts/orca-connectivity/<RUN_ID>/inpatient/38_hsacctmodv2/` に保存する。
