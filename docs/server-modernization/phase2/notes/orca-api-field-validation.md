...
- ORCA 連携検証は開発用 ORCA サーバー（`mac-dev-login.local.md` 参照）のみを対象とし、ローカル WebORCA コンテナや `curl --cert-type P12` は使用しない。開発環境では「新規登録／更新／削除 OK」として CRUD 実測を行う。
...
- 共通: `curl -u user:pass -H 'Content-Type: application/xml; charset=UTF-8' -X POST --data-binary @docs/server-modernization/phase2/operations/assets/orca-api-requests/xml/44_system01dailyv2_request.xml '<URL>/api/api01rv2/system01dailyv2'` を実行すると HTTP 200 (`Api_Result=00`) となる。Shift_JIS 送信では `Api_Result=91`。Evidence: `artifacts/orca-connectivity/20251119TorcaTrialCrudZ1/httpdump/system01dailyv2/` を取得予定。
...
- **push-exchanger（PUSH/帳票 API）設定**
  ...
  - メモ: `:use_weborca: true` と `:api_server=<URL>` を指定し、Basic 認証 `user/pass` を `.env` に設定する。Trial 切替後は PKCS#12 を使わず、`curl -u user:pass` → TLS ハンドシェイク（`openssl s_client <HOST>:<PORT>`）の順で確認する。
...
> **2025-11-19（RUN_ID=`20251119TorcaTrialCrudZ1` 着手）:** 開発用 ORCA サーバーへ接続を切り替え、病棟/食事/ADL/ユーザー権限は Trial UI から直接 CRUD する前提に変更した。`assets/seeds/api21_medical_seed.sql` や `operations/assets/seeds/inpatient_master_seed_20251114.md` はコード確認用のアーカイブとして残し、実データは開発環境で再作成する。以降の RUN_ID は `curl -u user:pass` で採取し、`artifacts/orca-connectivity/<RUN_ID>/trial/` に POST/DELETE 証跡を保存する。
...
- **RUN_ID / 次アクション:** `RUN_ID=20251119TorcaTrialCrudZ1` で開発環境向け UTF-8 XML（`<data><private_objects>`）を `curl -u user:pass '<URL>/api01rv2/hsconfbasev2?class=01'` へ送信する。レスポンスと `Api_Result` を `artifacts/orca-connectivity/20251119TorcaTrialCrudZ1/httpdump/19_hsconfbasev2/` に保存し、CRUD ログにも記載する。
...
- **RUN_ID / 次アクション:** `RUN_ID=20251119TorcaTrialCrudZ1` で開発環境へ `Request_Number=1/2` の XML を送信し、病棟/病室を新規作成→取得→削除まで一連で確認する。`curl -u user:pass '<URL>/api01rv2/hsconfwardv2?class=01'` を利用し、`trial/20_hsconfwardv2/` に CRUD 証跡を保存する。
...
- **RUN_ID / 次アクション:** `RUN_ID=20251119TorcaTrialCrudZ2` で 2 パターン（保険者番号のみ／保険番号のみ）を開発環境へ送信し、`curl -u user:pass '<URL>/api01rv2/insprogetv2?class=01'` のレスポンスを取得する。結果は `httpdump/22_insprogetv2/` へ保存し、`DOC_STATUS` と同期。
...
- **RUN_ID / 次アクション:** `RUN_ID=20251119TorcaTrialCrudZ4` で入院患者の退院予定日を指定し、`curl -u user:pass '<URL>/api01rv2/hsacsimulatev2?class=01'` を送信して `hsacctmodv2` の会計結果と比較する。
...
- **RUN_ID / 次アクション:** 未実測（再測定キュー: `RUN_ID=20251119TorcaTrialCrudZ#`。`curl -u user:pass` で取得予定）。月次パターン（`Perform_Month`）と年次パターン（`Perform_Year`）を切り替えて XML 投稿し、会計 UI と金額突合を行う。
...
- **RUN_ID / 次アクション:** 未実測（再測定キュー: `RUN_ID=20251119TorcaTrialCrudZ#`。`curl -u user:pass` で取得予定）。`ServerInfoResource` の時刻と合わせた XML を送信し、Legacy/Modernized 間の差分を記録する。
...
- **RUN_ID / 次アクション:** 未実測（再測定キュー: `RUN_ID=20251119TorcaTrialCrudZ#`。`curl -u user:pass` で取得予定）。公式コードに合わせた XML を作成し、`logs/ORCA_HTTP_404405_HANDBOOK.md` 手順で 405 証跡→サポート問い合わせ用のテンプレを整備する。
...
- **RUN_ID / 次アクション:** 未実測（再測定キュー: `RUN_ID=20251119TorcaTrialCrudZ#`。`curl -u user:pass` で取得予定）。用途毎に XML へ変換し、`/orca31` POST 405 を `ORCA_HTTP_404405` テンプレで採取する。用途の違いは `orca-api-matrix` の備考で整理する。
...
- **RUN_ID / 次アクション:** 未実測（再測定キュー: `RUN_ID=20251119TorcaTrialCrudZ#`。`curl -u user:pass` で取得予定）。`hsptevalv2` で取得した最新値を XML に落とし込み、`/orca32` POST 405 の証跡と併せて ORCA へルート公開を依頼する。
...
- **RUN_ID / 次アクション:** 未実測（再測定キュー: `RUN_ID=20251119TorcaTrialCrudZ#`。`curl -u user:pass` で取得予定）。ユーザー一覧（01）と登録（02）の XML テンプレを作り、`/orca101` POST 405 をログ化→管理者 seed 状況を `DOC_STATUS.md` で追跡する。
...
- **RUN_ID / 次アクション:** 未実測（再測定キュー: `RUN_ID=20251119TorcaTrialCrudZ#`。`curl -u user:pass` で取得予定）。`ModuleListConverter` と突合できるよう XML に変換し、`/orca21` POST 405 を採取する。
...
- **RUN_ID / 次アクション:** 未実測（再測定キュー: `RUN_ID=20251119TorcaTrialCrudZ#`。`curl -u user:pass` で取得予定）。照会/登録の 2 ステップを XML に起こし、`/orca31` POST 405 を `ORCA_HTTP_404405` 手順で取得する。
...
- **RUN_ID / 次アクション:** 未実測（再測定キュー: `RUN_ID=20251119TorcaTrialCrudZ#`。`curl -u user:pass` で取得予定）。XML 化と 200 件超えテストを行い、患者登録画面の保険タブとレスポンスを突合させる。
...
- **RUN_ID / 次アクション:** 未実測（再測定キュー: `RUN_ID=20251119TorcaTrialCrudZ#`。`curl -u user:pass` で取得予定）。ORCA 側 405 ルートのため XML へ直したうえで `ORCA_HTTP_404405` テンプレに沿った証跡を残し、スタンプ連携と整合させる。
...
- **RUN_ID / 次アクション:** 未実測（再測定キュー: `RUN_ID=20251119TorcaTrialCrudZ#`。`curl -u user:pass` で取得予定）。`diseasev2` と同様 XML 化＋405 証跡→サポートエスカレーションを行い、v2/v3 の差分を Runbook に追記する。
...
- **RUN_ID / 次アクション:** 未実測（再測定キュー: `RUN_ID=20251119TorcaTrialCrudZ#`。`curl -u user:pass` で取得予定）。`Request_Number=9` の XML を用意し、`/orca31` POST 405 のログを取得→既存会計と突合する準備を進める。
