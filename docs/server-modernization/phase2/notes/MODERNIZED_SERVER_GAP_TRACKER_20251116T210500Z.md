...
| ORCA-05 | 薬剤・特定器材・検査分類マスタ（例: `TBL_GENERIC_CLASS`, `TBL_GENERIC_PRICE`, `TBL_MATERIAL_*`, `TBL_KENSASORT`）を返す REST が不存在 | 薬効/用法/材料/検査分類コードを返却する新規 REST を追加し、DTO/Schema に分類・最低薬価・用法コードを含める。ステータス=Done、オーナー=Worker-B、優先度=P1。RUN_ID=`20251219T140028Z` で DTO/監査メタ/ページング/契約テストを実装。性能/監査計測ドラフト（RUN_ID=`20251124T111500Z`）の P99・アラート・必須ログ項目は運用フェーズで適用予定。 | `docs/server-modernization/phase2/operations/logs/20251219T140028Z-orca-05-master-generic.md`; `docs/server-modernization/phase2/operations/logs/20251123T135709Z-orca-master-gap.md`; `operations/ORCA_CONNECTIVITY_VALIDATION.md` §7 |
