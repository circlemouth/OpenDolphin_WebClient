...
- **ORCA サンドボックスの前提確認**  
  `docs/server-modernization/phase2/operations/EXTERNAL_INTERFACE_COMPATIBILITY_RUNBOOK.md#44` どおり `docker/orca/jma-receipt-docker` を起動し、`docker ps` + `curl http://localhost:8000/` + `docker run --network jma-receipt-docker-for-ubuntu-2204_default curlimages/curl:8.7.1 http://orca:8000/` を `artifacts/orca-connectivity/20251108T101538Z/` に保存した。2025-11-09 時点では Legacy WildFly 10 を `./scripts/start_legacy_modernized.sh start --build` → `start` で再起動し、`doctor1/dolphin` ユーザーで `ServerInfoResource` (`GET /openDolphin/resources/serverinfo/{claim/conn,jamri,cloud/zero}`) を叩いて最新値を採取した（`artifacts/parity-manual/CLAIM_SEND_ATTEMPT/legacy/serverinfo_*_20251108T19*.txt`）。`claim.conn` が `server` であること、`jamri` が空文字、`cloud.zero=false` であることを証跡化し、`ops/shared/docker/custom.properties` の `claim.host=orca` / `claim.send.port=8000` / `claim.send.encoding=MS932` が Compose へ反映されている状態を再確認。
