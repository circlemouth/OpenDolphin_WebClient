...
| --- | --- | --- | --- | --- |
| 2 | `/orca14/appointmodv2` | `POST /orca/appointments/mutation` | ◎ 実装完了 | 予約登録/取消/新規患者番号割当を DTO `AppointmentMutationRequest/Response` で受け、`OrcaAppointmentMutationFacade`→`AppoServiceBean` と監査ログを同期。Trial通信不可だが実装完了（RUN_ID=20251116T164200Z, 仕様ベース）。詳細: [appointmodv2 設計](../domains/ORCA_REST_IMPLEMENTATION_NOTES.md#appointmodv2-matrix-no2)。 |
| 4 | `/orca11/acceptmodv2` | `POST /orca/visits/mutation` | ◎ 実装完了 | 受付 CRUD・新規患者受付。`VisitMutationRequest/Response` が `PVTServiceBean`／`ChartEventServiceBean`／`PatientServiceBean` を橋渡しする。Trial通信不可だが実装完了（RUN_ID=20251116T164200Z, 仕様ベース）。詳細: [acceptmodv2 設計](../domains/ORCA_REST_IMPLEMENTATION_NOTES.md#acceptmodv2-matrix-no4)。 |
| 6 | `/api01rv2/appointlstv2` | `POST /orca/appointments/list` | ◎ 実装完了 | `OrcaAppointmentResource#listAppointments` が DTO/Service 化済み。Trial＝Blocker`TrialLocalOnly` のため WEB はスタブ応答で整合性を確認（RUN_ID=`20251116T170500Z`）。 |
...
| 42 | `/orca42/receiptprintv3` | `POST /orca/report/print` | ◎ 実装完了 | 帳票印刷を `ReportPrintJobRequest/Response` で受付し、`push-exchanger`＋`/blobapi/{dataId}` へ流す。Trial通信不可だが実装完了（RUN_ID=20251116T164200Z, 仕様ベース）。詳細: [receiptprintv3 設計](../domains/ORCA_REST_IMPLEMENTATION_NOTES.md#receiptprintv3-matrix-no42)。 |
