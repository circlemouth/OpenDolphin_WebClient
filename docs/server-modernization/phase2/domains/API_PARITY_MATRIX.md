...
| --- | --- | --- | --- | --- |
| 2 | `/orca14/appointmodv2` | `POST /orca/appointments/mutation` | ◎ 実装完了 | 予約登録/取消/新規患者番号割当を DTO `AppointmentMutationRequest/Response` で受け、`OrcaAppointmentMutationFacade`→`AppoServiceBean` と監査ログを同期。`RUN_ID=20251121T153200Z` および `RUN_ID=20251121PostOpenCheckZ1`（いずれも ORMaster `http://100.102.17.40:8000`）で `HTTP 405 (Allow: OPTIONS, GET)` を再現し、POST 未開放が継続。TrialLocalOnly → HTTP405(Local) へ Blocker 理由を更新。Evidence: `artifacts/orca-connectivity/20251121PostOpenCheckZ1/crud/appointmodv2/headers_*.txt`（前回: `20251121T153200Z`）。詳細: [appointmodv2 設計](../domains/ORCA_REST_IMPLEMENTATION_NOTES.md#appointmodv2-matrix-no2)。 |
