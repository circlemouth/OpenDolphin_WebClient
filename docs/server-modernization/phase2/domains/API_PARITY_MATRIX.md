...
- 最新実測（RUN_ID=`20251122T132337Z`, mac-dev ORCA）: `system01lstv2` class=02 で Dr `10000/10001` を取得し、`patientmodv2` auto で `Patient_ID=00005`（8 桁指定は `Api_Result=P1` で拒否）。`acceptmodv2` / `appointmodv2` / `medicalmodv2` / `acceptlstv2` / `appointlstv2` はいずれも HTTP200 / `Api_Result=00`。残課題=「8 桁患者番号は ORCA 管理連番設定変更が前提」「medicalmodv2 警告=W02/M05/M01（保険組合せ 0、診療種別未設定、点数マスタ未登録）」。

| No | ORCA API | モダナイズ REST | 状態 | 実装計画 |
| --- | --- | --- | --- | --- |
| 2 | `/orca14/appointmodv2` | `POST /orca/appointments/mutation` | ◎ 実装完了 | 予約登録/取消/新規患者番号割当を DTO `AppointmentMutationRequest/Response` で受け、`OrcaAppointmentMutationFacade`→`AppoServiceBean` と監査ログを同期。`RUN_ID=20251122T132337Z` で Mac Dev ORCA `HTTP 200 / Api_Result=00`（Physician_Code=`10000`, Patient_ID=`00005`、`Appointment_Id=00001`）を取得。Evidence: `artifacts/orca-connectivity/20251122T132337Z/crud/appointmodv2/response_20251122T045741Z.xml`。8 桁 Patient_ID は ORCA 設定変更が前提。詳細: [appointmodv2 設計](../domains/ORCA_REST_IMPLEMENTATION_NOTES.md#appointmodv2-matrix-no2)。 |
| 4 | `/orca11/acceptmodv2` | `POST /orca/visits/mutation` | ◎ 実装完了 | 受付 CRUD・新規患者受付。`VisitMutationRequest/Response` が `PVTServiceBean`／`ChartEventServiceBean`／`PatientServiceBean` を橋渡しする。`RUN_ID=20251122T132337Z` で Mac Dev ORCA `HTTP 200 / Api_Result=00`（Physician_Code=`10000`, Patient_ID=`00005`、`Acceptance_Id=00002`）を取得。Evidence: `artifacts/orca-connectivity/20251122T132337Z/crud/acceptmodv2/response_20251122T045705Z.xml`。8 桁 Patient_ID を使う場合は ORCA 管理連番設定変更が必須。詳細: [acceptmodv2 設計](../domains/ORCA_REST_IMPLEMENTATION_NOTES.md#acceptmodv2-matrix-no4)。 |
| 6 | `/api01rv2/appointlstv2` | `POST /orca/appointments/list` | ◎ 実装完了 | `OrcaAppointmentResource#listAppointments` が DTO/Service 化済み。`RUN_ID=20251122T132337Z` で Mac Dev ORCA `HTTP 200 / Api_Result=00`（Physician_Code=`10000`、`Appointment_Id=01`）を取得し seed 解消。Evidence: `artifacts/orca-connectivity/20251122T132337Z/crud/appointlstv2/response_20251122T045828Z.xml`。 |
...
| 11 | `/api01rv2/system01lstv2` | `POST /orca/system/management` | ◎ 実装完了 | 診療科/職員/医療機関基本情報など 7 クラスを `SystemMasterSnapshotRequest/Response` で統合し、`SystemServiceBean` キャッシュへ保存。`RUN_ID=20251122T132337Z` で Mac Dev ORCA `HTTP 200 / Api_Result=00`（class=02, Base_Date=2025-11-22）を取得し Dr `10000/10001` を確認。Evidence: `artifacts/orca-connectivity/20251122T132337Z/crud/system01lstv2/response_20251122T045513Z.xml`。詳細: [system01lstv2 設計](../domains/ORCA_REST_IMPLEMENTATION_NOTES.md#system01lstv2-matrix-no11)。 |
...
| 14 | `/orca12/patientmodv2` | `POST /orca/patient/mutation` | △ Spec-based（delete 未開放） | create/update を `PatientServiceBean#addPatient/#update` と `AuditTrail` に連携。`RUN_ID=20251122T132337Z` で Mac Dev ORCA auto 番号 `HTTP 200 / Api_Result=K0`（`Patient_ID=00005`）を取得。`Patient_ID=00000001` は `Api_Result=P1` で 8 桁拒否。delete は Trial 禁止のため引き続き `Api_Result=79 (Spec-based)`。Evidence: `artifacts/orca-connectivity/20251122T132337Z/{crud/patientmodv2,blocked/patientmodv2-8digit}/`。詳細: [patientmodv2 設計](../domains/ORCA_REST_IMPLEMENTATION_NOTES.md#patientmodv2-matrix-no17)。 |
...
- patientmodv2: 8�����Ҕԍ�(00000001)�� Api_Result=P1 �̂܂܁BORCA�ݒ�iORCBPTNUMCHG�j8�����͖{���ŕs�̂��߁A6���^�p�ŃN���[�Y�B
- medicalmodv2: �ی��g����=0001 + Medical_Class=210 + Medication_Code=620001402�i�f�Ó������炷 or Medical_Uid �����񂵁j�� Api_Result=00/�x���Ȃ����m�F�BDr=10000/10001 ���g�p�B
- Dr/���� seed �ς� (Patient=00005, Dr=10000/10001)�Baccept/appoint/medical/acceptmod/appointmod �� HTTP200/Api_Result=00�B
