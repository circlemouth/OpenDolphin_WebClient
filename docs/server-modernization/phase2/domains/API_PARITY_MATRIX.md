...
### ORCA API ラッパー計画（Matrix No.6-18, 33-37）

| No | ORCA API | モダナイズ REST | 状態 | 実装計画 |
| --- | --- | --- | --- | --- |
| 6 | `/api01rv2/appointlstv2` | なし | ✖ 未実装 | Phase2 Sprint2（2025-12）に [ORCA-REST-01](../../web-client/architecture/SERVER_MODERNIZATION_PLAN.md#phase-2-backlog-orca-rest-%E3%83%A9%E3%83%83%E3%83%91%E3%83%BC) として `POST /orca/appointments/list` を追加し、DTO `OrcaAppointmentListRequest/Response` で `OrcaConnect#appointlst` → `AppoServiceBean` を呼び出す。詳細: [ORCA-REST-01 設計](#orca-rest-01phase2-sprint2)。 |
...
| 8 | `/api01rv2/patientlst1v2` | なし | ✖ 未実装 | Phase2 Sprint2（2025-12）の [ORCA-REST-02](../../web-client/architecture/SERVER_MODERNIZATION_PLAN.md#phase-2-backlog-orca-rest-%E3%83%A9%E3%83%83%E3%83%91%E3%83%BC) で `POST /orca/patients/id-list`（DTO:`PatientIdBatchRequest/Response`）を実装し、`OrcaConnect` で ID 群を取得 → `PatientServiceBean` のバッチ同期を起動。詳細: [ORCA-REST-02 設計](#orca-rest-02phase2-sprint2)。 |
| 9 | `/api01rv2/patientlst2v2` | なし | ✖ 未実装 | Phase2 Sprint2（2025-12）で同じく [ORCA-REST-02](../../web-client/architecture/SERVER_MODERNIZATION_PLAN.md#phase-2-backlog-orca-rest-%E3%83%A9%E3%83%83%E3%83%91%E3%83%BC) の一部として `POST /orca/patients/batch` を追加し、DTO `PatientListBatchRequest/Response` を `OrcaConnect` → `PatientServiceBean` → `HealthInsuranceModel` で反映する。詳細: [ORCA-REST-02 設計](#orca-rest-02phase2-sprint2)。 |
| 10 | `/api01rv2/patientlst3v2` | なし | ✖ 未実装 | Phase2 Sprint2（2025-12）で [ORCA-REST-02](../../web-client/architecture/SERVER_MODERNIZATION_PLAN.md#phase-2-backlog-orca-rest-%E3%83%A9%E3%83%83%E3%83%91%E3%83%BC) 対象の `POST /orca/patients/name-search` を実装し、DTO `PatientNameSearchRequest/Response` によるカナ検索を ORCA → `PatientServiceBean#getPatientsByKana` へブリッジする。詳細: [ORCA-REST-02 設計](#orca-rest-02phase2-sprint2)。 |
| 11 | `/api01rv2/system01lstv2` | なし | ✖ 未実装 | Phase2 Sprint3（2026-01）で `POST /orca/system/management` を追加、DTO `SystemMasterSnapshot` を `SystemServiceBean`＋`ServerInfoResource` へ渡して職員・診療科マスタの seed を自動化。 |
| 12 | `/api01rv2/medicalgetv2` | なし | ✖ 未実装 | Phase2 Sprint3（2026-01）に `POST /orca/medical/records` を実装し、DTO `MedicalGetRequest/Response` を `OrcaConnect`→`KarteServiceBean`／`ClaimHelper` で解釈してカルテ履歴へマージする。 |
| 13 | `/api01rv2/diseasegetv2` | `GET /orca/disease/import/{pid}` | △ 部分実装 | Phase2 Sprint2（2025-12）で DTO `OrcaDiseaseImportResponse` を導入し、既存 GET へ `Api_Result` 等 ORCA 項目を追加しつつ `RegisteredDiagnosisListConverter`＋`AuditTrailService` でレスポンス差分を記録する。 |
| 14 | `/orca12/patientmodv2` | なし | ✖ 未実装 | Phase2 Sprint2（2025-12）の [ORCA-REST-02](../../web-client/architecture/SERVER_MODERNIZATION_PLAN.md#phase-2-backlog-orca-rest-%E3%83%A9%E3%83%83%E3%83%91%E3%83%BC) で `POST /orca/patient/mutation` を実装し、DTO `PatientMutationRequest/Response` を `OrcaConnect` → `PatientServiceBean` → `AuditTrailService` へ流す。詳細: [ORCA-REST-02 設計](#orca-rest-02phase2-sprint2)。 |
| 15 | `/api01rv2/appointlst2v2` | なし | ✖ 未実装 | Phase2 Sprint2（2025-12）で [ORCA-REST-01](../../web-client/architecture/SERVER_MODERNIZATION_PLAN.md#phase-2-backlog-orca-rest-%E3%83%A9%E3%83%83%E3%83%91%E3%83%BC) の一部として `POST /orca/appointments/patient` を提供し、DTO `PatientAppointmentListRequest/Response` を `OrcaConnect#appointlst2`＋`AppoServiceBean` で扱う。詳細: [ORCA-REST-01 設計](#orca-rest-01phase2-sprint2)。 |
| 16 | `/api01rv2/acsimulatev2` | なし | ✖ 未実装 | Phase2 Sprint2（2025-12）に [ORCA-REST-01](../../web-client/architecture/SERVER_MODERNIZATION_PLAN.md#phase-2-backlog-orca-rest-%E3%83%A9%E3%83%83%E3%83%91%E3%83%BC) として `POST /orca/billing/estimate` を追加し、DTO `BillingSimulationRequest/Response` を `ClaimSender`／`KarteServiceBean`／`OrcaConnect#acsimulate` へ接続する。詳細: [ORCA-REST-01 設計](#orca-rest-01phase2-sprint2)。 |
| 17 | `/orca25/subjectivesv2` | なし | ✖ 未実装 | Phase2 Sprint2（2025-12）で [ORCA-REST-02](../../web-client/architecture/SERVER_MODERNIZATION_PLAN.md#phase-2-backlog-orca-rest-%E3%83%A9%E3%83%83%E3%83%91%E3%83%BC) に前倒しし、`POST /orca/chart/subjectives`（DTO:`SubjectiveEntryRequest/Response`）を `ChartEventServiceBean`＋`KarteServiceBean`＋`OrcaConnect` で同期。詳細: [ORCA-REST-02 設計](#orca-rest-02phase2-sprint2)。 |
| 18 | `/api01rv2/visitptlstv2` | なし | ✖ 未実装 | Phase2 Sprint2（2025-12）で [ORCA-REST-01](../../web-client/architecture/SERVER_MODERNIZATION_PLAN.md#phase-2-backlog-orca-rest-%E3%83%A9%E3%83%83%E3%83%91%E3%83%BC) 対象 `POST /orca/visits/list`（DTO:`VisitPatientListRequest/Response`）を追加し、`OrcaConnect#visitptlst` と `PVTServiceBean` の来院キャッシュを連携する。詳細: [ORCA-REST-01 設計](#orca-rest-01phase2-sprint2)。 |
| 33 | `/orca21/medicalsetv2` | なし | ✖ 未実装 | Phase2 Sprint4（2026-02）で `POST /orca/medical-sets` を提供し、DTO `MedicalSetMutationRequest/Response` を `OrcaConnect`＋`StampServiceBean`＋`ModuleListConverter` で処理してセット CRUD を一本化。 |
| 34 | `/orca31/birthdeliveryv2` | なし | ✖ 未実装 | Phase2 Sprint4（2026-02）に `POST /orca/birth-delivery` を追加、DTO `BirthDeliveryRequest/Response` を `OrcaConnect`、`AdmissionResource`、`HealthInsuranceModel` で処理して申請データを保存する。 |
| 35 | `/api01rv2/patientlst6v2` | なし | ✖ 未実装 | Phase2 Sprint2（2025-12）で [ORCA-REST-02](../../web-client/architecture/SERVER_MODERNIZATION_PLAN.md#phase-2-backlog-orca-rest-%E3%83%A9%E3%83%83%E3%83%91%E3%83%BC) 対象 `POST /orca/insurance/combinations` を実装し、DTO `InsuranceCombinationRequest/Response` を `OrcaConnect`＋`PatientServiceBean`＋`HealthInsuranceModel` に流して保険組合せキャッシュを構築する。詳細: [ORCA-REST-02 設計](#orca-rest-02phase2-sprint2)。 |
| 36 | `/orca22/diseasev2` | なし | ✖ 未実装 | Phase2 Sprint3（2026-01）に `POST /orca/disease` を追加、DTO `DiseaseMutationRequest/Response` を `OrcaConnect`／`RegisteredDiagnosisModel`／`AuditTrailService` で処理して v2 登録をラップ。 |
| 37 | `/orca22/diseasev3` | なし | ✖ 未実装 | Phase2 Sprint3（2026-01）で `POST /orca/disease/v3` を追加し、DTO `DiseaseMutationV3Request/Response` と `RegisteredDiagnosisModel`＋`AuditTrailService` で補足コメント・BaseMonth の差分管理を行う。 |

#### ORCA-REST-01（Phase2 Sprint2）

| Wrapper | ORCA API（class） | REST Path / Method | リクエスト schema | レスポンス schema | バリデーション / ORCA 呼び出しハンドラ |
| --- | --- | --- | --- | --- | --- |
| 予約一覧（1日分） | `/api01rv2/appointlstv2?class=01` | `POST /orca/appointments/list` | `OrcaAppointmentListRequest`：`appointmentDate`(必須 `yyyy-MM-dd`)、`medicalInformation`(任意、`01-07` or `99`)、`physicianCode`(任意 5 桁)、`includeVisitStatus`(bool) | `OrcaAppointmentListResponse`：`apiResult` / `apiResultMessage` / `slots[]`（時刻、診療科、医師、来院フラグ、`PatientSummary`）／`rawXml`(任意)／`runId` | `appointmentDate` が欠落または不正フォーマットなら 400。`medicalInformation` は `^(0[1-7]|99)$`。`OrcaResource#postAppointmentsList` が `OrcaConnect#appointlst` を実行し、戻り値を `AppoServiceBean` と Web クライアント双方へ返す。 |
| 予約一覧（患者単位） | `/api01rv2/appointlst2v2?class=01` | `POST /orca/appointments/patient` | `PatientAppointmentListRequest`：`patientId`(必須)、`baseDate`(任意 `yyyy-MM-dd`)、`departmentCode`、`requestNumber` (`01/02`) | `PatientAppointmentListResponse`：`apiResult`、`reservations[]`（予約日時、診療科、来院済みフラグ、`PatientSummary`）、`warnings[]` | `patientId` は `^[0-9]{1,16}$`。`baseDate` が 180 日超未来の場合は 422。`OrcaResource#postPatientAppointments` → `OrcaConnect#appointlst2`。レスポンスは `AppoServiceBean#getAppointmentList` の結果と統合し、監査イベント `APPOINTMENT_SYNC` を記録。 |
| 請求試算 | `/api01rv2/acsimulatev2?class=01` | `POST /orca/billing/estimate` | `BillingSimulationRequest`：`patientId`、`karteId`、`performDate`、`departmentCode`、`physicianCode`、`claimBundles[]`（code, qty, point, memo） | `BillingSimulationResponse`：`apiResult`、`estimate`（総額/公費控除/自己負担/入外区分）、`warnings[]`、`mirroredClaimXml`、`runId` | `claimBundles` が空の場合 400。`performDate` は診療日当日±1 営業日。`OrcaResource#postBillingEstimate` が `ClaimSender` でスタブ CLAIM を生成して `OrcaConnect#acsimulate` へ渡し、結果を `KarteServiceBean` へ通知（ドラフトカルテへ埋め込み）する。 |
| 来院患者一覧 | `/api01rv2/visitptlstv2?class=01/02` | `POST /orca/visits/list` | `VisitPatientListRequest`：`visitDate`(必須 `yyyy-MM-dd`)、`departmentCode`、`physicianCode`、`requestNumber`(01: 詳細、02: 月次カレンダー) | `VisitPatientListResponse`：`apiResult`、`visitDate`、`visits[]`（受付番号、順番、`PatientSummary`、`insuranceSummary`、`Visit_Calendar`）、`pvtCacheUpdated` | `visitDate` フォーマット厳密化。`departmentCode` は `SystemServiceBean` キャッシュに存在するコードのみ許可。`OrcaResource#postVisitPatients` が `OrcaConnect#visitptlst` を叩き、戻り値を `PVTServiceBean` の来院キャッシュへ反映して Web クライアントへ返す。 |

#### ORCA-REST-02（Phase2 Sprint2）

| Wrapper | ORCA API（class） | REST Path / Method | リクエスト schema | レスポンス schema | バリデーション / ORCA 呼び出しハンドラ |
| --- | --- | --- | --- | --- | --- |
| 患者 ID 一括取得 | `/api01rv2/patientlst1v2?class=01` | `POST /orca/patients/id-list` | `PatientIdBatchRequest`：`baseStartDate` / `baseEndDate`（必須、最大 90 日幅）、`includeTestPatient`(bool) | `PatientIdBatchResponse`：`apiResult`、`targetCount`、`patients[]`（`patientId`、氏名、作成/更新日時、`testPatientFlag`）、`syncTicketId` | 日付範囲外は 400。`includeTestPatient` デフォルト false。`OrcaResource#postPatientIdBatch` が `OrcaConnect#patientlst1` を呼び出し、`PatientServiceBean` の差分同期ジョブへ投入。 |
| 患者詳細バッチ取得 | `/api01rv2/patientlst2v2?class=01` | `POST /orca/patients/batch` | `PatientListBatchRequest`：`patientIds[]`（最大 200 件）または `continuationToken`、`includeInsurance` | `PatientListBatchResponse`：`patients[]`（基本属性＋`HealthInsuranceModel` 配列）、`continuationToken`、`apiResult` | `patientIds` が空なら 400。`includeInsurance` true の場合は `HealthInsuranceModel` を正規化して永続化。`OrcaResource#postPatientBatch` → `OrcaConnect#patientlst2` → `PatientServiceBean#mergeOrcaPatient`。 |
| 氏名カナ検索 | `/api01rv2/patientlst3v2?class=01` | `POST /orca/patients/name-search` | `PatientNameSearchRequest`：`name` or `kana`（必須）、`birthDate`、`sex`、`fuzzyMode`（`prefix/suffix/partial`） | `PatientNameSearchResponse`：`hits[]`（`patientId`、`fullname`、`kana`、`birthday`、`sex`、`lastVisitDate`）＋`apiResult` | `name`・`kana` が両方未指定は 400。`fuzzyMode` のデフォルトは `partial`。`OrcaResource#postPatientNameSearch` が `OrcaConnect#patientlst3` を呼び、ローカル `PatientServiceBean#getPatientsByName/Kana` の差分も返す。 |
| 患者登録/更新/削除 | `/orca12/patientmodv2?class=01-03` | `POST /orca/patient/mutation` | `PatientMutationRequest`：`operation`(`create/update/delete`)、`patientPayload`（`PatientModel` 相当）、`traceId` | `PatientMutationResponse`：`apiResult`、`patientId`、`warnings[]`、`auditId` | `operation` と payload の整合を検証。診療科/医師コードは `SystemServiceBean` キャッシュ一致が必須。`OrcaResource#postPatientMutation` → `OrcaConnect#patientmod` で ORCA 反映後、`PatientServiceBean` と `AuditTrailService` へ書き戻し。 |
| 保険組合せ取得 | `/api01rv2/patientlst6v2?class=01` | `POST /orca/insurance/combinations` | `InsuranceCombinationRequest`：`patientId`（必須）、`baseDate`、`rangeStart`、`rangeEnd` | `InsuranceCombinationResponse`：`combinations[]`（組合せ番号、給付率、適用期間、保険種別、公費情報）＋`apiResult` | `rangeStart/End` は 1 年以内。`OrcaResource#postInsuranceCombination` → `OrcaConnect#patientlst6`。結果を `PatientServiceBean` で `HealthInsuranceModel` へ反映し、`PVTServiceBean` にも共有。 |
| 症状詳記登録 | `/orca25/subjectivesv2?class=01` | `POST /orca/chart/subjectives` | `SubjectiveEntryRequest`：`patientId`、`karteId`、`performDate`、`soapCategory`(`S/O/A/P`)、`body`（1,000 文字以内）、`insuranceCombinationNumber`、`departmentCode`、`physicianCode` | `SubjectiveEntryResponse`：`apiResult`、`entryId`、`warnings[]`、`echoedBody`、`auditId` | `body` 長 >1,000 文字で 422。`soapCategory` が無効なら 400。`OrcaResource#postSubjectives` が `OrcaConnect#subjectives` で ORCA 登録後、`ChartEventServiceBean`＋`KarteServiceBean` へ伝播し、`ORCA_CONNECTIVITY_VALIDATION.md` §4.4 の RUN_ID をレスポンスへ含める。 |
