# 外部連携 Jakarta EE 10 対応状況（更新日: 2025-11-03）
...
| オンライン資格確認／電子処方箋（OQS） | `ext_lib/OpenDolphin-ORCA-OQS` （Git サブモジュール） | `MasudaNaika/OpenDolphin-ORCA-OQS` をサブモジュールとして取り込み、OQS SDK の Jakarta 互換化調査はこれから。 | モダナイズ版サーバーに統合する際は REST→OQS API ブリッジの設計、Secrets（資格確認・電子処方箋鍵）の保護ポリシー、CI へのビルド取り込み手順を確立する必要がある。 | High |
| PDF 署名／帳票 | `server-modernized/src/main/java/open/dolphin/reporting/PdfSigningService.java`<br>`server-modernized/src/main/java/open/dolphin/reporting/SigningConfig.java`<br>`server-modernized/pom.xml` | OpenPDF 1.3.41 と BouncyCastle 1.82 を BOM 管理。`PdfSigningService` は `com.lowagie.text.pdf` API + `Signature(SHA256withRSA)` による独自実装を維持。 | Java 17 ターゲットのため OpenPDF 1.3 系を継続使用。3.x へ移行する際は Java 21 対応と `org.openpdf.*` API の再確認が必要。 | Medium |
...
### 2025-11-03 追記（Worker0/1 成果ログ）

- ✅ 2025-11-03 Worker1: CLAIM / PVT 送信系の XSLT テンプレートを Java ビルダーへ置換し、Jakarta XML Binding で生成した XML を既存フローへ組み込んだ。生成差分レポートはチームの共有フォルダで配布済み。
- ✅ 2025-11-03 Worker0: Hibernate のカスタム `StringClobType` 互換 JAR を廃止し、Jakarta Persistence 3.1 + Hibernate 6 標準 Type へ移行。外部連携モジュールの CLOB フィールドは `@Lob` + `TEXT` マッピングで動作確認済み。
- ⚠️ 2025-11-03 Codex: 上記変更に合わせた自動テスト（CLAIM 送信 E2E、Hibernate 6 での大規模文書保存）が未整備。`PHASE2_PROGRESS.md` の TODO に記載したとおり、2025-11-08 までに Worker4 がテストシナリオを設計する。
...
| OpenPDF | `server-modernized/pom.xml` | 1.3.41 | 3.x 系で `org.openpdf.*` へ移行し Java 21 用クラスファイルとなるため、現行 Java 17 では採用不可。 | 1.3.41 を基準に `PdfSigningService` の回帰テストを整備し、Java 21 移行時に 3.x を再評価する。 |
...
- OpenPDF 1.3.41 反映と同時に `docs/server-modernization/reporting/LICENSE_COMPATIBILITY.md` のバージョン表を更新する。
...
- **OpenPDF 1.3.41**（LGPL 2.1 / MPL 1.1 デュアルライセンス）: WAR 配布時にソース入手方法の告知が必要。`docs/server-modernization/reporting/LICENSE_COMPATIBILITY.md` と `NOTICE` の整合を保つ。
