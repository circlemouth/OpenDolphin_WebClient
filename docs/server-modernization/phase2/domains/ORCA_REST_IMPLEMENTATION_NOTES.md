...
- **テスト/Runbook 連携**: `ORCA_CONNECTIVITY_VALIDATION.md` §4.3 の P0-2 手順で curl 証跡を取得し、`ORCA_API_STATUS.md` 2.1 行に RUN_ID を追記。`logs/<date>-orca-connectivity.md` の `P0_appointlstv2` ディレクトリと突き合わせる。2025-11-21 (`RUN_ID=20251121T153200Z`, ormaster/change_me @ `http://100.102.17.40:8000`) は HTTP200 だが `Api_Result=12 (ドクターが存在しません)` を返したため、医師マスタ seed が揃ってから再測する。Evidence: `artifacts/orca-connectivity/20251121T153200Z/crud/appointlstv2/response_*.xml`。
...
- **テスト/Runbook 連携**: WebORCA Trial は POST `/orca14/appointmodv2` を 405（Allow: OPTIONS, GET）で遮断するため CRUD 実測不可。2025-11-21 の ORMaster 再測 (`RUN_ID=20251121T153200Z`, `RUN_ID=20251121PostOpenCheckZ1`) でも `HTTP 405 (Allow: OPTIONS, GET)` が継続。Blocker 理由を HTTP405(Local) のまま維持し、POST 開放後に `logs/20251121PostOpenCheckZ1-api-contract.md#orca14appointmodv2` を再取得する。Evidence: `artifacts/orca-connectivity/20251121PostOpenCheckZ1/crud/appointmodv2/headers_*.txt`（前回: `20251121T153200Z`）。 
...
- **テスト/Runbook 連携**: Trial 環境は `/orca11/acceptmodv2` も 405 応答となるため、RUN_ID=`20251116T164200Z` の検証ログでは「Trial通信不可だが実装完了（仕様ベース）」タグと `trialsite.md#limit` 引用のみを残す。2025-11-21 ORMaster (`RUN_ID=20251121T153200Z`, `RUN_ID=20251121PostOpenCheckZ1`) でも `HTTP 405 (Allow: OPTIONS, GET)` 継続。`ORCA_CONNECTIVITY_VALIDATION.md` §4.3 の P0-1 テンプレは HTTP405(Local) 根拠として `artifacts/orca-connectivity/20251121PostOpenCheckZ1/crud/acceptmodv2/headers_*.txt` を参照する。
...
- **テスト/Runbook 連携**: trialsite §4 の制限により帳票印刷はトライアル環境で禁止されている。2025-11-21 ORMaster (`http://100.102.17.40:8000`, `RUN_ID=20251121T153200Z`, `RUN_ID=20251121PostOpenCheckZ1`) も `HTTP 405 (Allow: OPTIONS, GET)` を返し POST 未開放。`ORCA_API_STATUS.md` Matrix No.42 には HTTP405(Local) を Blocker として残し、push-exchanger／`/blobapi` 構成後に再捕捉する。Evidence: `artifacts/orca-connectivity/20251121PostOpenCheckZ1/crud/receiptprintv3/headers_*.txt`（前回: `20251121T153200Z`）。

#### <a id="patientmemomodv2-matrix-no53"></a> patientmemomodv2（Matrix No.53）
- **ORCA spec 参照**: [`../operations/assets/orca-api-spec/raw/patientmemomodv2.md`](../operations/assets/orca-api-spec/raw/patientmemomodv2.md)。`patient_memomodreq` の必須項目（`Request_Number`, `Patient_ID`, `Department_Code`, `Memo_Class`, `Patient_Memo`）を確認。
- **モダナイズ REST URI**: 未定義（ラッパー検討中）。ORCA 原本の POST が開放され次第、`POST /orca/patient/memos` 相当の REST を追加予定。
- **テスト/Runbook 連携**: 2025-11-21 ORMaster 再測 (`RUN_ID=20251121PostOpenCheckZ1`) でも `/orca06/patientmemomodv2` が `HTTP 405 (Allow: OPTIONS, GET)` のまま。Trial 側も POST 禁止。Evidence: `artifacts/orca-connectivity/20251121PostOpenCheckZ1/crud/patientmemomodv2/headers_*.txt`。POST 開放後に DTO 方針と REST Path を決定し、Matrix へ追加する。
