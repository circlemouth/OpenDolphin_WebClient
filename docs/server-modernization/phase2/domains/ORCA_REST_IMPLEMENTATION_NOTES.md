...
- **テスト/Runbook 連携**: `ORCA_CONNECTIVITY_VALIDATION.md` §4.3 の P0-2 手順で curl 証跡を取得し、`ORCA_API_STATUS.md` 2.1 行に RUN_ID を追記。`logs/<date>-orca-connectivity.md` の `P0_appointlstv2` ディレクトリと突き合わせる。2025-11-21（RUN_ID=`20251121T153200Z`、接続詳細は `mac-dev-login.local.md` 参照）は HTTP200 だが `Api_Result=12 (ドクターが存在しません)` を返したため、医師マスタ seed が揃ってから再測する。Evidence: `artifacts/orca-connectivity/20251121T153200Z/crud/appointlstv2/response_*.xml`。
...
- **テスト/Runbook 連携**: trialsite §4 の制限により帳票印刷はトライアル環境で禁止されている。2025-11-21 の開発接続先（詳細は `mac-dev-login.local.md` 参照、`RUN_ID=20251121T153200Z`, `RUN_ID=20251121PostOpenCheckZ1`）も `HTTP 405 (Allow: OPTIONS, GET)` を返し POST 未開放。`ORCA_API_STATUS.md` Matrix No.42 には HTTP405(Local) を Blocker として残し、push-exchanger／`/blobapi` 構成後に再捕捉する。Evidence: `artifacts/orca-connectivity/20251121PostOpenCheckZ1/crud/receiptprintv3/headers_*.txt`（前回: `20251121T153200Z`）。
