...
- 実施者: Codex（macOS 15.1 CLI）。Basic 認証 `<MASKED>/<MASKED>` のみを使用し、PKCS#12 は禁止ポリシーに従い未使用。
...
| 実行方法 | `curl -u <MASKED>:<MASKED>` に `X-Facility-Id=1234567`, `X-Touch-TraceId=20251121TrialPHRSeqZ1-CDE`, `X-Access-Reason=<api>` を付与。PHR-06 は `Content-Type: application/json` + `X-Consent-Token`、PHR-07 は `Accept: image/jpeg`、PHR-11 は `Accept: application/json` を指定。`httpdump/` へレスポンスを保存し、`trace/` へ `--trace-ascii` を記録。
