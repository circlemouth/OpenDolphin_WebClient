...
## 概要
- `src/outpatient_ux_modernization/03_モダナイズAPIマッピング.md` に Reception/Charts/Patients/Administration の外来 API（`/orca/claim/outpatient/*`、`/orca/appointments/list/*`、`/orca12/patientmodv2/outpatient`、`/orca21/medicalmodv2/outpatient`）のレスポンス設計と `auditEvent` フィールドの要件を整理した。
...
## ギャップ
1. `/orca/claim/outpatient/*` および `/orca/appointments/list/*` は現行 `KarteResource`/`OrcaAppointmentResource` が `recordAudit` を実行していないため `d_audit_event` に `claim`/`appointment` 系 `action` が記録されていない。`auditEvent.details` に `facilityId`/`patientId`/`appointmentId`/`claimBundles` 等を追加する設計を検討してほしい。
