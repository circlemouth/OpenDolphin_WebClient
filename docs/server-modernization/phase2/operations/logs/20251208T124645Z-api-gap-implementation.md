...
- 対象: `/orca/claim/outpatient/mock` と `/orca21/medicalmodv2/outpatient` の modernized 実装／telemetry + audit 整合、Web クライアント契約モック更新、DOC_STATUS 反映。
...
- `server-modernized`:
  - `OrcaClaimOutpatientResource` に RUN_ID=20251208T124645Z を設定し、`recordsReturned`/`telemetryFunnelStage=resolve_master` を含む audit details を追加。`SessionAuditDispatcher` で `ORCA_CLAIM_OUTPATIENT` を SUCCESS として送出し、`traceId`/`requestId`/`actorId` を付与。
...
## テスト / 検証
- 単体: `mvn -f pom.server-modernized.xml -pl server-modernized -Dtest=OrcaClaimOutpatientResourceTest,OrcaMedicalModV2ResourceTest test`（PASS）。両エンドポイントが `traceId`/`requestId`/`recordsReturned` を含む audit 付きレスポンスを返すことを確認。
