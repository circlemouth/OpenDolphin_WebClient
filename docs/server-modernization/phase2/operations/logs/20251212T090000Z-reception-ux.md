...
- `/orca/appointments/list/*`・`/orca/claim/outpatient/*` など `OUTPATIENT_API_ENDPOINTS` に登録した外来 endoint を `httpClient` 層経由で `auditMetadata` を付与し、`missingMaster` true が `tone=server` に転換する条件を `docs/server-modernization/phase2/operations/ORCA_API_STATUS.md` と照合しながら明記。
