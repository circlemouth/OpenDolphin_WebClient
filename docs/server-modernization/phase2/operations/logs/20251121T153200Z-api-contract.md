...
- 接続先: 開発用 ORCA（詳細は `docs/web-client/operations/mac-dev-login.local.md` を参照）。Python 不使用、`curl` のみ。
...
- コマンド: `curl -s -u <DEV_ORCA_BASIC> -H 'Accept: application/xml' -H 'Content-Type: application/xml' --data-binary @request_2025-11-21.xml <DEV_ORCA_HOST>/api01rv2/acceptlstv2?class=01`
...
- コマンド: `curl -s -u <DEV_ORCA_BASIC> ... <DEV_ORCA_HOST>/api01rv2/appointlstv2?class=01`
- Evidence: `crud/appointlstv2/response_2025-11-21T06-48-07Z.xml`（Api_Result=12）。
- 所見: 医師マスタ不足。

### <a id="medicalmodv2"></a>`/api/api21/medicalmodv2`
- コマンド: `curl -s -u <DEV_ORCA_BASIC> ... <DEV_ORCA_HOST>/api/api21/medicalmodv2?class=01`
...
- コマンド: `curl -s -u <DEV_ORCA_BASIC> ... <DEV_ORCA_HOST>/orca11/acceptmodv2?class=01`
...
- コマンド: `curl -s -u <DEV_ORCA_BASIC> ... <DEV_ORCA_HOST>/orca14/appointmodv2?class=01`
...
- コマンド: `curl -s -u <DEV_ORCA_BASIC> ... <DEV_ORCA_HOST>/orca06/patientmemomodv2`
...
- コマンド: `curl -s -u <DEV_ORCA_BASIC> ... <DEV_ORCA_HOST>/orca42/receiptprintv3`
