...
- 接続先: `https://weborca-trial.orca.med.or.jp/`（BASIC=`trial`/`<MASKED>`）。Python 不使用、`curl`/shell のみ。
...
- 例: `curl -s -u <MASKED>:<MASKED> -H 'Accept: application/xml' -H 'Content-Type: application/xml' --retry 2 --retry-all-errors --max-time 20 --data-binary @payloads/acceptlst_trial.xml https://weborca-trial.orca.med.or.jp/api01rv2/acceptlstv2?class=01`。
