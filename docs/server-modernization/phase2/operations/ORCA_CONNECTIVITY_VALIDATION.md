...
| 認証情報 | `ORCAcertification/103867__JP_u00001294_client3948.p12` と `ORCAcertification/新規 テキスト ドキュメント.txt`（ORCAMO ID, API キー, 接続 URL, ポート）。必要に応じて同ディレクトリのパスフレーズファイルを参照し、作業後は必ず `unset ORCA_PROD_*`。 |
...
| `新規 テキスト ドキュメント.txt` | 接続 URL／ポート／ORCAMO ID／API キーのメモ | 内容は秘匿扱い。`cat` したら即 `pbcopy` などで別ツールへ渡さず、環境変数経由で使用する。 |
...
5. 成功時は `PHASE2_PROGRESS.md` に `RUN_ID`, `HTTP`, `Api_Result`, `証跡パス` を追記。失敗時は原因（証明書期限, ネットワーク, 認証失敗等）と次アクションを明記。
