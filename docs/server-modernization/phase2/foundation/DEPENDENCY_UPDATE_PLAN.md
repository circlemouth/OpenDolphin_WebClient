...
| `jakarta.naming` | `server-modernized` は Jakarta BOM 依存、旧 `server` は Java EE 7 のため `javax.naming` を継続利用 | Jakarta Naming API は JBoss Public Repository 専用配布のため、社内ネットワークから 403 応答となり依存解決が失敗。 | JBoss Public Repository を `pom.server-modernized.xml` へ登録しつつ、アクセス制限を解消できるまでビルド時はエラーになることを明示。旧サーバーは従来スタックを維持し、Jakarta 化はモダナイズ移行時にのみ検討する。 | Eclipse Public License 2.0 / GPL v2 with Classpath Exception。 |
| `jakarta.websocket` | WildFly BOM が `2.1.0-jbossorg-2` を参照 | 403 により JBoss 公開リポジトリへアクセスできず、`jakarta.websocket-(client-)api` の取得に失敗。 | Maven Central に存在する `2.1.0` へ手動でバージョン固定し、`server-modernized/pom.xml` に `provided` 依存として追加。 | Eclipse Public License 2.0 / GPL v2 with Classpath Exception。 |
