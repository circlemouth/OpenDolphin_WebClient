...
- ⚠️ 2025-11-03 Codex: 旧 `ops/legacy-server/docker/Dockerfile`・`ops/modernized-server/docker/Dockerfile` に残る `dependency:get org.hibernate:hibernate-core:5.0.10.Final` の削除と CI キャッシュ更新を Worker C が追跡中。Compose から新 Type を使用する回帰テストを 2025-11-06 までに実施。
...
- ✅ 2025-11-19 MSG-01: `server-modernized/src/main/java/open/dolphin/session/MessageSender.java` の MDB 実装を完全な Jakarta Messaging 3.0 準拠へ復旧。`@MessageDriven` で `java:/queue/dolphin` へ接続し、`DocumentModel` (CLAIM)、`DiagnosisSendWrapper`（診断名）、`PatientVisitModel` (PVT) を JMS 経由で送信。`MessagingConfig` を通じた `diagnosis.claim.send` フラグ制御も実装済み。証跡: `operations/logs/20251119T140358Z-jms-implementation.md`
- ✅ 2025-11-19 MSG-01: `server-modernized/src/main/java/open/dolphin/msg/gateway/MessagingGateway.java` を更新し、JMS enqueue 失敗時のフォールバックロジックを整備。`diagnosisClaimSend` フラグによる傷病名送信制御を追加し、`MessagingConfig` との一貫性を確保。`@Resource` による `java:/JmsXA` および `java:/queue/dolphin` の JNDI 参照はコード内で明示済み。
- ✅ 2025-11-19 MSG-01: `server-modernized/src/main/java/open/dolphin/msg/DiagnosisSender.java` から Legacy の `custom.properties` 直接読み込みを除去し、`MessagingConfig` による設定集約へ移行。`diagnosis.claim.send` プロパティは `MessagingConfig.ClaimSettings` のレコードフィールドとして一元管理。
- ⚠️ JMS 連携は実装復旧済みだが、実際の ORCA 送信および WildFly 33 上での ACK 受信証跡は未取得。`ops/tools/jms-probe.sh` 等を使用した実測検証を 2025-11-25 までに実施し、`operations/logs/\<RUN_ID\>-jms-orca-ack.md` へ記録すること。
...
- MicroProfile Metrics サブシステムは WildFly 33 で非推奨となり、デフォルトで Micrometer が推奨される。`RequestMetricsFilter`／`DatasourceMetricsRegistrar` を `io.micrometer.core.instrument` へ書き換えるとともに、`ops/modernized-server/docker/configure-wildfly.cli` へ `subsystem=micrometer` の有効化と Prometheus エクスポータ設定を追加する必要がある。運用ドキュメント（`docs/server-modernization/operations/OBSERVABILITY_AND_METRICS.md`）も Micrometer 前提に更新すること。
