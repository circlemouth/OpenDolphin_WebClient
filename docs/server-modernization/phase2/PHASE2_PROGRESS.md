...
## 2025-11-15 追記: API パリティ自動チェック再実行（担当: Codex）
- **RUN_ID=`20251115TapiparityZ1`**: `ops/tests/api-smoke-test/run.sh --scenario base_readonly --run-id 20251115TapiparityZ1 --dual` を `BASE_URL_{LEGACY,MODERN}=http://localhost:{8080,9080}/openDolphin/resources` で実行し、成果物を `artifacts/parity-manual/smoke/20251115TapiparityZ1/{legacy,modernized}/` に保存。全 3 ケースが Legacy/Modernized とも 401 で終了し、本文は `<html>...Unauthorized</html>` のみ。Modernized は `Referrer-Policy` / `Content-Security-Policy` / `Strict-Transport-Security` / `X-Content-Type-Options` を返す一方、Legacy は `Server` / `X-Powered-By` を露出（例: `.../base_readonly_dolphin/headers.txt`）。
- **Touch parity checker**: `PARITY_OUTPUT_DIR=tmp/parity-touch/20251115TapiparityZ1 ops/tools/send_parallel_request.sh --config scripts/api_parity_targets.touch.json` で `touch_sendPackage_javaTime` ほか 2 ケースの 401 レスポンスを採取し、`LEGACY_API_BASE=... python3 scripts/api_parity_response_check.py --config tmp/parity-touch/20251115TapiparityZ1/api_targets.json` を実行。`diff.txt` は 3 件すべて `[PASS] Body text matches.` だが、`meta.json` には 401 のまま記録されており JSON 比較は未実施。
- **Root cause**: `artifacts/parity-manual/smoke/20251115TapiparityZ1/logs/legacy_server_tail.log` に `HHH000262: Table not found: d_users / d_patient / d_stamp` が多数出力され、Legacy DB シードが消失。`.../modern_server_tail.log` でも `WFLYCTL0013 Failed services` や `org.postgresql.util.PSQLException: ERROR: relation "d_patient_memo" does not exist` が繰り返され、Modernized も初期データ欠落により Weld 起動が失敗している。結果として Basic 認証がユーザー検索前に 401 で停止し、base_readonly/Touch シナリオがすべてブロック。
- **TODO**: (1) `docs/server-modernization/phase2/operations/POSTGRES_BASELINE_RESTORE.md` に沿って Legacy/Modernized 双方へ `d_users/d_patient/d_stamp/d_diagnosis` を再投入し、`open_dolphin-server.war` のデプロイ成功 (`WFLYSRV0026` 緑化) を確認、(2) doctor1/admin で 200 応答が返るまで `base_readonly` を rerun → `tmp/parity-touch/20251115TapiparityZ1/api_targets.json` の `compare: "text"` を `json` に戻す、(3) 再取得後に `SERVER_MODERNIZED_DEBUG_CHECKLIST.md` フェーズ9と `docs/web-client/planning/phase2/DOC_STATUS.md` Testing セクションを「Recovered」で更新し、`touch-api-parity.md` 12章の TODO をクローズする。
...
- ステータス: **Maven package 成功（再ビルド前）**。`mvn -f pom.server-modernized.xml -pl server-modernized -am package -DskipTests` を再実行し、`server-modernized/target/opendolphin-server.war` が生成されたこととログ `artifacts/parity-manual/build/duplicate_project/mvn-package-server-modernized.log`（`ls -l server-modernized/target` 付き）を保存済み。
- 重複原因は `server/pom.xml` と `server-modernized/pom.xml` が同一 artifactId (`opendolphin-server`) を宣言したまま単一リアクターに登録されている点。詳細メモは `docs/server-modernization/phase2/notes/domain-transaction-parity.md §3.5` を参照。
- 修正案（Modernized 側 artifactId のリネーム vs 専用リアクター分離）は `patches/maven_duplicate_fix_plan.md` に整理中。今回の再ビルド結果に合わせて `SERVER_MODERNIZED_DEBUG_CHECKLIST.md` の該当行も「Maven package 成功（再ビルド前）」へ更新した。
...
- 🔁 次アクション: (1) `tmp/legacy-fixtures/adm10|adm20` を git 管理へ含めるか否かを判断（Checklist #78 継続）、(2) Snapshot テストを Phase2 モダナイズ CI へ組み込み、`adm.snapshot.update=true` 実行ガイドを `test-data-inventory.md` に補足、(3) ADM DTO 追加時は本テストのフィクスチャ更新と `rest-touch-diff-report.md` の RUN_ID セクション追記をルール化する。
- 🧪 CI 取り込み設計完了: `mvn -f reporting/pom.xml install -DskipTests` → `mvn -f server-modernized/pom.xml test -Dtest=AdmConverterSnapshotTest -DskipITs -Dsurefire.failIfNoSpecifiedTests=false` を 1 ジョブにまとめ、`ADM_SNAPSHOT_RUN_ID=$(date -u +%Y%m%dT%H%M%SZ)` を用いた RUN_ID 命名・Surefire ログのアーティファクト保存・`artifacts/parity-manual/adm-snapshots/${ADM_SNAPSHOT_RUN_ID}` への差分退避案を `notes/rest-touch-diff-report.md` に追記済み。CI では差分ディレクトリ検出後に `upload-artifact` へ渡すだけでよく、Docker 起動は不要。
- 📦 フィクスチャ管理 TODO: `tmp/legacy-fixtures/adm10|adm20` は計 40 KB・年数回更新見込み。Git 管理（再現性担保）か Runbook ベース（都度 `adm.snapshot.update=true` 実行）かの判断は関係者レビュー待ち。判断材料（サイズ・更新頻度・再生成コマンド・CI での扱い）は `notes/rest-touch-diff-report.md` に記録し、結論確定後に Checklist/DOC_STATUS を更新する。
