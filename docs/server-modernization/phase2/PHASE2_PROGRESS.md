...
- **RUN_ID=`20251122TorcaHttpLogZ1`（2025-11-13 22:20 JST）**: Basic 認証を `ormaster:change_me`（`docker/orca/jma-receipt-docker/docker-compose.yml` の `ORMASTER_PASS`）へ合わせたところ 401→404 に切り替わり、認証自体は突破できることを確認。`artifacts/orca-connectivity/20251122TorcaHttpLogZ1/httpdump/api01rv2_patientgetv2{,_basic}/` に 401/404 の httpdump、`logs/http_live_20251113T131848Z.log`／`http_404405_extract_20251113T131848Z.log` にタイムライン、`logs/echo_panic_stacktrace.txt` に Go Echo `runtime error: index out of range [-1]` のスタックを収集。`/opt/jma/weborca/app/log/error.log` は存在しない（`logs/app_log_dir_listing.txt`）ため、`LOGDIR=/opt/jma/weborca/log` 配下の `orca_http.log` を継続的に参照する運用へ切り替えた。`receipt_route.ini` はテンプレと同一、`online.env` / `jma-receipt.env` には Basic 情報が無い旨を Runbook に追記し、今後は両ファイルに `API_BASIC_*` コメントを残して誤用を防ぐ。
...
- **RUN_ID=`20251113T032614Z`（W44）**: WebORCA ログ永続化 override を適用。`docker/orca/jma-receipt-docker/docker-compose.override.yml` に `./logs/orca:/opt/jma/weborca/log` を定義し、`mkdir -p docker/orca/jma-receipt-docker/logs/orca` → `docker compose up -d --force-recreate orca` で orca コンテナのみ再作成。`docker exec jma-receipt-docker-orca-1 grep LOGDIR /opt/jma/weborca/app/etc/jma-receipt.env` で `LOGDIR=/opt/jma/weborca/log` を確認し、ホスト側 `docker/orca/jma-receipt-docker/logs/orca/` と同期していることを `ls -R` で採取した。結果は `artifacts/orca-connectivity/20251113T032614Z/log-persistence/success.log` へ保存し、Runbook §4.2 を「ログ永続化導入済み」へ更新済み。今後は tail/証跡採取を同ディレクトリ基準で実施する。
- **RUN_ID=`20251119TrouteLogTestZ1`（2025-11-13）**: route/log テンプレ追認タスク。`docker/orca/jma-receipt-docker/example/receipt_route.ini` を `docker cp` で `/opt/jma/weborca/app/etc/receipt_route.ini` へ上書きし、`artifacts/orca-connectivity/20251119TrouteLogTestZ1/receipt_route.ini` として吸い上げて `diff -u` が空（テンプレとの差分ゼロ）であることを証跡化。`docker ps`→`docker-ps.txt`／`COMPOSE_PROJECT_NAME=jma-receipt-docker-for-ubuntu-2204 docker compose ... ps`→`docker-compose-ps.txt`／`docker exec ... ps -ef`→`orca-ps.txt` を保存し、既存 ORCA/DB 2 コンテナのみ稼働（再作成禁止条件を順守）で route/log のホット適用が完了したことを示した。`ls -l /opt/jma/weborca/log` とホスト `ls -l docker/orca/jma-receipt-docker/logs/orca` をそれぞれ `logs/orca-log-dir-listing.txt`／`host_orca_log_dir.txt` に残し、`orca-db-install-5.2.0.log` / `orca-db-install-thistime.log` / `orca-db-patch.log` の 3 ファイルだけがマウント同期している点と `logs/*.log` へ差分ログを退避済みであることを記録。`config/jma-receipt-env-logdir.txt` には `LOGDIR=/opt/jma/weborca/log` を再取得した結果を保存済み。次アクション: HTTP ログ出力先（`REDIRECTLOG` / `/var/log/orca/`）の追跡と Runbook §4.2 への反映。

## 2025-11-13 追記: ORCA route/log plan drafted（担当: Codex）
- ORCA route/log plan drafted — `docker/orca/jma-receipt-docker/example/receipt_route.ini` を追加し、Runbook §4.5（HTTP 404/405 対応フロー）から直接参照できるよう README にテンプレ適用手順を追記。
...
- **W41 追加（2025-11-13, RUN_ID=`20251113TorcaRouteTemplateW41`）**: `docker/orca/jma-receipt-docker` 配下を `rg --files -g '*route*'` / `rg 'receipt_route'` で調査し、`receipt_route.ini` や `route.yaml` を生成するスクリプトが存在しないことを確認。再生成手順を `docs/server-modernization/phase2/operations/ORCA_CONNECTIVITY_VALIDATION.md` §4.5 Step2 に追記し、`artifacts/orca-connectivity/templates/receipt_route.template.ini`・`route.template.yaml` をテンプレとして整備した。次回検証ではテンプレを RUN_ID ディレクトリへコピー→`docker cp`→`su - orca` 再起動で 404/405 を即復旧できる状態。Evidence: [artifacts/orca-connectivity/templates/](../../../artifacts/orca-connectivity/templates/)（テンプレ） / `docs/server-modernization/phase2/operations/logs/2025-11-13-orca-connectivity.md`（route 不在ログ）。
...
- **W63 追加（2025-11-13, RUN_ID=`20251113TorcaLegacyStaffZ1`）**: Legacy Swing クライアントと `server/` コードを精査し、職員登録は `/user` REST (`UserResource`→`UserServiceBean`) だけで完結し ORCA API へは連携していないこと、`UserModel#orcaId` は手入力の doctor コードをカルテ送信時に流用しているだけであることを確認。`ext_lib/OpenDolphin-ORCA-OQS` には `getManageusersres` が定義されているが import 先がなく未配線で、firecrawl 取得仕様 (`assets/orca-tec-index/raw/api_userkanri.md`) / サンプル電文 (`manageusers_{register,update,delete}.xml`) も参照のみ。`receipt_route.ini` テンプレ（`docker/orca/jma-receipt-docker/example/receipt_route.ini`）には `/orca101` ブロックが存在せず、405 解消には WebORCA ルーティング側で POST を許可する追加作業が必要と整理した。Evidence: `docs/server-modernization/phase2/operations/logs/2025-11-13-orca-connectivity.md` 「Legacy 参照」節 / `artifacts/orca-connectivity/20251113T150730Z/manageusers/`（既存 405 証跡を再参照）。
