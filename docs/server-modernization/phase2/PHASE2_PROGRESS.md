...
- 📋 **Letter/Lab Audit 次 RUN 条件**:
  1. Legacy/Modern 両方で `psql -v run_id=<RUN_ID> -f ops/db/local-baseline/reset_d_audit_event_seq.sql` を実行し、`/tmp/d_audit_event_{before_seq_reset,seq_validation,seq_status}_<RUN_ID>.{csv,txt}` を `artifacts/parity-manual/db/<RUN_ID>/` に転送する（TraceId とシーケンスの整合性を証跡化）。
  2. `docker run --rm --network legacy-vs-modern_default -v "$PWD":/workspace -w /workspace buildpack-deps:curl bash` で helper コンテナを起動し、`BASE_URL_{LEGACY,MODERN}` を `http://server:8080/openDolphin/resources` / `http://server-modernized-dev:8080/openDolphin/resources` に設定したうえで `ops/tools/send_parallel_request.sh --profile compose PUT /odletter/letter rest_error_letter_audit` および `... GET /lab/module/WEB1001,0,5 rest_error_lab_audit` を実行する。
  3. 成果物は `artifacts/parity-manual/{letter,lab}/<RUN_ID>/` に `{legacy,modern}/response.json`, `headers/`, `meta/`、および `logs/{send_parallel_request.log,d_audit_event_*.tsv,jms_dolphinQueue_read-resource*.txt}` を揃えて保存し、`TRACEID_JMS_RUNBOOK.md §5.8` と `DOC_STATUS.md` の対象行へ RUN_ID を追記する。
  - `rest_error_stamp_data_exception`: Legacy=500（`First Commit Win Exception`）、Modern=500（`Session layer failure ... putTree`）。`artifacts/parity-manual/stamp/20251111TrestfixZ/` に 500 応答・`logs/d_audit_event_stamp.tsv`（0 行）・`logs/jms_dolphinQueue_read-resource.txt` を保存し、排他制御の残課題を継続。
