...
- ⚠️ `POST /karte/diagnosis/claim 20251109T201827Z_DIAGNOSIS` は HTTP 200 でも `diseaseHelper.vm` 未同梱のため JMS DLQ に 1 件滞留し、`d_diagnosis` も ID=1 のまま。`logs/jms_DLQ_list-messages.txt` / `db/d_diagnosis_tail.txt` を README（`artifacts/parity-manual/CLAIM_DIAGNOSIS_FIX/README.md`）にリンクし、DLQ evacuation と DB 反映を残タスクに設定。
...
- ⚠️ ホスト `localhost:9080` が応答しないため、CLI はヘルパーコンテナ経由とした。恒久対策（`socat` 等でのポート再公開）を課題化し、README に迂回手順を明記。
- 次アクション: (1) `diseaseHelper.vm` を WAR に含めて DLQ を空にする、(2) Diagnosis API 成功時の DB 挙動を再確認、(3) Legacy 側 403 の根本原因調査、(4) ホスト→9080 疎通改善または常設ヘルパーを定義、(5) `CLAIM_DIAGNOSIS_FIX` README で DLQ/DB 状態を継続更新。
...
- ⚠️ `ScheduleServiceBean` と `AppoServiceBean` は Modernized 側の `persistence.xml` に `PatientVisitModel` / `AppointmentModel` が登録されておらず、`UnknownEntityException` / `Unable to locate entity descriptor` で 500。`ops/tests/api-smoke-test/rest_error_scenarios.manual.csv` へ `rest_error_schedule_unknown_entity` / `rest_error_appo_missing_entity` を新規追加し、再現ペイロードを `ops/tests/api-smoke-test/payloads/appo_cancel_sample.json` に保存。  
- 📁 証跡: `artifacts/parity-manual/JPQL/20251109T201157Z/{PatientServiceBean,KarteServiceBean,ScheduleServiceBean,AppoServiceBean}/`, `docs/server-modernization/phase2/notes/domain-transaction-parity.md#3-2025-11-09-karte-patient-appo-schedule`  
