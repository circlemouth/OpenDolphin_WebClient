...
- **結果サマリ（RUN_ID=`20251110TnewZ`）**
  ...
  - `PUT /stamp/tree`: Legacy=500（`First Commit Win Exception`）、Modern=500（`Bad value for type long … treeBytes`）。`d_stamp_tree` DDL/seed は適用済み。Legacy 側での排他制御と Modern 側の `StampTreeModelConverter` の bytea 変換を修正する必要がある。
- **PK 揃え済み→再取得待ち**: 2025-11-11 06:23Z (`artifacts/parity-manual/db/20251111T062323Z/karte_id_check.txt`) で Legacy=10 / Modern=10 を確認できるよう `ops/db/local-baseline/local_synthetic_seed.sql` に `WEB1001` の `d_karte.id=10` 強制ロジックを追加し、Modern DB 側も `docker exec opendolphin-postgres-modernized psql -c "INSERT id=10"`→`DELETE id=6` で入れ替え後 `SELECT setval('opendolphin.hibernate_sequence',10,true);` を実行済み。Appo/Schedule/Letter/Lab/Stamp parity は Docker 再デプロイと RUN_ID `20251110TnewZ` の再取得を待機。
...
- 証跡: `artifacts/parity-manual/{appo|schedule|letter|lab|stamp}/20251110TnewZ/`（HTTP/headers/meta/`logs/{legacy,modern}_trace*.log`/`d_audit_event_{legacy,modern}.txt`/`jms_note.txt`）。前 RUN (`20251110T234440Z`) も比較用に保持。
- `rest_error_scenarios.manual.csv` / `ops/tests/api-smoke-test/README.manual.md` に RUN_ID=`20251110TnewZ` のヘッダー（MD5 password, TraceId 命名, `Content-Type`）と証跡配置テンプレを追加し、`tmp/parity-headers/<case>_<RUN_ID>.headers` を複製するだけで `rest_error_letter_fk` / `rest_error_lab_empty` / `rest_error_stamp_data_exception` が再実行できるように整理した。
...
- **アクション:** (1) AuditTrailService で TraceId を payload に保存する or `d_audit_event` に専用カラムを追加し突合可能にする。(2) `KarteBeanConverter` を null-safe 化し `trace_http_500` を 500 で揃える。(3) Legacy `LogFilter` の匿名時 NPE を解消する。(4) `d_audit_event_id_seq` の正常化と再採番手順を `TRACEID_JMS_RUNBOOK.md` に反映したうえで再取得する。
