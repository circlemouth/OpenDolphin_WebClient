...
- 接続先の正本は `docs/server-modernization/operations/ORCA_CERTIFICATION_ONLY.md`。
- `docs/server-modernization/phase2/operations/ORCA_CERTIFICATION_ONLY.md` は Legacy/Archive（参照専用）。
- 参照先の更新は `docs/DEVELOPMENT_STATUS.md` に記録済み。
...
- 接続先は `docs/server-modernization/operations/ORCA_CERTIFICATION_ONLY.md` の Trial（XML/UTF-8 + Basic）を使用する。
...
- 抜粋例（確認ポイント付き）
```text
timestamp=2026-01-05T21:56:36Z action=orca.api.request endpoint=/orca/disease status=200 apiResult=00
metric=orca_api_success_total value=1 source=api
auditEvent=ORCA_API_OPERATION operation=DISEASE_UPDATE outcome=SUCCESS runId=<RUN_ID>
```
  - `endpoint` が API 系であること（CLAIM ではない）
  - `status=200` / `apiResult=00` / `outcome=SUCCESS` が揃っていること
  - `source=api` や `auditEvent` が API フローを示していること
