...
- `/schedule/document`（`server-modernized/src/main/java/open/dolphin/rest/ScheduleResource.java`）
  - 予定カルテ作成 + CLAIM 送信。
- `/api01rv2/claim/outpatient/*`（`server-modernized/src/main/java/open/dolphin/rest/OutpatientClaimResource.java`）
...
- `server-modernized/src/test/java/open/dolphin/rest/OutpatientClaimResourceTest.java`
...
| 外来請求バンドルの取得 | `/api01rv2/claim/outpatient/*` | `/orca21/medicalmodv2/outpatient` + `/api01rv2/appointment/outpatient` | API-only では ORCA API 系の取得を基準に集計。CLAIM バンドル形式の返却は互換用途。 |
...
- **UI/テレメトリ**: `/api01rv2/claim/outpatient` の互換レスポンス維持可否。
