...
2. Apache Maven 3.9.6 以上の環境を保証（既存の新しいバージョンがある場合は `dpkg --compare-versions` とバージョンソートの両方で確認し、条件を満たせば再インストールをスキップ。未満の場合は Apache Archive を優先して取得し、失敗時のみ `downloads.apache.org`・`dlcdn.apache.org` の順にフォールバックして `mvn` コマンドを再リンク）
3. `JAVA_HOME` を `/etc/profile.d/opendolphin-java.sh` に定義
4. `ext_lib/AppleJavaExtensions.jar` と `ext_lib/iTextAsian.jar` をローカル Maven リポジトリへ登録
5. `server-modernized` モジュールと依存モジュールのビルド（テストはスキップ）
...
- `mvn -v` で `Apache Maven 3.9.6` 以上と `Java version: 17` が表示される
