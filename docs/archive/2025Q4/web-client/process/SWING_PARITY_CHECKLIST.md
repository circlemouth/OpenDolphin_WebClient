...
- メモ: `/system/license → /dolphin/license` alias 提供済み。期待ステータス 405 を維持し、405 応答を Swing parity チェックの監視基準とする。
- 監視ログ:
    - 2025-11-13（RUN_ID=`20251119TlicenseMonitorZ1`）: `ops/tests/api-smoke-test/run.sh --scenario license --profile compose` を実行したが、`http://localhost:8080/openDolphin/resources`（Legacy）と `http://localhost:9080/openDolphin/resources`（Modernized）がいずれも `curl: (7) Failed to connect to localhost port {8080|9080}` で接続不可。証跡は `artifacts/parity-manual/license/20251119TlicenseMonitorZ1/{post,get,get-system}/`（headers/meta/response）に保存済み。コンテナ再作成禁止指示のため自前で再起動は行わず、既存 `legacy-vs-modern` Compose サービスの起動状態を運用側で確認してから再試行する。
    - 2025-11-13（RUN_ID=`20251119TlicenseMonitorZ2`）: `TRACE_RUN_ID=20251119TlicenseMonitorZ2 BASE_URL_LEGACY=http://localhost:8080/openDolphin/resources BASE_URL_MODERN=http://localhost:9080/openDolphin/resources ops/tests/api-smoke-test/run.sh --scenario license --profile modernized-dev --run-id 20251119TlicenseMonitorZ2` を実行。Legacy=POST 200（body=`0`）/GET 405（`Allow: POST, OPTIONS`, `X-Trace-Id: license-20251119TlicenseMonitorZ2`）を再取得できたが、Modernized は POST/GET/GET-system すべて `HTTP 500 Internal Server Error`（HTML body）。HTTP/headers/meta/response は `artifacts/parity-manual/license/20251119TlicenseMonitorZ2/{post,get,get-system}/` に保存し、Modern 側 500 の root cause を `logs/opendolphin-server-modernized-dev.log` で追跡中。RUN_ID=`20251119TbaselineFixZ1` で Modernized DB を再構築するまで parity 判定は Pending。
    - 2025-11-13（RUN_ID=`20251119TlicenseMonitorZ2Rerun`）: `RUN_ID=20251119TbaselineFixZ1` の Modernized DB 復旧後に同シナリオを再実行したところ、Legacy/Modernized とも `POST /dolphin/license=200 (body=0)`、`GET /dolphin/license=405`、`GET /system/license=405` に復帰。新しい HTTP/headers/meta/response は `artifacts/parity-manual/license/20251119TlicenseMonitorZ2/{post,get,get-system}/rerun-20251119TlicenseMonitorZ2Rerun/`、ログは `logs/modernized_server_rerun.log`／`logs/deployment_info/20251119TlicenseMonitorZ2Rerun.txt` に保存。
    - ステータス: **STABLE (2025-11-13 17:40 JST)**。Modernized ライセンス API は `/dolphin/license` POST=200 / GET=405, `/system/license` GET=405 を再取得済みで、Swing parity の監視指標を満たしている。
- 自動監視ライン（作成中）:
    - `docs/server-modernization/phase2/notes/license-config-check.md §7` を母体に、夜間ジョブで `ops/tests/api-smoke-test/run.sh --scenario license --profile modernized-dev --run-id <RUN_ID>` を定期実行する計画をまとめた。`RUN_ID=$(date -u +%Y%m%dT%H%M%SZ)licenseNightlyZ<SEQ>`、`TRACE_RUN_ID=${RUN_ID}` を共通命名とし、Vault シークレット取得 → API スモーク → `d_audit_event` エクスポートまでを 1 本のフローにする（`sysad-license.headers` の `X-Trace-Id` も RUN_ID をそのまま使用）。
    - 成果物は `artifacts/parity-manual/license/<RUN_ID>/secrets|http|logs/` を必須とし、**Modernized** 側の `d_audit_event` に `SYSTEM_LICENSE_CHECK` が 1 件以上（`payload.status=success`）存在しない場合は Slack → PagerDuty の順で通知する。Legacy は依然として `SYSTEM_LICENSE_CHECK` を出力できないため、参考ログのみ採取する。`docs/web-client/planning/phase2/DOC_STATUS.md` の該当行ステータスは「自動化計画ドキュメント作成中」。
    - 2025-11-13 (`RUN_ID=20251120TlicenseNightlyZ1`): helper コンテナ経由でライセンス夜間ジョブのドライランを実施し、Legacy/Modernized とも `POST /dolphin/license=200` / `GET /dolphin|/system/license=405/404` を確認。`ops/db/local-baseline/local_synthetic_seed.sql` と `CREATE SEQUENCE d_audit_event_seq` を事前に適用してから `ops/tests/api-smoke-test/run.sh --scenario license --profile modernized-dev` を実行したが、`d_audit_event` には RUN_ID と突合可能な `SYSTEM_LICENSE_CHECK` が 0 件（Modernized 側は `trace_id=''`、Legacy 側は `REST_UNAUTHORIZED_GUARD` のみ）でゲート NG。Slack/PagerDuty 通知はネットワークポリシー上ドライランログ（`artifacts/parity-manual/license/20251120TlicenseNightlyZ1/notifications/`）で代替し、課題: Modernized 監査の `trace_id` / `payload.status` 未設定、Legacy 側の `SYSTEM_LICENSE_CHECK` 未出力、`TRACE_RUN_ID` が二重 prefix の 3 点を次回自動化までに解消する。
    - 2025-11-13 (`RUN_ID=20251121TlicenseNightlyZ1`): Modernized `SystemResource`／`AuditTrailService` 修正後に再実行。`TRACE_RUN_ID=20251121TlicenseNightlyZ1` で `POST /dolphin/license=200` / `GET /dolphin/license=405` を通過し、`d_audit_event`（Modernized）の `trace_id` および `payload.status=success` を確認。Legacy 側は引き続き 0 件のためゲート対象外。Slack/PagerDuty は成功のため送信せず、`notifications/{slack.log,pagerduty.log}` に `status=not_triggered` を記録。
    - 2025-11-13 (`RUN_ID=20251122TlicenseNightlyZ1`): `ops/tools/license_monitor_job.sh --profile compose` を初稼働。Vault→`fetch_license_secrets.sh`→`ops/tests/api-smoke-test/run.sh --targets modern`→`docker logs`→`d_audit_event` TSV 出力までをワンコマンド化し、Modernized のみに対して `POST /dolphin/license=200 (body=0)`／`GET /dolphin|/system/license=405` を取得。`logs/d_audit_event_modern.tsv` には `trace_id=20251122TlicenseNightlyZ1` / `payload.status=success` の行が 4 件、`legacy_count=0`。Slack／PagerDuty はどちらも `status=not_triggered` をログに記録するのみで送信なし。成果物は `artifacts/parity-manual/license/20251122TlicenseNightlyZ1/{secrets,http,logs,notifications}/`。
